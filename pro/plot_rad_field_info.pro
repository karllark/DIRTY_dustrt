    ...
    for j = 0,(size_cube[1]-1) do begin
        for k = 0,(size_cube[1]-1) do begin
            ...
            if ((n_indxs GT 0) AND (max(rad_field[i,j,k,*]) LT 1e10)) then begin
                ...
                total_rad[i,j,k] = simp_int(wave_grid[indxs],rad_field[i,j,k,indxs])
                koplot,wave_grid[indxs],rad_field[i,j,k,indxs]/total_rad[i,j,k],psym=psym,symsize=0.1
            endif
        endfor
    endfor
endfor
...
good_indxs = where(total_rad GT 0.0,n_good)
print,'n_good = ', n_good

min_val = min(total_rad[good_indxs],max=max_val)
n_histo = 50
min_val_log = alog10(min_val)
max_val_log = alog10(max_val)
binsize = (max_val_log - min_val_log)/(n_histo)
histo = histogram(alog10(total_rad[good_indxs]),min=min_val_log,max=max_val_log,binsize=binsize,locations=histo_vals,reverse_indices=r)
...
for i = 0,(n_histo-1) do begin
    ...
    if (r[i] NE r[i+1]) then begin
        for k = 0,(histo[i]-1) do begin
            ...
            ii = mi
;            print,save_mi,ii,jj,kk,total_rad[ii,jj,kk]
