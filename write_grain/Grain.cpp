...
void Grain::MakeGrain(string const &inComponentName, string const &fOpticalConstants, string const &fCalorimetry,
                      vector<float> &MasterWave, vector<float> &MasterSize, string const &Path, float a_min,
                      float a_max)
{
    ...
    if (interpSize)
    {
        MasterSize.erase(remove_if(MasterSize.begin(), MasterSize.end(), bind2nd(less<float>(), _size[0])),
                         MasterSize.end());
        MasterSize.erase(remove_if(MasterSize.begin(), MasterSize.end(), bind2nd(greater<float>(), _size[_nsize - 1])),
                         MasterSize.end());
