  ...
  if (grid_type == "res")
    {
      ...
      n_waves = int (log10 (wave_max / wave_min) / log10 ((1.0 + 2.0 * wave_res) / (2.0 * wave_res - 1.0)) + 1.0);
  ...
  else if (grid_type == "file")
    {
      ...
      for (i = 0; i < n_waves; i++)
        {
          check_input_param ("wavelength from file", in_wavelength[i], check_wave, 1e4);
  ...
  for (int i = 0; i < FitGrainDefines.getNComp (); ++i)
    {
      ...
      // {
      cout << "Working on " << FitGrainDefines.ComponentName (i) << " (" << i + 1 << " of "
           << FitGrainDefines.getNComp () << ")" << endl;
      ...
      for (int j = 0; j < nSize; ++j)
        {
          ...
          stringstream ss;
          ss << FitGrainDefines.getModelName () << "_c_" << (i + 1) << "_" << FitGrainDefines.ComponentName (i);
          ss << "_s_" << (j + 1) << ".dat";
          myfilename = ss.str ();
          ...
          for (int jj = 0; jj < nISRFs; jj++)
            {
              ...
              status = ComputeDustEmission_igrain (
                  RadiationField, cabs, wavelength, Enthalpy, CalorimetryTGrid, EquilibriumEmission, StochasticEmission,
                  TauHeating, TauCooling, TauScaling, StochasticallyHeated, FitGrainDefines.getModelName (),
                  FitGrainDefines.ComponentName (i), i, sizevals[j], j, RadiationFieldType, RadiationFieldScale[jj],
          ...
          myfile << "# " << sizevals[j] << " " << i << " " << j << " : grain size, compostion index, size index"
                 << endl;
          myfile << "# Heating and cooling timescale: " << TauHeating << " " << TauCooling << endl;
          myfile << "# Ratio of heating to cooling to determine stochastic cutoff: " << TauScaling << endl;
