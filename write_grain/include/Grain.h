...
public:
  ...
  void MakeGrain(string const& inComponentName,
		 string const& fOpticalConstants, 
		 string const& fCalorimetry, 
		 vector <float>& MasterWave, 
		 vector <float>&MasterSize, 
		 string const& Path, 
		 float a_min=-1,
		 float a_max=-1);

  // Return the name of the dust component
  inline string getComponentName(void) { return ComponentName; }

  // Return the filename of the optical constants
  inline string getOpticalFilename(void) { return OpticalFile; }
  ...
  inline float getSubTemp(void) { return SubT; }

  // Return number of grain sizes in object
  inline int getNSize(void) { return nsize; } 
  ...
  inline vector <float> getCAbs(int size_id) { return CAbs[size_id]; }

  // Return a vector with CSca tabulated at a specific size.
  inline vector <float> getCSca(int size_id) { return CSca[size_id]; }

  // Return a vector with Scattering Phase Function tabulated at a specific size.
  inline vector <float> getphFunc(int size_id) { return phFunc[size_id]; }

  // Retrun the temperature grid
  inline vector <float> getTemperature( void ) { return Temperature; }
  ...
  // Return a vector with phFunc tabulated at a specific size; here the size can be 
  // arbitrary and the function will return an interpolated phFunc
  vector <float> getphFunc(float a_size); 
  
  // Return a vector with CAbs tabulated at a specific wavelength. 
  inline vector <float> w_getCAbs( int wave_id ) {     
    vector <float> retvect(nsize); 
    for (int i=0;i<nsize;i++) retvect[i] = CAbs[i][wave_id]; 
    return retvect;
  }

  // Return a vector with CSca tabulated at a specific wavelength. 
  inline vector <float> w_getCSca( int wave_id ) {
    vector <float> retvect(nsize); 
    for (int i=0;i<nsize;i++) retvect[i] = CSca[i][wave_id];   
    return retvect; 
  }
  // Return a vector with phFunc tabulated at a specific wavelength.
  inline vector <float> w_getphFunc( int wave_id ) {
      vector <float> retvect(nsize); 
      for (int i=0;i<nsize;i++) retvect[i] = phFunc[i][wave_id];      
      return retvect;      
  }
