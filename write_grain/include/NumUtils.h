...
// Some extrapolators.
template <typename T>
inline T lextra(T &val1, T &val2, T &val3) {
  return val1 * val2 / val3;
}
template <typename T>
inline T cextra(T &val1, T &val2, T &val3) {
  T ratio = val2 / val3;
  return val1 * ratio * ratio * ratio;
}
template <typename T>
inline T qextra(T &val1, T &val2, T &val3) {
  T ratio = val2 / val3;
  return val1 * ratio * ratio * ratio * ratio;
}
template <typename T>
inline T frootextra(T &val1, T &val2, T &val3) {
  T ratio = val2 / val3;
...
template <typename T>
int maxID(vector<T> &vect);
template <typename T>
vector<int> sortID(vector<T> &vect);
template <typename T>
vector<int> sortID_src(vector<T> &vect);
template <typename T>
void sortIndexOnVector(vector<T> &vect, vector<int> &index, int beg, int end);
template <typename T, typename T1, typename T2>
T integrate(vector<T1> &x, vector<T2> &y);
template <typename T>
vector<T> interpol(vector<T> &v, vector<T> &x, vector<T> &u, int LoEx = 2,
                   int HiEx = 2);
template <typename T>
void interpolr(vector<T> &v, vector<T> &x, vector<T> &u, int LoEx = 2,
               int HiEx = 2);
template <typename T, typename T1, typename T2>
vector<T> bbodyCGS(T1 wave, vector<T2> &Temperature);
template <typename T, typename T1, typename T2>
vector<T> bbodyCGS(vector<T1> &wave, T2 Temperature);
template <class T, class T1, class T2, class T3>
vector<T> prod_bbodyCGS(T1 wave, vector<T2> &Temperature, vector<T3> &product);
template <class T, class T1, class T2, class T3>
vector<T> prod_bbodyCGS(vector<T1> &wave, T2 Temperature, vector<T3> &product);
template <class T, class T1, class T2, class T3>
vector<T> add_bbodyCGS(T1 wave, vector<T2> &Temperature, vector<T3> &add);
template <class T, class T1, class T2, class T3>
vector<T> add_bbodyCGS(vector<T1> &wave, T2 Temperature, vector<T3> &add);
template <typename T>
...
class Cube;
template <typename T>
Matrix<T> mInvert(Matrix<T> A);
template <typename T>
vector<T> poly_fit(vector<T> x, vector<T> y, int ndegree, vector<T> &yfit,
                   vector<T> &sigma);
...
template <typename T>
T sigma_clip(vector<T> &vect, T &hi_clip, T &lo_clip) {
  replace_if(vect.begin(), vect.end(), bind2nd(greater<T>(), hi_clip),
             NaN<T>());
  replace_if(vect.begin(), vect.end(), bind1st(greater<T>(), lo_clip),
             NaN<T>());
...
// "Clip" max/min values from a vector.  The nMax largest and nMin smalles
// values are replaced with nans.
template <typename T>
void minmax_clip(vector<T> &vect, long nMin, long nMax) {
...
void sortIndexOnVector(vector<T> &vect, vector<int> &index, int beg, int end) {
  ...
  for (int j = beg + 1; j < end; j++) {
    ...
    while (i > beg && vect[index[i - 1]] > a) {
      ...
      // cout << "Swapping " << vect[index[i]] << " " << vect[index[i-1]] <<
      // endl;
...
template <typename T>
vector<T> interpol(vector<T> &v, vector<T> &x, vector<T> &u, int LoEx,
                   int HiEx) {
  if (v.size() != x.size()) {
...
template <typename T>
void interpolr(vector<T> &v, vector<T> &x, vector<T> &u, int LoEx, int HiEx) {
  if (v.size() != x.size()) {
    cout << "Abscissa and ordinate lengths do not match in interpol" << endl;
    throw "size mismatch";
  }
...
vector<T> bbodyCGS(T1 wave, vector<T2> &Temperature) {
  ...
  /*       cout << Constant::PLANCKLIGHT << " " << Constant::BOLTZMAN << " " <<
   * wave << endl;  */
  ...
  for (_it = _itbeg; _it != _itend; ++_it, ++_itBB) {
    // cout << "INBB 1: " << static_cast<T1>(wave) << " " <<
    // c1/pow((static_cast<T1>(wave)),5) << endl; cout <<
    // (std::exp((c2/static_cast<T1>((*_it))))) << endl;
...
vector<T> prod_bbodyCGS(T1 wave, vector<T2> &Temperature, vector<T3> &product) {
  ...
  for (_it = _itbeg; _it != _itend; ++_it, ++_itp, ++_itBB) {
    *_itBB =
        (*_itp) * c1 / pow((wave), 5) * (1.0 / (std::exp(c2 / (*_it)) - 1.0));
  }
  return theBB;
}
// ****************************************************************************
...
vector<T> add_bbodyCGS(T1 wave, vector<T2> &Temperature, vector<T3> &add) {
  ...
  for (_it = _itbeg; _it != _itend; ++_it, ++_ita, ++_itBB) {
    *_itBB =
        (c1 / pow((wave), 5) * (1.0 / (std::exp(c2 / (*_it)) - 1.0))) + *_ita;
  }
  return theBB;
}
// ****************************************************************************
...
vector<T> prod_bbodyCGS(vector<T1> &wave, T2 Temperature, vector<T3> &product) {
  ...
  for (_it = _itbeg; _it != _itend; ++_it, ++_itp, ++_itBB)
    *_itBB =
        (*_itp) * c1 / pow((*_it), 5) * (1.0 / (std::exp(c2 / (*_it)) - 1.0));
...
vector<T> add_bbodyCGS(vector<T1> &wave, T2 Temperature, vector<T3> &add) {
  ...
  for (_it = _itbeg; _it != _itend; ++_it, ++_ita, ++_itBB)
    *_itBB =
        (c1 / pow((*_it), 5) * (1.0 / (std::exp(c2 / (*_it)) - 1.0))) + *_ita;
...
vector<T> poly_fit(vector<T> x, vector<T> y, int ndegree, vector<T> &yfit,
  ...
  if (yfit.size() != n) {
    cout << "Size of fit return not equal input vector sizes; poly_fit()"
         << endl;
    exit(8);
  }
  if (sigma.size() != m) {
    cout << "Size of parameter error array not equal to order+1; poly_fit()"
         << endl;
    exit(8);
  }
  ...
  vector<T> diff;
  transform(yfit.begin(), yfit.end(), y.begin(), back_inserter(diff),
            minus<T>());
...
template <typename T>
NumUtils::Matrix<T> mInvert(NumUtils::Matrix<T> A) {
  int N = A.nRow();
...
// Cube class.
template <typename T>
class Cube : public std::vector<T> {
 public:
  // Constructors/destructors.
  Cube() : std::vector<T>() {}
...
// Overload of operator()
template <typename T>
inline T &Cube<T>::operator()(int n1_id, int n2_id, int n3_id) {
  if (n1_id < 0 || n1_id >= _n1) {
    cout << "out_of_range Cube::operator(), element 1 " << n1_id << "," << n2_id
         << "," << n3_id << " " << _n1 << endl;
    std::string ExceptionObject = "out_of_range Cube::operator(), element 1";
    throw std::out_of_range(ExceptionObject);
  }
  if (n2_id < 0 || n2_id >= _n2) {
    cout << "out_of_range Cube::operator(), element 2 " << n1_id << "," << n2_id
         << "," << n3_id << " " << _n2 << endl;
    std::string ExceptionObject = "out_of_range Cube::operator(), element 2";
    throw std::out_of_range(ExceptionObject);
  }
  if (n3_id < 0 || n3_id >= _n3) {
    cout << "out_of_range Cube::operator(), element 3 " << n1_id << "," << n2_id
         << "," << n3_id << " " << _n3 << endl;
...
template <typename T>
void Cube<T>::CSize(int n1_id, int n2_id, int n3_id) {
  _n1 = n1_id;
  _n2 = n2_id;
  _n3 = n3_id;
  Cube::clear();
...
// Overload of operator()
template <typename T>
inline T &FourVector<T>::operator()(int n1_id, int n2_id, int n3_id,
                                    int n4_id) {
  if (n1_id < 0 || n1_id >= _n1) {
    cout << "out_of_range FourVector::operator(), element 1 " << n1_id << ","
         << n2_id << "," << n3_id << " " << n4_id << " " << _n1 << endl;
    std::string ExceptionObject =
        "out_of_range FourVector::operator(), element 1";
    throw std::out_of_range(ExceptionObject);
  }
  if (n2_id < 0 || n2_id >= _n2) {
    cout << "out_of_range FourVector::operator(), element 2 " << n1_id << ","
         << n2_id << "," << n3_id << " " << n4_id << " " << _n2 << endl;
    std::string ExceptionObject =
        "out_of_range FourVector::operator(), element 2";
    throw std::out_of_range(ExceptionObject);
  }
  if (n3_id < 0 || n3_id >= _n3) {
    cout << "out_of_range FourVector::operator(), element 3 " << n1_id << ","
         << n2_id << "," << n3_id << " " << n4_id << " " << _n3 << endl;
    std::string ExceptionObject =
        "out_of_range FourVector::operator(), element 3";
    throw std::out_of_range(ExceptionObject);
  }
  if (n4_id < 0 || n4_id >= _n4) {
    cout << "out_of_range FourVector::operator(), element 4 " << n1_id << ","
         << n2_id << "," << n3_id << " " << n4_id << " " << _n4 << endl;
    std::string ExceptionObject =
        "out_of_range FourVector::operator(), element 3";
    throw std::out_of_range(ExceptionObject);
  }
  return *(this->begin() + n4_id * _n1 * _n2 * _n3 + n3_id * _n1 * _n2 +
           n2_id * _n1 + n1_id);
}

// Size the FourVector after instatiation.
template <typename T>
void FourVector<T>::FVSize(int n1_id, int n2_id, int n3_id, int n4_id) {
  _n1 = n1_id;
  _n2 = n2_id;
  _n3 = n3_id;
  _n4 = n4_id;
...
template <typename T>
int FourVector<T>::nRow() {
  return _n1;
}
template <typename T>
int FourVector<T>::nCol() {
  return _n2;
}
template <typename T>
int FourVector<T>::n3rd() {
  return _n3;
}
template <typename T>
int FourVector<T>::n4th() {
  return _n4;
}
...
// given the index of the cell, return the x, y and z indices
template <typename T>
inline void FourVector<T>::get_xyzz(int cell_num, int &n1_id, int &n2_id,
                                    int &n3_id, int &n4_id) {
  div_t div_result;
  if (cell_num < 0 || cell_num > (int)this->size()) {
