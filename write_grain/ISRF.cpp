...
ISRF::ISRF(vector<float> in_wave, float in_XMMP) {
  ...
  for (iW = _W.begin(); iW != _W.end(); iW++, ++iT) {
    ...
    for (iTempISRF = _TempISRF.begin(); iTempISRF != _TempISRF.end();
         iTempISRF++, ++iISRF) {
      *iTempISRF *= *iW;
  ...
  // float UVMod;
  loIter =
      find_if(wave.begin(), wave.end(), bind2nd(greater_equal<float>(), UVMin));
  if (loIter != wave.end()) {
    hiIter =
        find_if(wave.begin(), wave.end(), bind2nd(greater<float>(), UVMax));
    int loIdx = distance(wave.begin(), loIter);
    int hiIdx = distance(wave.begin(), hiIter);
    for (int i = loIdx; i < hiIdx; i++)
      theISRF[i] +=
          _UVP[0] * exp(-pow((wave[i] - _UVP[1]) / _UVP[2], 2) / 2.0) +
          _UVP[3] + _UVP[4] * wave[i] + _UVP[5] * wave[i] * wave[i];
  ...
  transform(theISRF.begin(), theISRF.end(), theISRF.begin(),
            bind2nd(multiplies<float>(), UtoJ));
  // cout << "DONE!" << endl;
}
