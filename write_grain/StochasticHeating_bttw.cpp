...
int ComputeGrid(vector<float> &enth, vector<float> &denth, vector<float> &temp,
                vector<float> &tgrid, vector<float> &Temperature,
                vector<float> &Enthalpy, float &TMax, float &TMin, int &nBins);

int ComputeTransitionMatrix(vector<vector<double>> &TM, vector<float> &wave,
                            vector<float> &temp, vector<float> &enth,
                            vector<float> &cabs, vector<float> &cJprod,
                            vector<float> &denth, int &nBins);

int SolveTransitionMatrix(vector<vector<double>> &TM, int &nBins,
                          vector<double> &P);

int StochasticHeating_bttw(vector<float> &wave, vector<float> &cJprod,
                           vector<float> &cabs, vector<float> &Temperature,
                           vector<float> &Enthalpy, float &EAbs, float &TMin,
                           float &TMax, vector<double> &Emission,
  ...
  for (int i = 0; i < nbins; ++i) TransitionMatrix[i].resize(nbins, 0);

  // Initial setup
  status = ComputeGrid(enth, denth, temp, tgrid, Temperature, Enthalpy, TMax,
                       TMin, nbins);
  ...
  status = ComputeTransitionMatrix(TransitionMatrix, wave, temp, enth, cabs,
                                   cJprod, denth, nbins);
  if (status != Flags::FSUCCESS) return status;
  status =
      SolveTransitionMatrix(TransitionMatrix, nbins, ProbabilityDistribution);
  if (status != Flags::FSUCCESS) return status;
  ...
  idx = nbins - 1;
  while (ProbabilityDistribution[idx] <= PTol && idx > 0) --idx;
  ...
  if (ProbabilityDistribution[idx] < PTol) {
    while (ProbabilityDistribution[idx] < PTol) ++idx;
  ...
  // Trim the grid, from upper limit
  idx = nbins - 1;
  if (ProbabilityDistribution[idx] > PTol) TMax *= 1.5;
  idx = 0;
  if (ProbabilityDistribution[idx] < PTol) {
    while (ProbabilityDistribution[idx] < PTol) ++idx;
    if (idx > nbins - 1) return Flags::FST_SMALL_PROBABILITY_LO;
  ...
  for (int i = 0; i < nwave; ++i) {
    Probability_BBody = NumUtils::prod_bbodyCGS<double>(
        wave[i], temp, ProbabilityDistribution);  // P(T)*B(T)
    ProbabilitySum = 0.0;                         // Redundant...
    ProbabilitySum =
        accumulate(Probability_BBody.begin(), Probability_BBody.end(),
                   0.0);  // Sum_T (P(T)*B(T))
...
int ComputeGrid(vector<float> &enth, vector<float> &denth, vector<float> &temp,
  ...
  float _DT =
      (TMax - TMin) / static_cast<float>(nBins);  // Linear temperature grid
...
int ComputeTransitionMatrix(vector<vector<double>> &TM, vector<float> &wave,
  ...
  double c1 =
      2.0 * Constant::PLANCK * pow(Constant::LIGHT, 2) * Constant::PLANCKLIGHT;
  ...
  for (int f = 0; f < nBins; ++f) {  // Loop over final energy states
    ...
    if (f != nBins - 1) {  // Cooling transitions - only f+1 to f
      ...
      for (it = itb; it != ite; ++it, ++it0, ++it1)
        *it0 = (*it1 / pow(*it, 5)) *
               (1.0 / (std::exp(c2 / ((*it) * temp[f + 1])) - 1.0));
      TM[f][f + 1] = c1 / (enth[f + 1] - enth[f]) *
                     NumUtils::integrate<double>(wave, _integrand);
      ...
      if (_wT < wave[0] || _wT > wave[_nw - 1])
        ...
        if (idx == 0 || idx == _nw - 1)
          ...
          if ((wave[idx] - wave[idx - 1]) != 0)
            _slp =
                (cJprod[idx] - cJprod[idx - 1]) / (wave[idx] - wave[idx - 1]);
          else
        ...
        TM[f][i] =
            Constant::IPLANCKLIGHT * _thiscjprod * _wT * _wT * _wT * denth[f];
        if (f == nBins - 1 && idx > 0) {  // Put all transitions out of defined
...
int SolveTransitionMatrix(vector<vector<double>> &TM, int &nBins,
  ...
  _Bij.resize(nBins);
  for (int i = 0; i < nBins; ++i) _Bij[i].resize(nBins);
