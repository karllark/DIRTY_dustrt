...
int StochasticHeating_bttw(vector <float> & wave, 
			   vector <float> & cJprod, 
			   vector <float> & cabs, 
...
{ 
  ...
  //for (uint i=0;i<temp.size();++i) cout << i << " " << temp[i] << " " << enth[i] << " " << denth[i] << endl; 
...
//   }
  ...
  //for (uint i=0;i<temp.size();++i) cout << i << " " << tgrid[i] << " " << temp[i] << " " << tgrid[i+1] << endl; 
  ...
  //for (int i=0;i<nbins;++i) cout << i << " " << temp[i] << " " << ProbabilityDistribution[i] << endl; 
  ...
  status = ComputeGrid(enth,denth,temp,tgrid,Temperature,Enthalpy,TMax,TMin,nbins); 
  //for (int i=0;i<nbins;++i) cout << i << " " << temp[i] << " " << enth[i] << " " << denth[i] << endl;  
  if (status != Flags::FSUCCESS) return status;
  status = ComputeTransitionMatrix(TransitionMatrix,wave,temp,enth,cabs,cJprod,denth,nbins);
  if (status != Flags::FSUCCESS) return status; 
  status = SolveTransitionMatrix(TransitionMatrix,nbins,ProbabilityDistribution);
  ...
  for (int i=0;i<nwave;++i) { 
    ...
    ProbabilitySum = accumulate(Probability_BBody.begin(),Probability_BBody.end(),0.0);   // Sum_T (P(T)*B(T))
  ...
  //cout << "Energy Balance: " << EnergyEmitted << " " << EAbs << " " << ETol << " " << TMax << " " << TMin << endl; 
  //for (uint i=0;i<temp.size();i++) cout << i << " " << temp[i] << " " << ProbabilityDistribution[i] << endl;
