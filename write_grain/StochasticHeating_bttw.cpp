// Jun 3-4 2009: Changed return value to int indicating status.  StochasticLum is now returned in
// the parameter list.
//               Added DirtyFlags.h dependency.
//               Modified return values of all sub calls to be int - will return status to be passed
//               back up the line -KAM
#include <iostream>
#include <vector>
...
int StochasticHeating_bttw(vector<float> &wave, vector<float> &cJprod, vector<float> &cabs, vector<float> &Temperature,
                           vector<float> &Enthalpy, float &EAbs, float &TMin, float &TMax, vector<double> &Emission,
                           vector<double> &ProbabilityDistribution, vector<float> &temp)
{
    ...
    // for (uint i=0;i<temp.size();++i) cout << i << " " << temp[i] << " " << enth[i] << " " <<
    // denth[i] << endl;
    if (status != Flags::FSUCCESS)
        return status;
    // refine the grid limits using maximum photon energy to truncate the enthalpy grid
    //   idx = 1;
    ...
    // for (uint i=0;i<temp.size();++i) cout << i << " " << tgrid[i] << " " << temp[i] << " " <<
    // tgrid[i+1] << endl;
    ...
    // for (int i=0;i<nbins;++i) cout << i << " " << temp[i] << " " << ProbabilityDistribution[i] <<
    // endl;
    ...
    // Refine nbins so we don't have too many elements at essentially the same enthalpy.
    ...
    status = ComputeGrid(enth, denth, temp, tgrid, Temperature, Enthalpy, TMax, TMin, nbins);
    // for (int i=0;i<nbins;++i) cout << i << " " << temp[i] << " " << enth[i] << " " << denth[i] <<
    // endl;
    if (status != Flags::FSUCCESS)
        return status;
    status = ComputeTransitionMatrix(TransitionMatrix, wave, temp, enth, cabs, cJprod, denth, nbins);
    ...
    {
        Probability_BBody = NumUtils::prod_bbodyCGS<double>(wave[i], temp, ProbabilityDistribution); // P(T)*B(T)
        ...
        Emission[i] = integrand[i]; // this is C(lam)*Sum_T (P(T)*B(T,lam)) ~ stochastic L(lam)
    }
    EnergyEmitted = NumUtils::integrate<double>(wave, integrand);
    ETol = abs(EAbs - EnergyEmitted) / EAbs;
    // cout << "Energy Balance: " << EnergyEmitted << " " << EAbs << " " << ETol << " " << TMax << " "
    // << TMin << endl; for (uint i=0;i<temp.size();i++) cout << i << " " << temp[i] << " " <<
    ...
    { // Loop over final energy states
        ...
        { // Heating transitions - all initial < final.
            ...
            { // Photons exist that will induce this transition
                ...
                if (f == nBins - 1 && idx > 0)
                { // Put all transitions out of defined states into last state.
                    it0 = wave.begin() + idx;
