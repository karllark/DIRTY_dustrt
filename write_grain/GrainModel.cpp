...
GrainModel::MakeGrainModel (ConfigFile &_mbkcf, vector<float> &MasterWave)
{
  ...
  for (int cmp = 0; cmp < nComp; cmp++)
    { // Loop over all components
            ...
            case 3: // Gaussian
              {
                ...
                if (_mcf.isBadFloat (MeanMolecularWeight))
                  {
                    cout
                        << "Mean molecular weight not defined - required for this size distribution"
                        << endl;
                ...
                // dimensionless, AMU_CGS is gm/H, the dimensions os _newAmp are cm^-1 H^-1 ...
                // float _newAmp =
      ...
      // Generate the normalized size distribution to avoid computational loops everytime it's
      // called
  ...
  // Total normalization; this is the denominator for C_abs,sca size integrated, comp summed
  // quantities.
    ...
    { // Loop over wavelength
      ...
      for (int cmp = 0; cmp < nComp; cmp++)
        { // Loop over Chemical composition
          ...
          // for (ithisSize = _thisSize.begin()+1;ithisSize!=_thisSize.end();ithisSize++) { //
          // Integration loop.
