...
void FitGrains::MakeFitGrains(ConfigFile & _mbkcf, 
  ...
  for (int cmp=0;cmp<nComp;cmp++) {  // Loop over all components
    ...
    Component[cmp].MakeGrain(_thisComponentName,
			     _CrossSectionsPath+_thisCrossSectionFile,
...
vector <float> FitGrains::Wave( int cmp ) 
{ 
  return Component[cmp].getWave(); 
} 

int FitGrains::nWave ( int cmp )
{
  return Component[cmp].getNWave(); 
}

int FitGrains::getNComp ( void ) 
{ 
  return nComp; 
}

string FitGrains::getModelName ( void ) 
{ 
  return ModelName; 
}

// These functions are 'pass through' to the relevant Grain objects
string FitGrains::ComponentName( int cmp) 
{ 
  return Component[cmp].getComponentName(); 
}

string FitGrains::OpticalFilename( int cmp) 
{ 
  return Component[cmp].getOpticalFilename(); 
}

int FitGrains::nSize ( int _cmp ) 
{ 
  return Component[_cmp].getNSize(); 
}

vector <float> FitGrains::Size ( int _cmp ) 
{ 
  return Component[_cmp].getSize(); 
}

float FitGrains::Size ( int _cmp, int _szid ) 
{ 
  return Component[_cmp].getSize(_szid); 
}

vector <float> FitGrains::CAbs ( int _cmp, int _szid ) 
{ 
  return Component[_cmp].getCAbs(_szid); 
}

vector <float> FitGrains::CSca ( int _cmp, int _szid ) 
{ 
  return Component[_cmp].getCSca(_szid); 
}

vector <float> FitGrains::phFunc ( int _cmp, int _szid ) 
{ 
  return Component[_cmp].getphFunc(_szid); 
}

float FitGrains::Density ( int _cmp ) 
{ 
  return Component[_cmp].getDensity(); 
}

vector <float> FitGrains::CAbs ( int _cmp, float _sz ) 
{ 
  return Component[_cmp].getCAbs(_sz); 
}

vector <float> FitGrains::CSca ( int _cmp, float _sz ) 
{ 
  return Component[_cmp].getCSca(_sz); 
}

vector <float> FitGrains::phFunc ( int _cmp, float _sz ) 
{ 
  return Component[_cmp].getphFunc(_sz); 
}

vector <float> FitGrains::wCAbs ( int _cmp, int _wid ) 
{ 
  return Component[_cmp].w_getCAbs(_wid); 
}

vector <float> FitGrains::CalTemp ( int _cmp ) 
{ 
  return Component[_cmp].getTemperature(); 
}

vector <float> FitGrains::SpecificHeatCapacity ( int _cmp ) 
{ 
  return Component[_cmp].getSpecificHeatCapacity(); 
}

vector <float> FitGrains::HeatCapacity ( int _szid, int _cmp ) 
{ 
  return Component[_cmp].getHeatCapacity(_szid); 
} 

vector <float> FitGrains::SpecificEnthalpy ( int _cmp ) 
{ 
  return Component[_cmp].getSpecificEnthalpy(); 
}

vector <float> FitGrains::Enthalpy ( int _szid, int _cmp ) 
{ 
