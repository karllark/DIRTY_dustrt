...
extern int StochasticHeating_bttw (vector<float> &wave, vector<float> &J, vector<float> &cabs,
                                   vector<float> &Temp, vector<float> &Enthalpy, float &EAbs,
...
int
ComputeDustEmission_igrain (vector<float> &J, vector<float> &CAbs, vector<float> &Wave,
                            ...
                            int ComponentIndex, float Size, int SizeIndex,
                            string RadiationFieldType, float RadiationFieldScale,
  ...
  if (StochasticallyHeated)
    {
      ...
      status
          = StochasticHeating_bttw (Wave, _cJprod, CAbs, CalorimetryTGrid, Enthalpy, EAbs, _mintemp,
                                    _maxtemp, StochasticEmission, ProbabilityDistribution, temp);
      if (status != Flags::FSUCCESS)
        cout << " Huh..." << status << endl;

      ss << ModelName << "_c_" << ComponentIndex + 1 << "_" << ComponentName << "_s_"
         << SizeIndex + 1 << "_" << RadiationFieldType << "_" << RadiationFieldScale
         << "_PDIST.dat";
      pdistFilename = ss.str ();
      pdistStream.open (pdistFilename.c_str ());
      pdistStream << "# " << ModelName << endl;
      pdistStream << "# Component, Size Index, Size: " << ComponentName << " " << SizeIndex << " "
                  << Size << endl;
      pdistStream << "# Tau Heat, Cool, Ratio, Termination Ratio: " << TauHeating << " "
                  << TauCooling << " " << TauCooling / TauHeating << " " << TauScaling << endl;
