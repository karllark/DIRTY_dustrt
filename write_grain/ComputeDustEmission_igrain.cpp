...
extern int StochasticHeating_bttw(vector<float> &wave, vector<float> &J,
                                  ...
                                  vector<float> &Enthalpy, float &EAbs,
...

{
  ...
  status = EqTemp(Wave, J, CAbs, EAbs, EquilibriumGrainTemperature, _mintemp,
                  _maxtemp);
  ...
  if (StochasticallyHeated) {
    ...
    ss << ModelName << "_c_" << ComponentIndex + 1 << "_" << ComponentName
       ...
       << RadiationFieldScale << "_PDIST.dat";
    pdistFilename = ss.str();
    pdistStream.open(pdistFilename.c_str());
    pdistStream << "# " << ModelName << endl;
    pdistStream << "# Component, Size Index, Size: " << ComponentName << " "
                << SizeIndex << " " << Size << endl;
    pdistStream << "# Tau Heat, Cool, Ratio, Termination Ratio: " << TauHeating
                << " " << TauCooling << " " << TauCooling / TauHeating << " "
                << TauScaling << endl;
    pdistStream << "# Radition Field: " << RadiationFieldType << " "
                << RadiationFieldScale << " " << RadiationFieldTemperature
                << endl;
