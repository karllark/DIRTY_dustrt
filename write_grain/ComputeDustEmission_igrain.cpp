...
extern int StochasticHeating_bttw(vector <float> & wave, 
				  vector <float> & J, 
				  vector <float> & cabs, 
				  vector <float> & Temp, 
...
int ComputeDustEmission_igrain ( vector <float>  & J, 
				 vector <float>  & CAbs, 
				 vector <float>  & Wave, 
				 vector <float>  & Enthalpy, 
...

{
  ...
  if (StochasticallyHeated) { 
    ...
    status = StochasticHeating_bttw(Wave,_cJprod,CAbs,CalorimetryTGrid,Enthalpy,EAbs,_mintemp,_maxtemp,StochasticEmission,ProbabilityDistribution,temp); 
    if (status != Flags::FSUCCESS) cout << " Huh..." << status << endl; 

    ss << ModelName << "_c_" << ComponentIndex+1 << "_" << ComponentName << "_s_" << SizeIndex+1 << "_" << RadiationFieldType << "_" << RadiationFieldScale << "_PDIST.dat"; 
    ...
    pdistStream << "# Tau Heat, Cool, Ratio, Termination Ratio: " << TauHeating << " " << TauCooling << " " << TauCooling/TauHeating << " " << TauScaling << endl; 
    pdistStream << "# Radition Field: " << RadiationFieldType << " " << RadiationFieldScale << " " << RadiationFieldTemperature << endl; 
    pdistStream << "# Temperature  PDist" << endl; 
    for (uint i=0;i<ProbabilityDistribution.size();++i) pdistStream << temp[i] << " " << ProbabilityDistribution[i] << endl; 
