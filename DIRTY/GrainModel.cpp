...
{ 
  ...
  for (int cmp=0;cmp<nComp;cmp++) {  // Loop over all components
    ...
    // SizeTypeID is a STRING (_thisSizeType) to INTEGER map defined in GrainModel.h
    // NULL  -> 0  ; size grid will be taken from the optical constants definition file
    // DEF   -> 0  ; size grid will be taken from the optical constants definition file
    // FILE  -> 1  ; size grid will be read from an on disk tabulation
    // NSIZE -> 2  ; size grid will be constructed from amin,amax,nsize for each component
      ...
      case 1: 
          ...
          //     to cm via Constant::UM_CM
            ...
            // Trim the master size to our min/max
	          _MasterSize.erase(remove_if(_MasterSize.begin(),_MasterSize.end(),bind2nd(less<float>(),   _thisA_min)),_MasterSize.end());
	          _MasterSize.erase(remove_if(_MasterSize.begin(),_MasterSize.end(),bind2nd(greater<float>(),_thisA_max)),_MasterSize.end());
          ...
          cout << "Generating size grid internally" << endl;
	        if (_thisSizeIN <= 0) { 
	          ...
	          if (_thisSizeScale == _mcf.BadString()) {
	            ...
	            transform(_thisSizeScale.begin(),_thisSizeScale.end(),_thisSizeScale.begin(),(int(*)(int))toupper);
	          } 
    ...
    if (SizeDistID.find(_thisSizeDistribution) != SizeDistID.end()) {  // Determine which sd to use
      ...
      cout << _thisSizeDistribution << " is not a defined size distribution type." << endl; 
      cout << "Allowed size distribution types are: " << endl; 
      for (iMap=SizeDistID.begin();iMap!=SizeDistID.end();iMap++) cout << iMap->first << " " << iMap->second << endl;
      cout << "****************************************************" << endl; 
      exit(8); 
    ...
    _integrand.resize(Component[cmp].nsize); 
    for (int sz=0;sz<Component[cmp].nsize;sz++) _integrand[sz]=Component[cmp].mass[sz]*SizeDistribution[cmp][sz];
    ...
    // Compute the 'volume' in this component; we could roll into the loop above to save computation at the cost
    // of additional RAM, but computational cost at this stage of the process is not a large factor, RAM may be.
    for (int sz=0;sz<Component[cmp].nsize;sz++) _integrand[sz]=Constant::VFAC*pow(Component[cmp].size[sz],3)*SizeDistribution[cmp][sz];
    ...
    transform(SizeDistribution[cmp].begin(),SizeDistribution[cmp].end(),back_inserter(SizeDistributionNorm[cmp]),bind2nd(multiplies<float>(),Normalization[cmp]));
  ...
  for (int wv=0;wv<nWave;wv++) { // Loop over wavelength
    ...
    for (int cmp=0;cmp<nComp;cmp++) { // Loop over Chemical composition
      ...
      for (ithisSize = Component[cmp].size.begin()+1;ithisSize!=Component[cmp].size.end();ithisSize++) { // Integration loop.
	      ...
	      _phFuncIntegral += delta*( (*ithisphFunc)*(*ithisCSca)*(*ithisSizeDist) + (*(ithisphFunc-1))*(*(ithisCSca-1))*(*(ithisSizeDist-1)) );
...
vector <float> GrainModel::getZDA_sdist(vector <float> coeff, int cmp) 
{
    ...
    *iretvec = coeff[1];           // log(g) = c0
    if (!isnan(coeff[2])) {         // b0 is defined
      *iretvec += coeff[2]*log10(thissz);
      //cout << "b0: " << cmp << " " << thissz << " " << coeff[2]*log10(thissz) << endl; 
    }
    if (!isnan(coeff[3])) {         // b1 is defined
      *iretvec -= coeff[3]*pow(fabs(log10(thissz/coeff[4])),coeff[5]);
      //cout << "b1: " << cmp << " " << thissz << " " << coeff[3] << " " << coeff[4] << " " << coeff[5] << endl; 
      //cout << "    " << cmp << " " << thissz << " " << pow(fabs(log10(thissz/coeff[4])),coeff[5]) << endl; 
    }
    if (!isnan(coeff[6])) {          // b2 is defined 
      *iretvec -= coeff[6]*pow(fabs(log10(thissz/coeff[7])),coeff[8]);
      //cout << "b2: " << cmp << " " << thissz << " " << coeff[6] << " " << coeff[7] << " " << coeff[8] << endl; 
      //cout << "    " << cmp << " " << thissz << " " << pow(fabs(log10(thissz/coeff[7])),coeff[8]) << endl;
    }
    if (!isnan(coeff[9])) {          // b3 is defined
      *iretvec -= coeff[9]*pow(fabs( (thissz) - coeff[10]),coeff[11]);
      //cout << "b3: " << cmp << " " << thissz << " " <<  coeff[9]*pow(fabs( (thissz) - coeff[10]),coeff[11]) << endl; 
    }
    if (!isnan(coeff[12])) {         // b4 is defined
      *iretvec -= coeff[12]*pow(fabs( (thissz) - coeff[13]),coeff[14]); 
      //cout << "b4: " << cmp << " " << thissz << " " <<  coeff[12]*pow(fabs( (thissz) - coeff[13]),coeff[14]) << endl; 
    }
  ...
  //for (int i=0;i<retvec.size();++i) cout << Component[cmp].size[i] << " " << retvec[i] << endl;
  float _Nfac = 1.0/NumUtils::integrate<float>(Component[cmp].size,retvec); 
  //for (iretvec=retvec.begin();iretvec!=retvec.end();++iretvec) cout << *iretvec << endl; 
  //cout << "The normalization was: " << _Nfac << endl << endl;
...
vector <float> GrainModel::getHD_sdist(vector <float> coeff, int cmp)
{
  ...
  // the size grid Component[cmp].size() should be in CM, so just make sure anywere
  // we ratio to other sizes, we are consistent.  Since these distributions are of 
  // the form FUNCTION/size and size is in CM, we are returning cm^-1 H^-1
  iretvec = retvec.begin(); 

  // Hensley & Draine size distribution functions have mixed units (because of course they do) 
  // - convert where necessary to ensure out put in cm^-1 H^-1.  Since some of the random
  // selection of units occur in powers of natural logs in sum of expoentials, not trivial
  // to absord into a redefinition of constants. 
  ...
  for (isz=Component[cmp].size.begin();isz!=Component[cmp].size.end();isz++,iretvec++) {
    *iretvec = (coeff[0]/(*isz))*exp((-1)*(pow(log((*isz)/(coeff[2]*Constant::UM_CM)),2)/_sig));
    ...
    if (coeff[1] != 0) *iretvec += (coeff[1]/(*isz))*exp((-1)*(pow(log((*isz)/(coeff[3]*Constant::UM_CM)),2)/_sig)); 
    //cout << *isz << " " << *iretvec << endl;
    if (coeff[5] != 0) { 
...
float GrainModel::getTau( float a_wave) 
{
  ...
  if (it_loc != Wave.end() ) { // Found an exact match. 
    ...
    //cout << Tau[idx]+slope*(a_wave-Wave[idx]) << " " << Tau[idx-1]+slope*(a_wave-Wave[idx-1]) << endl;
    //return Tau[idx]+slope*(a_wave-Wave[idx]);
