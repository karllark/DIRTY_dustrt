  ...
  for (int cmp = 0; cmp < nComp; cmp++)
    { // Loop over all components
      ...
      for (int sz = 0; sz < Component[cmp].nsize; sz++)
        _integrand[sz] = Component[cmp].mass[sz] * SizeDistribution[cmp][sz];
      DustMass[cmp]
          = Normalization[cmp] * NumUtils::integrate<float> (Component[cmp].size, _integrand);
      ...
      for (int sz = 0; sz < Component[cmp].nsize; sz++)
        _integrand[sz]
            = Constant::VFAC * pow (Component[cmp].size[sz], 3) * SizeDistribution[cmp][sz];
      DustVolume[cmp]
          = Normalization[cmp] * NumUtils::integrate<float> (Component[cmp].size, _integrand);

      number[cmp] = Normalization[cmp]
      ...
      for (int cmp = 0; cmp < nComp; cmp++)
        { // Loop over Chemical composition
          ...
          for (ithisSize = Component[cmp].size.begin () + 1;
               ithisSize != Component[cmp].size.end (); ithisSize++)
            { // Integration loop.
              delta = 0.5 * ((*ithisSize) - *(ithisSize - 1));
              // C(abs,sca) = int(C(a)*f(a)da)
              _CAbsIntegral += delta
                               * ((*ithisCAbs) * (*ithisSizeDist)
                                  + *(ithisCAbs - 1) * (*(ithisSizeDist - 1)));
              _CScaIntegral += delta
                               * ((*ithisCSca) * (*ithisSizeDist)
  ...
  for (isz = Component[cmp].size.begin (); isz != Component[cmp].size.end (); isz++)
    {
      ...
      *iretvec = coeff[3] / (*isz)
                 * pow ((*isz / (coeff[4] * static_cast<float> (Constant::UM_CM))), coeff[6]);
      if (coeff[7] >= 0.0)
      ...
      if ((*isz) > (coeff[4] * Constant::UM_CM))
        {
          *iretvec *= exp (
              -1 * pow (((*isz) - coeff[4] * Constant::UM_CM) / (coeff[5] * Constant::UM_CM), 3));
        }
...
GrainModel::getHD_sdist (vector<float> coeff, int cmp)
{
  ...
  for (isz = Component[cmp].size.begin (); isz != Component[cmp].size.end (); isz++, iretvec++)
    {
      *iretvec = (coeff[0] / (*isz))
                 * exp ((-1) * (pow (log ((*isz) / (coeff[2] * Constant::UM_CM)), 2) / _sig));
      // cout << *isz << " " << *iretvec << endl;
      if (coeff[1] != 0)
        *iretvec += (coeff[1] / (*isz))
                    * exp ((-1) * (pow (log ((*isz) / (coeff[3] * Constant::UM_CM)), 2) / _sig));
      // cout << *isz << " " << *iretvec << endl;
      if (coeff[5] != 0)
        {
