...
void
GrainModel::MakeGrainModel (ConfigFile &_mbkcf, vector<float> &MasterWave)
{
    string _thisSection;
    string _line;
    ...
    float delta, _CAbsIntegral, _CScaIntegral,
        _phFuncIntegral; //,_MassConstants;
    ...
    for (int cmp = 0; cmp < nComp; cmp++)
        { // Loop over all components
            ...
            // constants definition file DEF   -> 0  ; size grid will be taken from
            // the optical constants definition file FILE  -> 1  ; size grid will be
            // read from an on disk tabulation NSIZE -> 2  ; size grid will be
            // constructed from amin,amax,nsize for each component
                ...
                case 1:
                    {
                        ...
                        //     If the file lives in /home/dirty, the config file should
                        //     have "Size Definition File=/home/dirty/MySizeGrid.dat"
                        ...
                        if (_thisSizeFile == _mcf.BadString ())
                            {
                                cout << "Unable to retrieve size grid file name, resorting to "
                                        "default."
                                     << endl;
                                _MasterSize.push_back (-1);
                            }
                ...
                case 2:
                    {
                        ...
                        if (_thisSizeIN <= 0)
                            {
                                cout << "Unable to define size grid manually.  Resorting to "
                                        "default."
                                     << endl;
                                _thisSizeType = "DEF";
                                _MasterSize.push_back (-1);
                        ...
                        else
                            {
                                ...
                                if (_thisSizeScale == _mcf.BadString ())
                                    {
                                        cout << "Did not find size gridding definition - assuming "
                                                "linear"
                                             << endl;
            ...
            Component[cmp].MakeGrain (_CrossSectionsPath + _thisCrossSectionFile,
                                      _CalorimetryPath + _thisCalorimetryFile, MasterWave, _MasterSize, _TopLevelPath,
            ...
            if (SizeDistID.find (_thisSizeDistribution) != SizeDistID.end ())
                { // Determine which sd to use
                    ...
                    // GAUSS    = 3;    // Appox. single size grain distribution -
                    // Guassian HD23     = 4;    // Hendley & Draine 2023 NULL     = 99;
                        ...
                        case 3: // Gaussian
                            {
                                ...
                                // Shouldn't have to do this for every component, but I'm
                                // lazy...
                                DustToGasMassRatio = _mcf.FValue ("Model", "Dust to Gas Mass Ratio");
                                if (_mcf.isBadFloat (DustToGasMassRatio))
                                    {
                                        cout << "Dust to gas mass ratio not defined - required "
                                                "for this "
                                                "size "
                                                "distribution"
                                             << endl;
                                        cout << "Looked under section 'Model' for key 'Dust to "
                                                "Gas Mass "
                                                "Ratio'"
                                             << endl;
                                        exit (8);
                                ...
                                if (_mcf.isBadFloat (MeanMolecularWeight))
                                    {
                                        cout << "Mean molecular weight not defined - required for "
                                                "this size "
                                                "distribution"
                                             << endl;
                                        cout << "Looked under section 'Model' for key 'Mean "
                                                "Molecular "
                                                "Weight'"
                                             << endl;
                                        exit (8);
                                ...
                                // Convert all length units in gaussian parameters to CM - that
                                // way, the size distribution will be returned in cm^(-1)
                                // H^(-1)
                                ...
                                // Now renormalize to assure proper dust mass/tau/etc
                                // computations
                                ...
                                // size is in cm, integrand is in grams, _mmw is dimensionless,
                                // gdmris dimensionless, AMU_CGS is gm/H, the dimensions os
                                // _newAmp are cm^-1 H^-1 ... float _newAmp =
                                // (DustToGasMassRatio*MeanMolecularWeight*Constant::AMU_CGS)/NumUtils::integrate<float>(Component[cmp].size,_integrand);
                                // ... as are the dim. of SizeDistribution  since
                                // SizeDistribution was previously dimensionless = 1.0 *
                                // exp(-z^2/2).
                                Normalization[cmp]
                                    = (DustToGasMassRatio * MeanMolecularWeight * Constant::AMU_CGS * _massfrac)
                        ...
                        default:
                            {
                                ...
                                cout << "Key " << _thisSizeDistribution << " exists, but has no corresponding case."
                                     << endl;
            ...
            // Compute the 'volume' in this component; we could roll into the loop
            // above to save computation at the cost of additional RAM, but
            // computational cost at this stage of the process is not a large factor,
            // RAM may be.
            for (int sz = 0; sz < Component[cmp].nsize; sz++)
            ...
            transform (SizeDistribution[cmp].begin (), SizeDistribution[cmp].end (),
                       back_inserter (SizeDistributionNorm[cmp]), bind2nd (multiplies<float> (), Normalization[cmp]));

        } // End Component loop.

    // Total normalization; this is the denominator for C_abs,sca size
    // integrated, comp summed quantities.
        ...
        { // Loop over wavelength
            ...
            for (int cmp = 0; cmp < nComp; cmp++)
                { // Loop over Chemical composition
                    ...
                    // Integration loop.
                    for (ithisSize = Component[cmp].size.begin () + 1; ithisSize != Component[cmp].size.end ();
                         ithisSize++)
                        { // Integration loop.
                            ...
                            _CAbsIntegral
                                += delta
                                   * ((*ithisCAbs) * (*ithisSizeDist) + *(ithisCAbs - 1) * (*(ithisSizeDist - 1)));
                            _CScaIntegral
                                += delta
                                   * ((*ithisCSca) * (*ithisSizeDist) + *(ithisCSca - 1) * (*(ithisSizeDist - 1)));
                            // g = int(g(a)*Csca(a)*f(a)da
                            _phFuncIntegral += delta
                                               * ((*ithisphFunc) * (*ithisCSca) * (*ithisSizeDist)
                    ...
                    // Multiply by the appropriate normalization integral (eg. now we
                    // have
                    // []/Hatom
...
// sz)
vector<float>
GrainModel::getZDA_sdist (vector<float> coeff, int cmp)
{
            ...
            if (!isnan (coeff[3]))
                { // b1 is defined
                    *iretvec -= coeff[3] * pow (fabs (log10 (thissz / coeff[4])), coeff[5]);
                    ...
                    // " << thissz << " " << pow(fabs(log10(thissz/coeff[4])),coeff[5])
                    // << endl;
                }
            if (!isnan (coeff[6]))
                { // b2 is defined
                    *iretvec -= coeff[6] * pow (fabs (log10 (thissz / coeff[7])), coeff[8]);
                    ...
                    // " << thissz << " " << pow(fabs(log10(thissz/coeff[7])),coeff[8])
                    // << endl;
                }
            if (!isnan (coeff[9]))
                { // b3 is defined
                    *iretvec -= coeff[9] * pow (fabs ((thissz)-coeff[10]), coeff[11]);
                    // cout << "b3: " << cmp << " " << thissz << " " <<
                    // coeff[9]*pow(fabs( (thissz) - coeff[10]),coeff[11]) << endl;
                }
            if (!isnan (coeff[12]))
                { // b4 is defined
                    *iretvec -= coeff[12] * pow (fabs ((thissz)-coeff[13]), coeff[14]);
                    // cout << "b4: " << cmp << " " << thissz << " " <<
                    // coeff[12]*pow(fabs( (thissz) - coeff[13]),coeff[14]) << endl;
                }
            // cout << "LogG: " << cmp << " " << thissz << " " << *iretvec << endl;
            *iretvec = pow (10, (*iretvec)); // g(a) in um^-1
            // cout << "G:    " << cmp << " " << thissz << " " << *iretvec << endl;
            //  Zubko et al. functional definitions give g(a) in um^-1, convert to
            //  cm.
...
vector<float>
GrainModel::getWD_sdist (vector<float> coeff, int cmp)
{
    // float thissz;                     // Hold um copy of size at each point.
    vector<float> retvec (Component[cmp].size.size ()); // The size distribution [cm^-1]
    vector<float>::iterator isz;                        // Iterator through size vector
    ...
    // Wiengartner & Draine size distribution functions have mixed units -
    // convert where necessary to ensure out put in cm^-1 H^-1
    for (isz = Component[cmp].size.begin (); isz != Component[cmp].size.end (); isz++)
        {
            ...
            *iretvec = coeff[3] / (*isz) * pow ((*isz / (coeff[4] * static_cast<float> (Constant::UM_CM))), coeff[6]);
            ...
            if ((*isz) > (coeff[4] * Constant::UM_CM))
                {
                    *iretvec
                        *= exp (-1 * pow (((*isz) - coeff[4] * Constant::UM_CM) / (coeff[5] * Constant::UM_CM), 3));
                }
            if (coeff[0] > 0 && !isnan (coeff[0]))
                {
                    *iretvec += coeff[1] * coeff[0] / (*isz)
                                * exp (-0.5 * (pow ((log ((*isz) / (3.5 * Constant::ANG_CM)) / 0.4), 2)));
                    *iretvec += coeff[2] * coeff[0] / (*isz)
...
vector<float>
GrainModel::getHD_sdist (vector<float> coeff, int cmp)
{
    vector<float> retvec (Component[cmp].size.size ()); // The size distribution [cm^-1]
    vector<float>::iterator isz;                        // Iterator through size vector
    vector<float>::iterator iretvec;                    // Iterator through size dist vector
    ...
    for (isz = Component[cmp].size.begin (); isz != Component[cmp].size.end (); isz++, iretvec++)
        {
            *iretvec = (coeff[0] / (*isz)) * exp ((-1) * (pow (log ((*isz) / (coeff[2] * Constant::UM_CM)), 2) / _sig));
            ...
            if (coeff[1] != 0)
                *iretvec
                    += (coeff[1] / (*isz)) * exp ((-1) * (pow (log ((*isz) / (coeff[3] * Constant::UM_CM)), 2) / _sig));
            // cout << *isz << " " << *iretvec << endl;
            if (coeff[5] != 0)
                {
...
// {
//   vector <float> retvec(Component[cmp].size.size()); // The size
//   distribution [cm^-1] vector <float>::iterator isz;     // Iterator through
//   size vector vector <float>::iterator iretvec; // Iterator through size
//   dist vector
...
vector<float>
GrainModel::getGauss_sdist (vector<float> coeff, int cmp)
{
    vector<float> retvec (Component[cmp].size.size ()); // The size distribution [cm^-1]
    vector<float>::iterator isz;                        // Iterator through size vector
...
float
GrainModel::getTau (float a_wave)
{
