...
void
classify_scattered_photon (output_struct &output, photon_data &photon, geometry_struct &geometry,
            ...
            if ((i == 0) || (geometry.num_observers > 1))
                {
                    ...
                    // set the current grid number back to zero to ensure the full
                    // tracking can happen has to start in the base grid!
                    ...
                    tmp_photon.scat_weight
                        = scattered_weight_towards_observer (tmp_photon, geometry, output.outputs[i].observer_position);
#ifdef DEBUG_CSCP
                    if (photon.number == OUTNUM)
                        {
                    ...
                    if (geometry.internal_observer == 0)
                        {
                            ...
                            // save the scattered weight for use when different outputs are
                            // for emission/grain types
                    ...
                    else
                        {
                            ...
                            for (k = 0; k < 3; k++)
                                {
                                    obs_to_scat[k] = output.outputs[i].observer_position[k] - tmp_photon.position[k];
                    ...
                    if (photon.number == OUTNUM)
                        {
                            ...
                            cout << "scattered photon image indexs = (" << image_indxs[0] << "," << image_indxs[1]
                                 << ")" << endl;
                        }
#endif
                }
