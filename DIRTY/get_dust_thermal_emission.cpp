...
//                failure modes (not related to dust emission) need to be
//                added, it should be straitforward...
...
void
get_dust_thermal_emission (geometry_struct &geometry, runinfo_struct &runinfo,
            ...
            for (i = 0; i < geometry.grids[m].index_dim[0]; i++)
              {
                ...
                if (!geometry.emitted_energy_grid_initialized)
                  ...
                  // will need code that does not set the nonequilibrium
                  // thermal emission to zero when we are doing the next
                  // iteration for the case where we donot want to recompute
                  // the nonequilibrium case also make sure to set the total
                  // emission to the sum of the nonequilibrium emission
                  for (x = 0; x < runinfo.wavelength.size (); x++)
                    {
                      for (z = 0; z < n_emit_components; z++)
                ...
                // determine if there is any energy absorbed needing to be
                // remitted
                tot_abs_energy = 0.0;
                max_abs_energy = 0.0;
                tot_nonzero = 0;
                ...
                for (x = 0; x < runinfo.wavelength.size (); x++)
                  {
                    if (geometry.grids[m].grid (i, j, k).absorbed_energy_num_photons[x]
                        >= good_enough_photons)
                      {
                        tot_nonzero++;
                        tmp_abs_energy[x] = geometry.grids[m].grid (i, j, k).absorbed_energy[x] * 4.
                ...
                tot_abs_energy = NumUtils::integrate<double> (tmp_wave, tmp_abs_energy)
                ...
                if ((tot_abs_energy > 0.0)
                ...
                else if (tot_abs_energy >= min_enough_energy)
                  {
                    ...
                    // interoplate the radiative field to fill all the
                    // wavelength points if it has any nonzero points
                    if ((tot_nonzero != int (runinfo.wavelength.size ())) && (DoStochastic))
                      {
                        // 	    if (tot_nonzero !=
                        // int(runinfo.wavelength.size())) {
                        for (x = 0; x < runinfo.wavelength.size (); x++)
                          {
#ifdef DEBUG_GDTE
                            ...
                            if (geometry.grids[m].grid (i, j, k).absorbed_energy_num_photons[x]
                                >= good_enough_photons)
                              {
                                ...
                                tmp_j_for_interpol.push_back (
                                    double (geometry.grids[m].grid (i, j, k).absorbed_energy[x]));
                              }
                          }
#ifdef DEBUG_GDTE
                        cout << endl;
...
// int(0.75*runinfo.wavelength.size())) && (tot_abs_energy > 1e-35)) {
                    ...
                    for (x = 0; x < runinfo.wavelength.size (); x++)
                      {
                        ...
                        rad_unc
                            = (geometry.grids[m].grid (i, j, k).absorbed_energy_x2[x]
                               / geometry.grids[m].grid (i, j, k).absorbed_energy_num_photons[x])
                              - pow (double (geometry.grids[m].grid (i, j, k).absorbed_energy[x]
                                             / geometry.grids[m]
                                                   .grid (i, j, k)
                        ...
                        // 	      rad_unc *=
                        // geometry.grids[m].grid(i,j,k).absorbed_energy[x];
                        // cout << rad_unc << " "; the S/N reduces to the below
                        // equation (modulo a factor of (N-1)/N)
                    ...
                    cout << endl;
#endif
                  // get the dust emission spectrum given the input wavlength
                  // vector and radiation field vector emitted energy returned
                  // is in units of ergs s^-1 HI atom^-1
#ifdef DEBUG_GDTE
                    ...
                    else
                      {
                        ...
                        total_emit_energy
                            = NumUtils::integrate<double> (
                                  tmp_wave, geometry.grids[m].grid (i, j, k).emitted_energy[0])
                              * geometry.grids[m].grid (i, j, k).num_H;
                        ...
                        if (energy_diff > econs_tolerance)
                          {
                            ...
                            if (energy_diff > econs_tolerance_throwaway)
                              {
                                cout << "not using the energy emitted in this "
                                        "cell"
                                     << endl;
                                for (z = 0; z < n_emit_components; z++)
                                  for (x = 0; x < runinfo.wavelength.size (); x++)
                        ...
                        for (x = 0; x < runinfo.wavelength.size (); x++)
                          {
                            // set the emitted energy for the uniform emission
                            // case
                              ...
                              if (!finite (geometry.grids[m].grid (i, j, k).emitted_energy[z][x]))
                                {
                                  ...
                                  cout << geometry.grids[m].grid (i, j, k).emitted_energy.size ()
                                       << endl;
                                  cout << geometry.grids[m].grid (i, j, k).emitted_energy[z].size ()
                                       << endl;
                                  cout << z << " " << x << endl;
                                  cout << geometry.grids[m].grid (i, j, k).emitted_energy[z][x]
                                       << endl;
                                  cout << " emitted energy is not finite "
                                          "(before num_H calc) "
                                       << endl;
                                  exit (8);
                                }

                              // need to multiply the emitted energy passed
                              // back by dust_thermal_emission by the number of
                              // HI atoms in the cell to get the total emitted
                              // energy
                              geometry.grids[m].grid (i, j, k).emitted_energy[z][x]
                                  *= geometry.grids[m].grid (i, j, k).num_H;

                              // add up the emitted energy to the total emitted
                              // (per wavelength & component)
                              if (!finite (geometry.grids[m].grid (i, j, k).emitted_energy[z][x]))
                                {
                                  // Add failure status for this?
                                  cout << z << " " << x << endl;
