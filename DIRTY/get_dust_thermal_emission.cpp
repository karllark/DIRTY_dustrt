...
// 2008 Mar/KDG - updated calculation to do the units of the emitted energy
// correctly 2008 Sept 2 - added explicit cast to NumUtils::integrate 2009 Jun
// 3-4 - Modified handling of ComputeDustEmission returns to be failure
//                mode aware. Code will populate Failure instance with
//                information if dust heating code fails for any bin. If more
//                failure modes (not related to dust emission) need to be added,
...
void get_dust_thermal_emission(geometry_struct &geometry,
                               ...
                               GrainModel &CurGrainModel, DirtyFailure *Failure)

{
  ...
  // if (runinfo.do_emission_grain) n_emit_components +=
  // 2*CurGrainModel.getNComp();
  ...
  // calculate the total absorbed energy from the previously saved total
  // abosrbed energy by wavelength
  ...
  for (x = 0; x < runinfo.wavelength.size(); x++) {
    ...
    tmp_abs_energy[x] =
        runinfo.absorbed_energy[x] * 4. * Constant::PI * runinfo.ave_C_abs[x];
  }
  ...
  double min_enough_energy = min_energy_frac * runinfo.energy_conserve_target *
                             global_total_absorbed / geometry.num_cells;
        ...
        for (i = 0; i < geometry.grids[m].index_dim[0]; i++) {
          ...
          if (!geometry.emitted_energy_grid_initialized) {
            geometry.grids[m].grid(i, j, k).emitted_energy.resize(
                n_emit_components);
            for (z = 0; z < n_emit_components; z++)
              geometry.grids[m].grid(i, j, k).emitted_energy[z].resize(
                  runinfo.wavelength.size(), 0.0);

            geometry.grids[m].grid(i, j, k).emitted_energy_weighted.resize(
                runinfo.wavelength.size(), 0.0);
            geometry.grids[m].grid(i, j, k).emitted_energy_uniform.resize(
                runinfo.wavelength.size(), 0.0);
          } else
            // will need code that does not set the nonequilibrium thermal
            // emission to zero when we are doing the next iteration for the
            // case where we donot want to recompute the nonequilibrium case
            // also make sure to set the total emission to the sum of the
            // nonequilibrium emission
            for (x = 0; x < runinfo.wavelength.size(); x++) {
              for (z = 0; z < n_emit_components; z++)
          ...
          for (x = 0; x < runinfo.wavelength.size(); x++) {
            if (geometry.grids[m]
                    .grid(i, j, k)
                    .absorbed_energy_num_photons[x] >= good_enough_photons) {
              tot_nonzero++;
              tmp_abs_energy[x] =
                  geometry.grids[m].grid(i, j, k).absorbed_energy[x] * 4. *
                  Constant::PI * runinfo.ave_C_abs[x];
              if (tmp_abs_energy[x] > max_abs_energy)
                max_abs_energy = tmp_abs_energy[x];
            } else
              tmp_abs_energy[x] = 0.0;
          }
          tot_abs_energy =
              NumUtils::integrate<double>(tmp_wave, tmp_abs_energy) *
          ...
          if ((tot_abs_energy > 0.0) &&
            ...
            if ((tot_nonzero != int(runinfo.wavelength.size())) &&
              ...
              for (x = 0; x < runinfo.wavelength.size(); x++) {
                ...
                if (geometry.grids[m]
                        .grid(i, j, k)
                        .absorbed_energy_num_photons[x] >=
                    good_enough_photons) {
                  tmp_wave_for_interpol.push_back(
                      double(runinfo.wavelength[x]));
                  tmp_j_for_interpol.push_back(double(
...
#endif
              tmp_new_j_from_interpol = NumUtils::interpol(
                  tmp_j_for_interpol, tmp_wave_for_interpol, tmp_wave);
#ifdef DEBUG_GDTE
...
// 	    cout << "total min = " << tot_abs_energy << " " << min_enough_energy
// << endl; 	  if ((tot_abs_energy > 0.) && (tot_nonzero >
// int(0.75*runinfo.wavelength.size())) && (tot_abs_energy > 1e-35)) {
#ifdef DEBUG_GDTE
            // output the J
            float rad_unc = 0.0;
            for (x = 0; x < runinfo.wavelength.size(); x++) {
              cout << geometry.grids[m]
                          .grid(i, j, k)
                          .absorbed_energy_num_photons[x]
                   << " ";
              cout << geometry.grids[m].grid(i, j, k).absorbed_energy[x] << " ";
              cout << geometry.grids[m].grid(i, j, k).absorbed_energy_x2[x]
                   << " ";
              rad_unc =
                  (geometry.grids[m].grid(i, j, k).absorbed_energy_x2[x] /
                   geometry.grids[m]
                       .grid(i, j, k)
                       .absorbed_energy_num_photons[x]) -
                  pow(double(
            ...
            cout << endl;
#endif
          // get the dust emission spectrum given the input wavlength vector and
          // radiation field vector emitted energy returned is in units of ergs
          // s^-1 HI atom^-1
#ifdef DEBUG_GDTE
            cout << "entering ComputeDustEmission..." << endl;
#endif
            cur_plane_good++;
            status = ComputeDustEmission(
                geometry.grids[m].grid(i, j, k).absorbed_energy, CurGrainModel,
                geometry.grids[m].grid(i, j, k).emitted_energy, DoStochastic,
                runinfo.effective_grain_heating, _FailureSz, _FailureComp,
                _transitionSz);
#ifdef DEBUG_GDTE
            ...
            if (status != Flags::FSUCCESS) { //  Returned a failure from
              ...
              Failure->AddGrainInfo(CurGrainModel.getModelName(), _FailureSz,
                                    _FailureComp);
              Failure->AddEnergyInfo(
                  tot_abs_energy,
                  geometry.grids[m].grid(i, j, k).absorbed_energy);
            } else {
              total_emit_energy = 0.0;
              total_emit_energy =
                  NumUtils::integrate<double>(
                      tmp_wave,
                      geometry.grids[m].grid(i, j, k).emitted_energy[0]) *
                  geometry.grids[m].grid(i, j, k).num_H;

              float energy_diff =
                  fabs(1.0 - (total_emit_energy / tot_abs_energy));
              if (energy_diff > econs_tolerance) {
                // Add a failure status for this?  - maybe
                // right now - testing by not using this cell
                cout << "energy conservations worse than " << econs_tolerance
                     << endl;
                cout << "total_abs/H atom = " << tot_abs_energy << endl;
                cout << "total_emit_energy/H atom = " << total_emit_energy
                     << endl;
                cout << "ratio emit/abs = "
                     << total_emit_energy / tot_abs_energy << " ";
                ...
                for (x = 0; x < runinfo.wavelength.size(); x++) {
                  ...
                  if (!finite(geometry.grids[m]
                    ...
                    for (xx = 0; xx < runinfo.wavelength.size(); xx++) {
                      cout
                          << xx << " "
                          << geometry.grids[m].grid(i, j, k).absorbed_energy[xx]
                          << " ";
                      cout << geometry.grids[m]
                                  .grid(i, j, k)
                                  .absorbed_energy_num_photons[xx]
                           << " ";
                      cout << geometry.grids[m]
                                  .grid(i, j, k)
                                  .save_radiation_field_density[xx]
                           << " ";
                      cout << geometry.grids[m].grid(i, j, k).emitted_energy[0]
                                                                            [xx]
                           << " ";
                      cout << geometry.grids[m].grid(i, j, k).emitted_energy[1]
                                                                            [xx]
                           << " ";
                      cout << geometry.grids[m].grid(i, j, k).emitted_energy[2]
                                                                            [xx]
                           << " ";
                      cout << geometry.grids[m].grid(i, j, k).emitted_energy[3]
                                                                            [xx]
                           << " ";
                      cout << endl;
                    }
                    cout
                        << geometry.grids[m].grid(i, j, k).emitted_energy.size()
                        << endl;
                    cout << geometry.grids[m]
                                .grid(i, j, k)
                                .emitted_energy[z]
                         ...
                         << endl;
                    cout << z << " " << x << endl;
                    cout << geometry.grids[m].grid(i, j, k).emitted_energy[z][x]
                         << endl;
                    cout << " emitted energy is not finite (before num_H calc) "
                         << endl;
                    exit(8);
                  }

                  // need to multiply the emitted energy passed back by
                  // dust_thermal_emission by the number of HI atoms in the cell
                  // to get the total emitted energy
                  geometry.grids[m].grid(i, j, k).emitted_energy[z][x] *=
                      geometry.grids[m].grid(i, j, k).num_H;

                  // add up the emitted energy to the total emitted (per
                  ...
                  if (!finite(geometry.grids[m]
                    ...
                    cout << geometry.grids[m].grid(i, j, k).emitted_energy[z][x]
                         << endl;
                    cout << " emitted energy is not finite (after num_H calc) "
                         << endl;
                    exit(8);
                  }
                  runinfo.emitted_lum[z][x] +=
                      geometry.grids[m].grid(i, j, k).emitted_energy[z][x];
                }
            }
          } else {
  ...
  if (fabs(1.0 - (global_total_emitted / global_total_absorbed)) >
    ...
    cout << "or" << endl;
    cout << "(more likely) need to add more photons to get better radiation "
            "fields."
         << endl;
    ...
    cout << "ratio emit/abs = " << global_total_emitted / global_total_absorbed
         << endl;

    cout << "num cells = " << geometry.num_cells << endl;
    cout << "num cells w/ enough energy = " << num_cells_enough << endl;
    cout << "num cells w/ not enough energy (includes zero) = "
         << num_cells_not_enough << endl;
    cout << "num cells w/ zero energy = " << num_cells_zero << endl;
    cout << "num cells w/ too few wavelengths = " << num_cells_too_few_waves
         << endl;
