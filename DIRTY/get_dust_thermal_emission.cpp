...
void
get_dust_thermal_emission (geometry_struct &geometry, runinfo_struct &runinfo, GrainModel &CurGrainModel,
...

{
    ...
    double min_enough_energy
        = min_energy_frac * runinfo.energy_conserve_target * global_total_absorbed / geometry.num_cells;
                                ...
                                for (x = 0; x < runinfo.wavelength.size (); x++)
                                    {
                                        if (geometry.grids[m].grid (i, j, k).absorbed_energy_num_photons[x]
                                            ...
                                            {
                                                tot_nonzero++;
                                                tmp_abs_energy[x] = geometry.grids[m].grid (i, j, k).absorbed_energy[x]
                                                                    * 4. * Constant::PI * runinfo.ave_C_abs[x];
                                        ...
                                        if (status != Flags::FSUCCESS)
                                            { //  Returned a failure from
                                                ...
                                                Failure->AddEnergyInfo (
                                                    tot_abs_energy, geometry.grids[m].grid (i, j, k).absorbed_energy);
                                            }
                                        else
                                            {
                                                ...
                                                if (energy_diff > econs_tolerance)
                                                    {
                                                        ...
                                                        cout << "total_emit_energy/H atom = " << total_emit_energy
                                                             << endl;
                                                        cout
                                                            << "ratio emit/abs = " << total_emit_energy / tot_abs_energy
                                                            << " ";
                                                    ...
                                                    for (x = 0; x < runinfo.wavelength.size (); x++)
                                                        {
                                                            ...
                                                            runinfo.emitted_lum[z][x] += geometry.grids[m]
                                                                                             .grid (i, j, k)
                                                                                             .emitted_energy[z][x];
                                                        }
                                            }
