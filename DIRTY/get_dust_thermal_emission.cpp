...
void get_dust_thermal_emission(geometry_struct &geometry, runinfo_struct &runinfo, GrainModel &CurGrainModel,
...
// int(0.75*runinfo.wavelength.size())) && (tot_abs_energy > 1e-35)) {
                        ...
                        for (x = 0; x < runinfo.wavelength.size(); x++)
                        {
                            ...
                            if (rad_unc > 0.0)
                                rad_unc =
                                    sqrt(rad_unc); /// geometry.grids[m].grid(i,j,k).absorbed_energy_num_photons[x];
                            else
                                rad_unc = 0.0;
                            cout << rad_unc << " ";
                            // 	      rad_unc *=
                    ...
                    // s^-1 HI atom^-1
                        ...
                        else
                        {
                            ...
                            total_emit_energy = NumUtils::integrate<double>(
