            ...
            for (i = 0; i < geometry.grids[m].index_dim[0]; i++)
              {
                ...
                for (x = 0; x < runinfo.wavelength.size (); x++)
                  {
                    if (geometry.grids[m].grid (i, j, k).absorbed_energy_num_photons[x] >= good_enough_photons)
                      {
                        tot_nonzero++;
                        tmp_abs_energy[x] = geometry.grids[m].grid (i, j, k).absorbed_energy[x] * 4. * Constant::PI
                                            * runinfo.ave_C_abs[x];
                ...
                tot_abs_energy
                    = NumUtils::integrate<double> (tmp_wave, tmp_abs_energy) * geometry.grids[m].grid (i, j, k).num_H;
...
#endif
                        tmp_new_j_from_interpol
                            = NumUtils::interpol (tmp_j_for_interpol, tmp_wave_for_interpol, tmp_wave);
#ifdef DEBUG_GDTE
                    ...
                    for (x = 0; x < runinfo.wavelength.size (); x++)
                      {
                        ...
                        rad_unc = (geometry.grids[m].grid (i, j, k).absorbed_energy_x2[x]
                  ...
                  // is in units of ergs s^-1 HI atom^-1
...
#endif
                    cur_plane_good++;
                    status = ComputeDustEmission (geometry.grids[m].grid (i, j, k).absorbed_energy, CurGrainModel,
                    ...
                    if (status != Flags::FSUCCESS)
                    ...
                    else
                      {
                        ...
                        total_emit_energy
                            = NumUtils::integrate<double> (tmp_wave, geometry.grids[m].grid (i, j, k).emitted_energy[0])
