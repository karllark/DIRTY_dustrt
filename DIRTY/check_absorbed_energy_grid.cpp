...
void check_absorbed_energy_grid(geometry_struct &geometry, runinfo_struct &runinfo)
            ...
            for (k = 0; k < geometry.grids[m].index_dim[2]; k++)
                for (j = 0; j < geometry.grids[m].index_dim[1]; j++)
                    for (i = 0; i < geometry.grids[m].index_dim[0]; i++)
                    {
                        ...
                        {
                            abs_energy[x] += geometry.grids[m].grid(i, j, k).absorbed_energy[x] * Constant::FPI *
                                             runinfo.ave_C_abs[x] * geometry.grids[m].grid(i, j, k).num_H;
