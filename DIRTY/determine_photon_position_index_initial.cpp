...
void determine_photon_position_index_initial(geometry_struct &geometry, photon_data &photon)

{
    ...
    while (not done)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            //       // debug stuff
            //       if (photon.position_index[k][i] >=
            //       geometry.grids[cur_grid_num].index_dim[i]) {
            // 	cout << tmp_save_pindex << endl;
            // 	cout << photon.position[i] << endl;
            // 	cout << "photon.position[i] = " << photon.position[i] << endl;
            // 	cout << "photon.birth_position[i] = " <<
            // photon.birth_position[i] << endl; 	cout << "photon.dir_consines[i] = "
            // << photon.dir_cosines[i] << endl; 	cout << "photon.num_scat = " <<
            // photon.num_scat << endl; 	cout << "photon.position_index[k][i] = " <<
            // photon.position_index[k][i] << endl; 	cout << "real value of pindex = "
            // << (photon.position[i] - geometry.grids[cur_grid_num].positions[i][0])/
            // 	  geometry.grids[cur_grid_num].phys_cube_size[i] << endl;
            // 	cout << "geometry.grids[cur_grid_num].positions[i][0] = " <<
            // geometry.grids[cur_grid_num].positions[i][0] << endl; 	cout <<
            // "geometry.grids[cur_grid_num].phys_cube_size[i] = " <<
            // geometry.grids[cur_grid_num].phys_cube_size[i] << endl; 	cout << "found
            // the bastard ! " << endl; 	exit(8);
            //       }
            ...
            if (photon.number == OUTNUM)
            {
                ...
                // 	cout << "photon not in designated grid cell
                // (determine_photon.position_index_initial.cpp)." << endl; 	cout << "out
                // of bounds of correction." << endl;
                ...
                // geometry.grids[cur_grid_num].positions[i][0])/
                //   geometry.grids[cur_grid_num].phys_cube_size[i] << endl;
                cout << "cell min position = " << geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i]]
                     << endl;
                cout << "cell max position = "
                     << geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i] + 1] << endl;
                cout << "geometry.grids[cur_grid_num].positions[i][0] = "
                     << geometry.grids[cur_grid_num].positions[i][0] << endl;
                // cout << "geometry.grids[cur_grid_num].phys_cube_size[i] = " <<
                // geometry.grids[cur_grid_num].phys_cube_size[i] << endl;
                cout << "min edge diff = "
            ...
            if ((photon.position[i] < geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i]]) ||
                (photon.position[i] > geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i] + 1]))
            {
                // now make changes if this is a roundoff error issue (very near the
                // grid min/max)
                int not_ok = 0;
                ...
                else if (photon.position[i] < geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i]])
                {
                    ...
                    if (frac_miss < ROUNDOFF_ERR_INDEX)
                        ...
                        not_ok = 1;
                         ...
                         geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i] + 1])
                {
                    float frac_miss = (photon.position[i] -
                                       geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i] + 1]) /
                                      (geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i] + 1] -
                                       geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i]]);
                    cout << "frac_miss++(initial) = " << frac_miss << endl;
                ...
                {
                    cout << "photon not in designated grid cell "
                            "(determine_photon.position_index_initial.cpp)."
                         << endl;
                    ...
                    // geometry.grids[cur_grid_num].positions[i][0])/
                    //   geometry.grids[cur_grid_num].phys_cube_size[i] << endl;
                    cout << "cell min position = "
                         << geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i]] << endl;
                    cout << "cell max position = "
                         << geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i] + 1] << endl;
                    cout << "geometry.grids[cur_grid_num].positions[i][0] = "
                         << geometry.grids[cur_grid_num].positions[i][0] << endl;
                    ...
                    // "min edge diff/cube size = " <<
                    // (geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i]]
                    // - photon.position[i])/
                    //      geometry.grids[cur_grid_num].phys_cube_size[k] << endl;
                    // cout << "max edge diff/cub size = " << (photon.position[i] -
                    // geometry.grids[cur_grid_num].positions[i][photon.position_index[k][i]+1])/
                    //      geometry.grids[cur_grid_num].phys_cube_size[k] << endl;
                    cout << "ROUNDOFF_ERR_INDEX = " << ROUNDOFF_ERR_INDEX << endl;
                    exit(8);
