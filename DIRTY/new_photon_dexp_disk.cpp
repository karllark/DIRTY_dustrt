...
void
new_photon_dexp_disk (photon_data &photon, geometry_struct &geometry, random_dirty &random_obj)

{
  ...
  while ((fabs (geometry.stellar_emit_xy_vals[test_index] - ran_num) > 1e-8)
  ...
  // now interpolate between the two points to get an even better distrbuted
  // rho
  double frac
      = (ran_num - geometry.stellar_emit_xy_vals[min_index])
        / (geometry.stellar_emit_xy_vals[max_index] - geometry.stellar_emit_xy_vals[min_index]);
  //   cout << frac << endl;

  rho_xy
      = ((double (min_index) + frac) / double (geometry.stellar_emit_n_xy - 1)) * geometry.radius;
  //   cout << rho_xy << endl;
  //   exit(0);
  ...
  //   double right_side = log(1.0 -
  //   ran_num/geometry.stellar_emit_constant_xy); double left_side =
  //   -1.0*test_xy/geometry.stellar_scalelength +
