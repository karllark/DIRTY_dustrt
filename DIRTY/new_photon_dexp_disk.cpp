...
void new_photon_dexp_disk(photon_data &photon, geometry_struct &geometry,
...

{
  ...
  double frac = (ran_num - geometry.stellar_emit_xy_vals[min_index]) /
                (geometry.stellar_emit_xy_vals[max_index] -
                 geometry.stellar_emit_xy_vals[min_index]);
  //   cout << frac << endl;

  rho_xy =
      ((double(min_index) + frac) / double(geometry.stellar_emit_n_xy - 1)) *
      geometry.radius;
  //   cout << rho_xy << endl;
  //   exit(0);
