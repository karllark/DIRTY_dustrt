// History, if very sparse....
//
// 03/31/2025 - Bug fix for case with StochasticHeating requested but the
// smallest grain in the distribution
//              for a given component is in equilibrium for the specfified
//              radiation field. Bad indexing into the failure size, e.g. no
//              problem with any calculation, just specifying the size at which
//              we transition from stochastic to equilibrium when no such
...
extern int StochasticHeating (vector<float> &wave, vector<float> &J, vector<float> &cabs,
                              vector<float> &Temp, vector<float> &Enthalpy, float EAbs, float &TMin,
                              float &TMax, float &TEq, uint _sz,
                              vector<vector<double> > &StochasticLum);

int
ComputeDustEmission (vector<float> &J, GrainModel &GrainModel,
                     vector<vector<double> > &EmmittedEnergy, bool &DoStochastic,
                     bool &UseEffective_Grain, float &_FailureSz, int &_FailureComp,
    ...
    { // Component loop
      ...
      for (uint _sz = 0; _sz < _nsize; ++_sz, ++_it)
        { // Size loop
          ...
          // Get equilibrium temperature.  Return emitted energy as well since it's
          // used in stochastic calculation.
          status = EqTemp (_w, J, _cabs, EAbs, *_it, ((_tlo < 0) ? 1.0 : _tlo),
                           ((_thi > 2500.0) ? 2500.0 : _thi));
          ...
          if (_dostochastic[_sz])
            {
              ...
              // Since Enthalpy and CalTemp will change as we generate transition
              // matrix, regenerate at each size/cmp pair.
              ...
              if (tau_abs < tau_rad)
                { // Turn off stochastic heating for this and all
                  // subsequent sizes.
                  transform (_dostochastic.begin () + _sz, _dostochastic.end (),
                             _dostochastic.begin () + _sz, bind1st (multiplies<bool> (), false));
                  _sz == 0 ? _transitionSz[_cmp] = _size[_sz]
                           : _transitionSz[_cmp] = _size[_sz - 1];
                }
              else
                { // Compute StochasticLum, zero out Eq.
                  status = StochasticHeating (_w, _cJprod, _cabs, _CalTemp, _Enthalpy, EAbs, TMin,
                                              TMax, *_it, _sz, StochasticLum);
                  if (status != Flags::FSUCCESS)
                    return status; // There's been a stochastic heating failure.
                  EquilibriumLum[_sz].resize (_nw, 0.0);
                }
            }
          else
            StochasticLum[_sz].resize (
                _nw, 0.0); // Zero stochastic so we have zero's when we don't do it.
        }
