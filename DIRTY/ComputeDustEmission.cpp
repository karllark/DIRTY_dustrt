...
extern int StochasticHeating(vector<float> &wave, vector<float> &J,
                             vector<float> &cabs, vector<float> &Temp,
...
int ComputeDustEmission(vector<float> &J, GrainModel &GrainModel,
  ...
  // Local J that includes the microwave background - Otherwise the really small
  // guys want to cool to ~0K.
  // vector <float> _localJ=NumUtils::add_bbodyCGS<float>(_w,2.7,J);
  // for (int i=0;i<_nw;++i) cout << i << " " << _w[i] <<" " << J[i] << " " <<
  // _localJ[i] << endl;
  // Fit power law to J.
  ...
  for (int _cmp = 0; _cmp < _ncmp; ++_cmp) { // Component loop
    ...
    // vector.
    //_dostochastic.resize(_nsize,DoStochastic);  // Populate local with global
    //default.
    ...
    for (uint _sz = 0; _sz < _nsize; ++_sz, ++_it) { // Size loop
      ...
      if (_dostochastic[_sz]) {
        ...
        if (tau_abs < tau_rad) { // Turn off stochastic heating for this and all
                                 // subsequent sizes.
          transform(_dostochastic.begin() + _sz, _dostochastic.end(),
                    _dostochastic.begin() + _sz,
                    bind1st(multiplies<bool>(), false));
          _transitionSz[_cmp] = _size[_sz - 1];
        } else { // Compute StochasticLum, zero out Eq.
          status =
              StochasticHeating(_w, _cJprod, _cabs, _CalTemp, _Enthalpy, EAbs,
                                TMin, TMax, *_it, _sz, StochasticLum);
          if (status != Flags::FSUCCESS)
    ...
    if (!UseEffective_Grain) {
      ...
      for (uint _wv = 0; _wv < _nw;
           ++_wv, ++_iteec, ++_itees, ++_itet) { // wavelength loop
        ...
        *_iteec =
            Constant::FPI * NumUtils::integrate<double>(_size, _integrand);
        *_itees =
            Constant::FPI * NumUtils::integrate<double>(_size, _integrand1);
        *_itet += (*_iteec + *_itees);
      }
      ...
      for (uint _wv = 0; _wv < _nw; ++_wv, ++_itet, ++_iteec, ++_itees) {
        *_iteec = GrainModel.getNormalization() * Constant::FPI *
                  EquilibriumLum[0][_wv];
        *_itees = 0.0;
        *_itet = (*_iteec + *_itees);
      }
    }
