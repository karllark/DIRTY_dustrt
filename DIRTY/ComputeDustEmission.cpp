...
int ComputeDustEmission(vector<float> &J, GrainModel &GrainModel, vector<vector<double>> &EmmittedEnergy,
        ...
        { // Size loop
            ...
            if (_dostochastic[_sz])
            {
                ...
                { // Turn off stochastic heating for this and all
                    ...
                    _sz == 0 ? _transitionSz[_cmp] = _size[_sz] : _transitionSz[_cmp] = _size[_sz - 1];
                }
                else
                ...
                StochasticLum[_sz].resize(_nw, 0.0); // Zero stochastic so we have zero's when we don't do it.
        }
