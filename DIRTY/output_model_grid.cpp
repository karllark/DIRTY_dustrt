...
void
output_model_grid (geometry_struct &geometry, output_struct &output, runinfo_struct &runinfo)
  ...
  for (m = 0; m < geometry.grids.size (); m++)
    {
      ...
      tmp_rad_field.FVSize (geometry.grids[m].index_dim[0], geometry.grids[m].index_dim[1],
                            geometry.grids[m].index_dim[2], n_waves);

      // create a 4d matrix to copy the grid info into for output
      NumUtils::FourVector<float> tmp_rad_field_unc;
      tmp_rad_field_unc.FVSize (geometry.grids[m].index_dim[0], geometry.grids[m].index_dim[1],
                                geometry.grids[m].index_dim[2], n_waves);

      // create a 4d matrix to copy the grid info into for output
      NumUtils::FourVector<int> tmp_rad_field_npts;
      tmp_rad_field_npts.FVSize (geometry.grids[m].index_dim[0], geometry.grids[m].index_dim[1],
                                 geometry.grids[m].index_dim[2], n_waves);

      // create a 3d matrix to copy the grid info into for output
      NumUtils::Cube<float> tmp_tau;
      tmp_tau.CSize (geometry.grids[m].index_dim[0], geometry.grids[m].index_dim[1],
                     geometry.grids[m].index_dim[2]);

      // create a 3d matrix to copy the grid info into for output
      NumUtils::Cube<double> tmp_num_H;
      tmp_num_H.CSize (geometry.grids[m].index_dim[0], geometry.grids[m].index_dim[1],
                       geometry.grids[m].index_dim[2]);
              ...
              for (n = 0; n < n_waves; n++)
                {
                  ...
                  // compute the uncertainty on the average contribution from
                  // an individual photon
                  if (geometry.grids[m].grid (i, j, k).absorbed_energy_num_photons[n] >= 1)
                    {
                      // using eq. 14 of Camps & Baes (2018), equivalent to
                      // eqns in Gordon et al. (2001) with fewer computations
                      // *except* that the number of photons to use is the
                      // total, not the number in the cell do not understand
                      // why this is the case, but empirical tests with many
                      // independent runs confirm this (KDG 13 May 2024)
                      if (geometry.grids[m].grid (i, j, k).absorbed_energy[n] > 0.0)
                        {
                          rad_unc
                              = geometry.grids[m].grid (i, j, k).absorbed_energy_x2[n]
                                / pow (double (geometry.grids[m].grid (i, j, k).absorbed_energy[n]),
                                       double (2.0));
                      ...
                      // convert from radiation field mean intensity J (needed
                      // for dust emission calculations) to radiation field
                      // density U
                      tmp_rad_field (i, j, k, n) *= Constant::U_J;
                      tmp_rad_field_unc (i, j, k, n) *= Constant::U_J;
                    }
                  else
                    // store the indexes of the subgrids for cells that are
                    // subdivided
      ...
      fits_write_img (out_ptr, TFLOAT, 1,
                      geometry.grids[m].index_dim[0] * geometry.grids[m].index_dim[1]
                          * geometry.grids[m].index_dim[2] * n_waves,
      ...
      fits_write_img (out_pos_ptr, TDOUBLE, 1, tmp_pos_index[0] * tmp_pos_index[1], &tmp_pos[0],
                      &status);
      ...
      if (m != 0)
        {
          fits_write_key (out_tau_ptr, TINT, "PAR_GRID", &geometry.grids[m].parent_grid_num,
      ...
      else
        {
          ...
          fits_write_key (out_tau_ptr, TFLOAT, "RAD_TAU", &geometry.tau, "radial optical depth",
                          &status);
