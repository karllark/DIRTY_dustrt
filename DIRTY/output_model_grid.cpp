...
void output_model_grid(geometry_struct &geometry, output_struct &output,
...

{
  ...
  for (m = 0; m < geometry.grids.size(); m++) {
    ...
    tmp_rad_field.FVSize(geometry.grids[m].index_dim[0],
                         geometry.grids[m].index_dim[1],
                         geometry.grids[m].index_dim[2], n_waves);

    // create a 4d matrix to copy the grid info into for output
    NumUtils::FourVector<float> tmp_rad_field_unc;
    tmp_rad_field_unc.FVSize(geometry.grids[m].index_dim[0],
                             geometry.grids[m].index_dim[1],
                             geometry.grids[m].index_dim[2], n_waves);

    // create a 4d matrix to copy the grid info into for output
    NumUtils::FourVector<int> tmp_rad_field_npts;
    tmp_rad_field_npts.FVSize(geometry.grids[m].index_dim[0],
                              geometry.grids[m].index_dim[1],
                              geometry.grids[m].index_dim[2], n_waves);

    // create a 3d matrix to copy the grid info into for output
    NumUtils::Cube<float> tmp_tau;
    tmp_tau.CSize(geometry.grids[m].index_dim[0],
                  geometry.grids[m].index_dim[1],
                  geometry.grids[m].index_dim[2]);

    // create a 3d matrix to copy the grid info into for output
    NumUtils::Cube<double> tmp_num_H;
    tmp_num_H.CSize(geometry.grids[m].index_dim[0],
                    geometry.grids[m].index_dim[1],
                    geometry.grids[m].index_dim[2]);
    ...
    for (i = 1; i < 3; i++)
      if (geometry.grids[m].index_dim[i] > max_index)
        max_index = geometry.grids[m].index_dim[i];
    ...
    for (k = 0; k < geometry.grids[m].index_dim[2]; k++)
      for (j = 0; j < geometry.grids[m].index_dim[1]; j++)
        for (i = 0; i < geometry.grids[m].index_dim[0]; i++) {
          ...
          for (n = 0; n < n_waves; n++) {
            tmp_rad_field(i, j, k, n) =
                geometry.grids[m].grid(i, j, k).absorbed_energy[n];
            tmp_rad_field_npts(i, j, k, n) =
                geometry.grids[m].grid(i, j, k).absorbed_energy_num_photons[n];
            // compute the uncertainty on the average contribution from an
            // individual photon
            if (geometry.grids[m]
                    .grid(i, j, k)
                    .absorbed_energy_num_photons[n] >= 1) {
              // using eq. 14 of Camps & Baes (2018), equivalent to eqns in
              // Gordon et al. (2001) with fewer computations *except* that the
              // number of photons to use is the total, not the number in the
              // cell do not understand why this is the case, but empirical
              // tests with many independent runs confirm this (KDG 13 May 2024)
              if (geometry.grids[m].grid(i, j, k).absorbed_energy[n] > 0.0) {
                rad_unc =
                    ...
                    pow(double(
                        ...
                        double(2.0));
                rad_unc =
                    sqrt(rad_unc - (1. / output.outputs[0].total_num_photons));
              } else
                rad_unc = 0.0;
              ...
              // convert from radiation field mean intensity J (needed for dust
              // emission calculations) to radiation field density U
    ...
    for (i = 0; i < 3; i++)
      four_vector_size[i] = geometry.grids[m].index_dim[i];
    ...
    fits_write_img(out_ptr, TFLOAT, 1,
                   geometry.grids[m].index_dim[0] *
                       geometry.grids[m].index_dim[1] *
                       geometry.grids[m].index_dim[2] * n_waves,
    ...
    check_fits_io(status,
                  "fits_create_image : output_model_grid (rad_field unc)");

    fits_write_img(out_unc_ptr, TFLOAT, 1,
                   geometry.grids[m].index_dim[0] *
                       geometry.grids[m].index_dim[1] *
                       geometry.grids[m].index_dim[2] * n_waves,
    ...
    check_fits_io(status,
                  "fits_create_image : output_model_grid (rad_field npts)");

    fits_write_img(out_npts_ptr, TINT, 1,
                   geometry.grids[m].index_dim[0] *
                       geometry.grids[m].index_dim[1] *
                       geometry.grids[m].index_dim[2] * n_waves,
    ...
    fits_write_img(out_tau_ptr, TFLOAT, 1,
                   geometry.grids[m].index_dim[0] *
                       geometry.grids[m].index_dim[1] *
                       geometry.grids[m].index_dim[2],
                   &tmp_tau[0], &status);
    ...
    fits_create_img(out_num_H_ptr, -64, 3, geometry.grids[m].index_dim,
                    &status);
    check_fits_io(status, "fits_create_image : output_model_grid (num_H)");

    fits_write_img(out_num_H_ptr, TDOUBLE, 1,
                   geometry.grids[m].index_dim[0] *
                       geometry.grids[m].index_dim[1] *
                       geometry.grids[m].index_dim[2],
                   &tmp_num_H[0], &status);
    ...
    //     fits_write_key(out_pos_ptr, TLONG, "XSIZE",
    //     &geometry.grids[m].index_dim[0], "index dimension of x vals",
    //     &status); fits_write_key(out_pos_ptr, TLONG, "YSIZE",
    //     &geometry.grids[m].index_dim[1], "index dimension of y vals",
    //     &status); fits_write_key(out_pos_ptr, TLONG, "ZSIZE",
    //     &geometry.grids[m].index_dim[2], "index dimension of z vals",
    //     &status); check_fits_io(status,"fits_write_key : output_model_grid
    //     (pos)");

    if (m != 0) {
      fits_write_key(out_tau_ptr, TINT, "PAR_GRID",
                     &geometry.grids[m].parent_grid_num,
                     "grid number of parent", &status);
    } else {
      ...
      // final stuff for primary header
      fits_write_comment(out_unc_ptr, "**---------------------------------**",
                         &status);
      fits_write_comment(out_unc_ptr, "Output of the DIRTY model", &status);
      fits_write_comment(out_unc_ptr, "Karl D. Gordon & Karl A. Misselt",
                         &status);
      ...
      fits_write_key(out_tau_ptr, TFLOAT, "RAD_TAU", &geometry.tau,
                     "radial optical depth", &status);
      fits_write_key(out_tau_ptr, TINT, "GRDDEPTH", &geometry.max_grid_depth,
                     "maximum depth of the grid", &status);

      // final stuff for primary header
      fits_write_comment(out_tau_ptr, "**---------------------------------**",
                         &status);
      fits_write_comment(out_tau_ptr, "Output of the DIRTY model", &status);
      fits_write_comment(out_tau_ptr, "Karl D. Gordon & Karl A. Misselt",
                         &status);
      ...
      // final stuff for primary header
      fits_write_comment(out_num_H_ptr, "**---------------------------------**",
                         &status);
      fits_write_comment(out_num_H_ptr, "Output of the DIRTY model", &status);
      fits_write_comment(out_num_H_ptr, "Karl D. Gordon & Karl A. Misselt",
                         &status);
      ...
      // final stuff for primary header
      fits_write_comment(out_pos_ptr, "**---------------------------------**",
                         &status);
      fits_write_comment(out_pos_ptr, "Output of the DIRTY model", &status);
      fits_write_comment(out_pos_ptr, "Karl D. Gordon & Karl A. Misselt",
                         &status);
  ...
  check_fits_io(status,
                "fits_create_image : output_model_grid (wavelength grid)");

  fits_write_img(out_wave_ptr, TFLOAT, 1, n_waves, &runinfo.wavelength[0],
                 &status);
