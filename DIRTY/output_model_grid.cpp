...
void output_model_grid(geometry_struct &geometry, output_struct &output, runinfo_struct &runinfo)

{
    ...
    for (m = 0; m < geometry.grids.size(); m++)
    {
                ...
                for (i = 0; i < geometry.grids[m].index_dim[0]; i++)
                {
                    ...
                    for (n = 0; n < n_waves; n++)
                    {
                        ...
                        if (geometry.grids[m].grid(i, j, k).absorbed_energy_num_photons[n] >= 1)
                        {
                            ...
                            {
                                rad_unc = geometry.grids[m].grid(i, j, k).absorbed_energy_x2[n] /
                                          pow(double(geometry.grids[m].grid(i, j, k).absorbed_energy[n]), double(2.0));
        ...
        fits_write_img(out_tau_ptr, TFLOAT, 1,
                       geometry.grids[m].index_dim[0] * geometry.grids[m].index_dim[1] * geometry.grids[m].index_dim[2],
                       &tmp_tau[0], &status);
        ...
        if (m != 0)
        {
            fits_write_key(out_tau_ptr, TINT, "PAR_GRID", &geometry.grids[m].parent_grid_num, "grid number of parent",
                           &status);
        }
        else
        {
            ...
            fits_write_key(out_tau_ptr, TINT, "GRDDEPTH", &geometry.max_grid_depth, "maximum depth of the grid",
                           &status);
