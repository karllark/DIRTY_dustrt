...
void deposit_energy(const photon_data &dummy_photon, geometry_struct &geometry,
...
void move_photon(photon_data &photon, const photon_data &dummy_photon,
                 geometry_struct &geometry, const double target_tau,
                 double &tau_traveled) {
  ...
  one_grid &current_grid =
      geometry.grids[photon.grid_number[photon.current_grid_num]];
  for (int j = 0; j < 3; j++) {
    // compare photon.position with dummy_photon.path_pos_index[j][0] (1st path
    // segment)
    bool b1 = photon.position[j] <
              current_grid.positions[j][dummy_photon.path_pos_index[j + 1][0]];
    bool b2 =
        photon.position[j] >
        current_grid.positions[j][dummy_photon.path_pos_index[j + 1][0] + 1];
    if (b1 || b2) {
      double correct_pos =
          current_grid.positions[j][dummy_photon.path_pos_index[j + 1][0] +
                                    ((b1) ? 0 : 1)];
      ...
      cout << "move_photon(): photon #" << photon.number << ":"
           << photon.num_scat << " not in orig cell, frac_miss = " << frac_miss
           << endl;
  ...
  for (int i = 0; i < dummy_photon.path_cur_cells; i++) {
    ...
    if (tau_left <= dummy_photon.path_tau[i]) {
      ...
      if (i != 0) {
        ...
        for (int j = 0; j < 3; j++) {
          ...
          if (dummy_photon.path_pos_index[j + 1][i] !=
            ...
            double new_pos;
            if (dir_cosine > 0.0) {
              // if the photon is moving forward, it hits the back of the grid
              // cell
              new_pos = geometry.grids[photon.grid_number[k]]
                            .positions[j][photon.position_index[k][j]];
            } else if (dir_cosine < 0.0) {
              // if the photon is moving backward, it hits the front of the grid
              // cell
              new_pos = geometry.grids[photon.grid_number[k]]
      ...
      /*
      // using calc_delta_dist(): slow - calc_delta_dist() costs 80% of the time
      of this function double delta_tau; int escape; photon.path_cur_cells = -1;
      ...
      */
      {
        ...
        double dust_tau_ref_per_pc =
            current_grid
                .grid(position_index[0], position_index[1], position_index[2])
                .dust_tau_per_pc;
        double dust_tau_per_pc = dust_tau_ref_per_pc * geometry.tau_to_tau_ref;

        for (int j = 0; j < 3; j++) {
          double distance_j =
              photon.dir_cosines[j] * tau_left / dust_tau_per_pc;
          photon.position[j] += distance_j;

          // ensure that the photon is still in the grid cell
          bool b1 =
              photon.position[j] < current_grid.positions[j][position_index[j]];
          bool b2 = photon.position[j] >
                    current_grid.positions[j][position_index[j] + 1];
          if (b1 || b2) {
            double correct_pos =
                current_grid.positions[j][position_index[j] + ((b1) ? 0 : 1)];
            double cube_size =
                current_grid
                    .positions[j][position_index[j] + ((b1) ? 0 : 1) + 1] -
                current_grid.positions[j][position_index[j] + ((b1) ? 0 : 1)];
            double frac_miss = (photon.position[j] - correct_pos) / cube_size;
            cout << "move_photon(): photon #" << photon.number << ":"
                 << photon.num_scat
