...
void deposit_energy(const photon_data &dummy_photon, geometry_struct &geometry, const double photon_weight)
{
    /*
...
void move_photon(photon_data &photon, const photon_data &dummy_photon, geometry_struct &geometry,
                 const double target_tau, double &tau_traveled)
{
                ...
                for (int j = 0; j < 3; j++)
                {
                    ...
                    if (dummy_photon.path_pos_index[j + 1][i] != dummy_photon.path_pos_index[j + 1][i - 1])
                    {
                        ...
                        else if (dir_cosine < 0.0)
                        {
                            ...
                            // cell
                            new_pos =
                                geometry.grids[photon.grid_number[k]].positions[j][photon.position_index[k][j] + 1];
                        }
                        else
                        {
            ...
            */
            {
                ...
                double dust_tau_ref_per_pc =
                    current_grid.grid(position_index[0], position_index[1], position_index[2]).dust_tau_per_pc;
