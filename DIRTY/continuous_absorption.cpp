...
void
deposit_energy (const photon_data &dummy_photon, geometry_struct &geometry,
...
void
move_photon (photon_data &photon, const photon_data &dummy_photon, geometry_struct &geometry,
  ...
  for (int j = 0; j < 3; j++)
    {
      ...
      // segment)
      bool b1
          = photon.position[j] < current_grid.positions[j][dummy_photon.path_pos_index[j + 1][0]];
      ...
      if (b1 || b2)
        {
          ...
          double cube_size
              = current_grid
                    .positions[j][dummy_photon.path_pos_index[j + 1][0] + ((b1) ? 0 : 1) + 1]
                - current_grid.positions[j][dummy_photon.path_pos_index[j + 1][0] + ((b1) ? 0 : 1)];
