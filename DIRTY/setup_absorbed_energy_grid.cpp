              ...
              else
                {
                  ...
                  if (geometry.abs_energy_storage_type == 1)
                  ...
                  else if (!geometry.abs_energy_grid_initialized)
                    {
                      ...
                      if (doing_emission == 1)
                        {
                          geometry.grids[m].grid (i, j, k).save_radiation_field_density.resize (
                              runinfo.n_waves, 0.0);
                          geometry.grids[m].grid (i, j, k).save_radiation_field_density_x2.resize (
                              runinfo.n_waves, 0.0);
                          geometry.grids[m]
                              .grid (i, j, k)
                              .save_radiation_field_density_num_photons.resize (runinfo.n_waves, 0);
                        }
                    }
                  else
                    {
