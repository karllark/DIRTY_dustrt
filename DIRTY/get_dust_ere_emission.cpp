                    ...
                    else
                      {
                        ...
                        while ((runinfo.wavelength[x] <= runinfo.ere_excitation_wavelength)
                               && (x < runinfo.wavelength.size ()))
                          {
                            ...
                            geometry.grids[m].grid (i, j, k).absorbed_energy[x]
                                -= tmp_j_for_interpol[x] * runinfo.ere_efficiency * Constant::PLANCKLIGHT
                        ...
                        for (x = 0; x < runinfo.wavelength.size (); x++)
                          {
                            ere_sigma_ratio = fabs (runinfo.wavelength[x] - runinfo.ere_peak_wavelength)
                                              / (runinfo.ere_fwhm / 2.35);
                            // 	      cout << ere_sigma_ratio << " ";
                            if (ere_sigma_ratio < 4.0)
                              {
                                geometry.grids[m].grid (i, j, k).emitted_energy[0][x]
                                    = (total_ere_photons / 1.42) * exp (-(0.5 * ere_sigma_ratio * ere_sigma_ratio));
                                //  		cout << runinfo.wavelength[x]
                                //  << " " <<
                                //  geometry.grids[m].grid(i,j,k).emitted_energy[0][x];
                              }
                        ...
                        total_ere_energy = (total_ere_photons * (runinfo.ere_fwhm / 2.35) * pow (M_PI, 0.5)
                                            * geometry.grids[m].grid (i, j, k).num_H);
