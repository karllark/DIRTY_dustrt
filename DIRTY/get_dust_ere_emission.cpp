...
void
get_dust_ere_emission (geometry_struct &geometry, runinfo_struct &runinfo)
                                ...
                                if (geometry.grids[m].grid (i, j, k).dust_tau_per_pc > 0.0)
                                    {
                                        ...
                                        if ((tot_abs_energy > 0.0)
                                        ...
                                        else
                                            {
                                                ...
                                                if (tot_nonzero != int (runinfo.wavelength.size ()))
                                                    {
                                                        for (x = 0; x < runinfo.wavelength.size (); x++)
                                                            {
                                                                if (geometry.grids[m].grid (i, j, k).absorbed_energy[x]
                                                                    > 0)
                                                                    {
                                                                        tmp_wave_for_interpol.push_back (
                                                                            runinfo.wavelength[x]);
                                                ...
                                                // " " << min_enough_energy << endl; 	  if
                                                // ((tot_abs_energy > 0.) && (tot_nonzero >
                                                // int(0.75*runinfo.wavelength.size())) &&
                                                ...
                                                //   photon conversion efficiency, max wavelength for
                                                //   excitation central wavelength, FWHM

                                                // first integrate below the max wavelength in photon
                                                // units
                                                ...
                                                while ((runinfo.wavelength[x] <= runinfo.ere_excitation_wavelength)
                                                       && (x < runinfo.wavelength.size ()))
                                                    {
                                                        ...
                                                        // remove the energy just absorbed from J (it is
                                                        // now in ERE) (only ere peak photon energy)
                                                        geometry.grids[m].grid (i, j, k).absorbed_energy[x]
                                                            -= tmp_j_for_interpol[x] * runinfo.ere_efficiency
                                                               * Constant::PLANCKLIGHT / runinfo.ere_peak_wavelength;
                                                        // convert from rad field density to the absorbed
                                                        // energy density
                                                        tmp_j_for_interpol[x]
                                                            *= 4. * Constant::PI * runinfo.ave_C_abs[x];
                                                        x++;
                                                    }
                                                total_ere_photons = NumUtils::integrate<double> (tmp_wave_for_interpol,
                                                                                                 tmp_j_for_interpol);
                                                ...
                                                //   need to check the conversion between fwhm and
                                                //   sigma - on a plane and no internet)
                                                // adjust to account for non-zero width of emission
                                                //   this is a factor that works for a specific set of
                                                //   values need to work out the correct correction
                                                //   factor (takes into account width) KDG - 9 Jan 2009
                                                        ...
                                                        if (ere_sigma_ratio < 4.0)
                                                            {
                                                                geometry.grids[m].grid (i, j, k).emitted_energy[0][x]
                                                                    = (total_ere_photons / 1.42)
                                                                      * exp (
                                                                          -(0.5 * ere_sigma_ratio * ere_sigma_ratio));
                                                                //  		cout << runinfo.wavelength[x]
                                                                //  << " " <<
                                                                //  geometry.grids[m].grid(i,j,k).emitted_energy[0][x];
                                                            }
                                                    ...
                                                    for (x = 0; x < runinfo.wavelength.size (); x++)
                                                        {
                                                            ...
                                                            if (!finite (geometry.grids[m]
                                                                             ...
                                                                             .emitted_energy[z][x]))
                                                                {
                                                                    ...
                                                                    cout << geometry.grids[m]
                                                                                ...
                                                                                .emitted_energy[z][x]
                                                                         << endl;
                                                                    cout << " emitted ere energy is not finite "
                                                                            "(before num_H "
                                                                            "calc) "
                                                                         << endl;
                                                                    exit (8);
                                                            ...
                                                            runinfo.emitted_ere_lum[z][x] += geometry.grids[m]
                                                                                                 .grid (i, j, k)
                                                                                                 .emitted_energy[z][x];
                                                        }
                                            }
