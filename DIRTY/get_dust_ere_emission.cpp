...
void get_dust_ere_emission(geometry_struct &geometry, runinfo_struct &runinfo)
        ...
        for (i = 0; i < geometry.grids[m].index_dim[0]; i++) {
          ...
          if (geometry.grids[m].grid(i, j, k).dust_tau_per_pc > 0.0) {
            ...
            if ((tot_abs_energy > 0.0) &&
              ...
              // points if it has any nonzero points
            ...
            // min_enough_energy << endl; 	  if ((tot_abs_energy > 0.) &&
            // (tot_nonzero > int(0.75*runinfo.wavelength.size())) &&
            // (tot_abs_energy > 1e-35)) {
#ifdef DEBUG_GDEE
              // output the J
              for (x = 0; x < runinfo.wavelength.size(); x++) {
              ...
              while ((runinfo.wavelength[x] <=
                ...
                // remove the energy just absorbed from J (it is now in ERE)
                // (only ere peak photon energy)
                geometry.grids[m].grid(i, j, k).absorbed_energy[x] -=
                    tmp_j_for_interpol[x] * runinfo.ere_efficiency *
                    Constant::PLANCKLIGHT / runinfo.ere_peak_wavelength;
                // convert from rad field density to the absorbed energy density
                tmp_j_for_interpol[x] *=
                    4. * Constant::PI * runinfo.ave_C_abs[x];
                x++;
              }
              total_ere_photons = NumUtils::integrate<double>(
                  tmp_wave_for_interpol, tmp_j_for_interpol);

              // convert to the number of photons to emit in ERE
              total_ere_photons *= runinfo.ere_efficiency;

#ifdef DEBUG_GDEE
              cout << "total_ere_photons/H atom = " << total_ere_photons
                   << endl;
#endif
              ...
              total_ere_photons /=
                  (sqrt(2.0) * (runinfo.ere_fwhm / 2.35) * pow(M_PI, 0.5));

              // now create the Gaussian ERE emission
              //   need to check the conversion between fwhm and sigma - on a
              //   plane and no internet)
              // adjust to account for non-zero width of emission
              //   this is a factor that works for a specific set of values
              //   need to work out the correct correction factor (takes into
              //   account width) KDG - 9 Jan 2009 this is the 1.42 factor
              for (x = 0; x < runinfo.wavelength.size(); x++) {
                ere_sigma_ratio =
                    fabs(runinfo.wavelength[x] - runinfo.ere_peak_wavelength) /
                    (runinfo.ere_fwhm / 2.35);
                // 	      cout << ere_sigma_ratio << " ";
                if (ere_sigma_ratio < 4.0) {
                  geometry.grids[m].grid(i, j, k).emitted_energy[0][x] =
                      (total_ere_photons / 1.42) *
                      exp(-(0.5 * ere_sigma_ratio * ere_sigma_ratio));
                  //  		cout << runinfo.wavelength[x] << " " <<
                  //  geometry.grids[m].grid(i,j,k).emitted_energy[0][x];
                } else
              ...
              total_ere_energy =
                  (total_ere_photons * (runinfo.ere_fwhm / 2.35) *
                   pow(M_PI, 0.5) * geometry.grids[m].grid(i, j, k).num_H);
              if (fabs(1.0 - (total_emit_energy / total_ere_energy) > 1e-1)) {
                ...
                cout << "total_emit_energy/H atom = " << total_emit_energy
                     << endl;
                cout << "ratio emit/abs = "
                     << total_emit_energy / tot_abs_energy << endl;
                cout << "ratio emit/ere = "
                     << total_emit_energy /
                            (total_ere_photons * (runinfo.ere_fwhm / 2.35) *
                             pow(M_PI, 0.5) *
                ...
                for (x = 0; x < runinfo.wavelength.size(); x++) {
                  ...
                  if (!finite(geometry.grids[m]
                    ...
                    cout << geometry.grids[m].grid(i, j, k).emitted_energy[z][x]
                         << endl;
                    cout << " emitted ere energy is not finite (before num_H "
                            "calc) "
                         << endl;
                    exit(8);
                  }
                  ...
                  if (!finite(geometry.grids[m]
                    ...
                    cout << geometry.grids[m].grid(i, j, k).emitted_energy[z][x]
                         << endl;
                    cout << " emitted ere energy is not finite (after num_H "
                            "calc) "
                         << endl;
                    exit(8);
                  }
                  runinfo.emitted_ere_lum[z][x] +=
                      geometry.grids[m].grid(i, j, k).emitted_energy[z][x];
                }
            }
          }
