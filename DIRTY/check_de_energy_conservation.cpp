...
void
check_de_energy_conservation (runinfo_struct &runinfo, int &iter_done)
    ...
    for (i = 0; i < int (runinfo.wavelength.size ()); i++)
        {
            ...
            // de direct
            de_direct.push_back (runinfo.out_sed_lum[out_sed_lum_offset][i] * runinfo.emitted_lum[0][i]
                                 / Constant::CM_UM);
            // de scattered
            de_scattered.push_back (runinfo.out_sed_lum[1 + out_sed_lum_offset][i] * runinfo.emitted_lum[0][i]
                                    / Constant::CM_UM);
        }
