...
void
compute_observer_trans_matrix (output_struct &output, geometry_struct &geometry, int i)

{
  // **note added 3 Jul 2006**
  //   this transformation is clearly not the standard rotation matrix in 3D
  //   while I can't remember fully, I believe that I derived this
  //   transformation matrix specifically to rotate so the observer position is
  //   along the z-axis if the observer is put at the standard theta,phi
  //   location (need to check this)
  float theta;
  float phi;
  ...
  if (geometry.num_observers > 1)
    ...
    { // handle the case when the number of outputs is set by something
...
#ifdef DEBUG_COTM
  cout << "observer (theta,phi) = (" << theta * 180. / M_PI << "," << phi * 180. / M_PI << ")"
       << endl;
#endif
