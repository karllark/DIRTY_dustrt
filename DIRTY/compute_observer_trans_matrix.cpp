...

{
...
#ifdef DEBUG_COTM
  cout << "observer (theta,phi) = (" << theta * 180. / M_PI << ","
       << phi * 180. / M_PI << ")" << endl;
#endif
  ...
  output.outputs[i].observer_position[0] =
      geometry.distance * sin(theta) * cos(phi);
  output.outputs[i].observer_position[1] =
      geometry.distance * sin(theta) * sin(phi);
