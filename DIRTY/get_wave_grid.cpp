...
void get_wave_grid(ConfigFile &param_data, runinfo_struct &runinfo) {
  ...
  if (grid_type == "res") {
    ...
    float delta_log_wave =
        (log10(wave_max) - log_wave_min) / (runinfo.n_waves - 1);
    float cur_wave = 0.0;
    int i = 0;
