...
void
scatter_photon (geometry_struct &geometry, photon_data &photon, random_dirty &random_obj)

{
  ...
  if (geometry.g == -2)
  ...
  else if ((geometry.g > ROUNDOFF_ERR_TRIG) && (geometry.scat_angle_bias_fraction > 0.0))
    {
      ...
      if (ran_num >= geometry.scat_angle_bias_fraction)
        { // sample from HG function
          cos_alpha = (1.0 + sqr_g) - pow ((1.0 - sqr_g) / (1.0 - geometry.g + 2.0 * geometry.g * ran_num2), 2);
          cos_alpha /= (2.0 * geometry.g);
        }
      else
        { // sample from isotropic function
  ...
  else if (geometry.g > ROUNDOFF_ERR_TRIG)
    {
      ...
      cos_alpha
          = (1.0 + sqr_g) - pow ((1.0 - sqr_g) / (1.0 - geometry.g + 2.0 * geometry.g * random_obj.random_num ()), 2);
      cos_alpha /= (2.0 * geometry.g);
    }
  else
  ...
  if ((1.0 - fabs (photon.dir_cosines[2])) > ROUNDOFF_ERR_TRIG)
    {
      ...
      new_dir_cosines[0]
          = sin_alpha * (cos_phi * photon.dir_cosines[2] * photon.dir_cosines[0] - sin_phi * photon.dir_cosines[1]);
      new_dir_cosines[0] = (new_dir_cosines[0] / tmp) + cos_alpha * photon.dir_cosines[0];
      new_dir_cosines[1]
          = sin_alpha * (cos_phi * photon.dir_cosines[2] * photon.dir_cosines[1] + sin_phi * photon.dir_cosines[0]);
  ...
  // is for the *next* scattering/interaction
  {
    ...
    calc_photon_trajectory (dummy_photon, geometry, target_tau, target_dist, escape, tau_to_surface, 0);
    ...
    for (int i = 0; i < dummy_photon.path_cur_cells; i++)
      {
        ...
        grid_cell &this_cell = geometry.grids[dummy_photon.path_pos_index[0][i]].grid (
            dummy_photon.path_pos_index[1][i], dummy_photon.path_pos_index[2][i], dummy_photon.path_pos_index[3][i]);
        ...
        if (this_cell.last_photon_number != photon.number)
        ...
        else
          {
            ...
            // separately and then have a special step at the end to add the
            // total photon's contribution
