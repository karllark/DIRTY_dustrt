    ...
    else if ((geometry.g > ROUNDOFF_ERR_TRIG) && (geometry.scat_angle_bias_fraction > 0.0))
        {
            ...
            if (ran_num >= geometry.scat_angle_bias_fraction)
                { // sample from HG function
                    cos_alpha
                        = (1.0 + sqr_g) - pow ((1.0 - sqr_g) / (1.0 - geometry.g + 2.0 * geometry.g * ran_num2), 2);
    ...
    if ((1.0 - fabs (photon.dir_cosines[2])) > ROUNDOFF_ERR_TRIG)
        {
            ...
            new_dir_cosines[0]
                = sin_alpha
                  * (cos_phi * photon.dir_cosines[2] * photon.dir_cosines[0] - sin_phi * photon.dir_cosines[1]);
            new_dir_cosines[0] = (new_dir_cosines[0] / tmp) + cos_alpha * photon.dir_cosines[0];
            new_dir_cosines[1]
                = sin_alpha
                  * (cos_phi * photon.dir_cosines[2] * photon.dir_cosines[1] + sin_phi * photon.dir_cosines[0]);
