...

{
  ...
  if (geometry.g == -2)
  ...
  else if (geometry.g > ROUNDOFF_ERR_TRIG)
    {
      ...
      cos_alpha
          = (1.0 + sqr_g)
            - pow ((1.0 - sqr_g) / (1.0 - geometry.g + 2.0 * geometry.g * random_obj.random_num ()),
                   2);
      cos_alpha /= (2.0 * geometry.g);
    }
  else
  ...
  if ((1.0 - fabs (photon.dir_cosines[2])) > ROUNDOFF_ERR_TRIG)
    {
      ...
      new_dir_cosines[0] = sin_alpha
                           * (cos_phi * photon.dir_cosines[2] * photon.dir_cosines[0]
                              - sin_phi * photon.dir_cosines[1]);
      new_dir_cosines[0] = (new_dir_cosines[0] / tmp) + cos_alpha * photon.dir_cosines[0];
      new_dir_cosines[1] = sin_alpha
                           * (cos_phi * photon.dir_cosines[2] * photon.dir_cosines[1]
