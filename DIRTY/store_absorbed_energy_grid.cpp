...
void store_absorbed_energy_grid(geometry_struct &geometry, runinfo_struct &runinfo, output_struct &output, int index,
                                int doing_emission)

{
    ...
    for (m = 0; m < int(geometry.grids.size()); m++)
    {
        ...
        for (k = 0; k < geometry.grids[m].index_dim[2]; k++)
        {
            ...
            {
                size_y =
                    (Constant::PC_CM) * (geometry.grids[m].positions[1][j + 1] - geometry.grids[m].positions[1][j]);
                for (i = 0; i < geometry.grids[m].index_dim[0]; i++)
                {
                    size_x =
                        (Constant::PC_CM) * (geometry.grids[m].positions[0][i + 1] - geometry.grids[m].positions[0][i]);
                    ...
                    {
                        // # of H atoms in cell
                        geometry.grids[m].grid(i, j, k).num_H = num_H_const * vol *
                        ...
                        (geometry.grids[m].grid(i, j, k).absorbed_energy[geometry.abs_energy_wave_index] > 0.0))
                    {
                        ...
                        cout << "Raw E_abs = "
                             << geometry.grids[m].grid(i, j, k).absorbed_energy[geometry.abs_energy_wave_index] << endl;
                        ...
                        if (doing_emission)
                            if (runinfo.dust_thermal_emission)
                                flux_mult_factor =
                                    (runinfo.emitted_lum[0][index] / output.outputs[0].total_num_photons);
                            else
                                flux_mult_factor =
                                    (runinfo.emitted_ere_lum[0][index] / output.outputs[0].total_num_photons);
                        else
                        ...
                        {
#ifdef DEBUG_SAEG
                            cout << "roundoff error warning in "
                                    "store_absorbed_energy_grid."
                                 << endl;
                        ...
                        geometry.grids[m].grid(i, j, k).absorbed_energy_x2[geometry.abs_energy_wave_index] =
                            float(j_temp_x2);
                        // reset the number of photon to last contribute to this
                        // cell
                    ...
                    if (doing_emission)
                    {
                        geometry.grids[m].grid(i, j, k).absorbed_energy[geometry.abs_energy_wave_index] +=
                            geometry.grids[m]
                                .grid(i, j, k)
                                .save_radiation_field_density[geometry.abs_energy_wave_index];
                        geometry.grids[m].grid(i, j, k).absorbed_energy_x2[geometry.abs_energy_wave_index] +=
                            geometry.grids[m]
                                .grid(i, j, k)
                                .save_radiation_field_density_x2[geometry.abs_energy_wave_index];
                        geometry.grids[m].grid(i, j, k).absorbed_energy_num_photons[geometry.abs_energy_wave_index] +=
                            geometry.grids[m]
                                .grid(i, j, k)
                                .save_radiation_field_density_num_photons[geometry.abs_energy_wave_index];
                    }
    ...
    {
        cout << "no code written for storage_absorbed_energy_grid for disk "
                "storage"
             << endl;
        exit(8);
    }
