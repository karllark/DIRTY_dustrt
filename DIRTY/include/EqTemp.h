// Jun 3-4 2009: Modified to be DirtyFailure aware, eg. Teq is return in
...
int EqTemp(vector<T2> &wave, vector<T2> &J, vector<T2> &Q, T1 &EAbs, T1 &_Tk,
           T3 Tlo = 1, T3 Thi = 2500) {
  // Local variables
  // T1 _Tk;                                  // Equilibrium temperature of
  // grain, returned value
  T1 _LHS, _RHS;          // Left and Right hand sides of equilibrium equation
  vector<T1> _integrand;  // Um, the integrands?
  T1 _Convergence =
      0.001;  // Fractional convergence in (RHS/LHS) - 0.1%
  ...
  if (J.size() != Q.size()) {
    cout << "Input energy density arrays and grain arrays do not match in size"
         << endl;
    cout << "IN EqTemp(), J.size()==" << J.size() << " Q.size()==" << Q.size()
         << endl;
  ...
  for (_iJ = _ib; _iJ != _ie; ++_iJ, ++_iQ, ++_it) *_it = (*_iJ) * (*_iQ);
  ...
  while (_TestConvergence > _Convergence) {
    ...
    if (Tlo == Thi) {
      ...
      // exit(8);
      if (Tlo == lolim) return Flags::FEQ_ZEROBOUND_LOLIM;
      if (Thi == hilim) return Flags::FEQ_ZEROBOUND_HILIM;
