...
struct runinfo_struct {
  ...
  string
      dust_grain_filename;  // name of the file with the dust grain information
  ...
  int do_stochastic_dust_emission;  // do stochastic_dust thermal emission
                                    // (0=no)
  ...
  int dust_thermal_emission;  // set when doing thermal dust emission (used when
                              // do_emission_grain=1)
  ...
  int rt_check_converged;    // switch to see if we should check for rt
                             // convergence
  float rt_converge_target;  // target for rt convergance (fractional for total
                             // scattered)
  ...
  bool effective_grain_heating;  // true if using effective grain in heating -
                                 // defaults false.
  ...
  double total_absorbed_energy;          // total absorbed energy (computed in
                                         // get_thermal_dust_emission.cpp)
  double total_absorbed_stellar_energy;  // total absorbed energy from the very
                                         // first RT, for use with
                                         // energy_conserve_target2
  ...
  long num_cells_too_few_waves;  // cells with too few wavelengths for dust
                                 // emission
  ...
  // output images
  int do_image_output;  // set if output of images at each wavelength are
                        // desired

  // output global values
  int do_global_output;    // set if global, multiwavelength output is desired
  int out_sed_lum_offset;  // offset for vector to all for ERE/Dust
                           // direct/scattered to be saved as well as stellar
                           // direct/scattered
  std::vector<std::vector<double>>
      out_sed_lum;  // allow for direct/scattered lum
  std::vector<std::vector<double>>
      out_sed_lum_unc;  // allow for direct/scattered lum uncertainties
  /*   std::vector< std::vector<float> > out_emitted_lum;  // allow to be by
   * grain/emission type */
