...
struct geometry_struct {
  ...
  int abs_energy_wave_index;  // wavelength index of absorbed energy grid
                              // 0 if disk is used, otherwise equal to
                              // wavelength index
  ...
  vector<double>
      phi_sum;  // input model scattering phase function (running sum)
  ...
  double star_positions
      [5][MAX_MULTIPLE_STARS];  // position of stars in physical units
                                // luminosity is the 4th element, 5th element
                                // the running sum of the luminosity between 0
                                // and 1 for the determination of which star
                                // emits the current photon
  ...
  vector<double>
      diffuse_source_intensity;  // assumed to be in ergs s^-1 sr^-1 A^-1 cm^-2
  vector<double> diffuse_source_sum_intensity;  // between 0 and 1

  string type;   // type of geometry [controls which routine is called to setup
                 // dust grid]
  ...
  float
      max_tau_per_cell;  // the maximum tau per cell (used in subdividing cells)
  float max_tau_per_cell_x;   // the maximum tau per cell (used in subdividing
                              // cells)
  float max_tau_per_cell_y;   // the maximum tau per cell (used in subdividing
                              // cells)
  float max_tau_per_cell_z;   // the maximum tau per cell (used in subdividing
                              // cells)
