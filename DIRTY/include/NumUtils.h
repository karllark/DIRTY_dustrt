...
template <typename T>
inline T
binrepl (T &val1, T &val2)
{
  return (val1 <= val2) ? Constant::PLANCKLIGHT : 0.0;
}
template <typename T>
inline T
line (T x1, T x2, T y1, T y2, T x)
{
  return ((y2 - y1) / (x2 - x1) * (x - x1) + y1);
}
template <typename T>
inline int
index (T val, vector<T> &vect)
{
...
template <typename T>
inline T
lextra (T &val1, T &val2, T &val3)
{
  return val1 * val2 / val3;
}
template <typename T>
inline T
cextra (T &val1, T &val2, T &val3)
{
  T ratio = val2 / val3;
...
template <typename T>
inline T
frootextra (T &val1, T &val2, T &val3)
{
  T ratio = val2 / val3;
...
template <typename T> vector<T> interpol (vector<T> &v, vector<T> &x, vector<T> &u, int LoEx = 2, int HiEx = 2);
...
template <typename T> vector<T> poly_fit (vector<T> x, vector<T> y, int ndegree, vector<T> &yfit, vector<T> &sigma);
...
template <typename T>
T
sigma_clip (vector<T> &vect, T &hi_clip, T &lo_clip)
{
...
template <typename T>
void
minmax_clip (vector<T> &vect, long nMin, long nMax)
{
...
template <typename T>
void
sortIndexOnVector (vector<T> &vect, vector<int> &index, int beg, int end)
{
  int i;
  T a;
...
T
integrate (vector<T1> &x, vector<T2> &y)
{
  T answer = 0.0;
...
template <typename T>
vector<T>
interpol (vector<T> &v, vector<T> &x, vector<T> &u, int LoEx, int HiEx)
{
  if (v.size () != x.size ())
    {
...
template <typename T>
void
interpolr (vector<T> &v, vector<T> &x, vector<T> &u, int LoEx, int HiEx)
{
  if (v.size () != x.size ())
    {
      cout << "Abscissa and ordinate lengths do not match in interpol" << endl;
...
template <typename T, typename T1, typename T2>
vector<T>
bbodyCGS (T1 wave, vector<T2> &Temperature)
{
...
template <class T, class T1, class T2, class T3>
vector<T>
prod_bbodyCGS (T1 wave, vector<T2> &Temperature, vector<T3> &product)
{
  // Wave is in CGS (cm), return in CGS: erg/cm^2/s/cm/st
  T c1 = 2.0 * Constant::PLANCK * pow (Constant::LIGHT, 2);
  T c2 = Constant::PLANCKLIGHT / (Constant::BOLTZMAN * static_cast<T> (wave));
...
template <class T, class T1, class T2, class T3>
vector<T>
add_bbodyCGS (T1 wave, vector<T2> &Temperature, vector<T3> &add)
{
  // Wave is in CGS (cm), return in CGS: erg/cm^2/s/cm/st
  T c1 = 2.0 * Constant::PLANCK * pow (Constant::LIGHT, 2);
  T c2 = Constant::PLANCKLIGHT / (Constant::BOLTZMAN * static_cast<T> (wave));
...
template <typename T, typename T1, typename T2>
vector<T>
bbodyCGS (vector<T1> &wave, T2 Temperature)
{
  // Wave is in CGS (cm), return in CGS: erg/cm^2/s/cm/st
  T c1 = 2.0 * Constant::PLANCK * pow (Constant::LIGHT, 2);
...
template <class T, class T1, class T2, class T3>
vector<T>
prod_bbodyCGS (vector<T1> &wave, T2 Temperature, vector<T3> &product)
{
  // Wave is in CGS (cm), return in CGS: erg/cm^2/s/cm/st
  T c1 = 2.0 * Constant::PLANCK * pow (Constant::LIGHT, 2);
  T c2 = Constant::PLANCKLIGHT / (Constant::BOLTZMAN * Temperature);
...
template <class T, class T1, class T2, class T3>
vector<T>
add_bbodyCGS (vector<T1> &wave, T2 Temperature, vector<T3> &add)
{
  // Wave is in CGS (cm), return in CGS: erg/cm^2/s/cm/st
  T c1 = 2.0 * Constant::PLANCK * pow (Constant::LIGHT, 2);
  T c2 = Constant::PLANCKLIGHT / (Constant::BOLTZMAN * Temperature);
...
template <typename T>
vector<T>
poly_fit (vector<T> x, vector<T> y, int ndegree, vector<T> &yfit, vector<T> &sigma)
{
  int m = ndegree + 1;
  int n = x.size ();
  // cout << m << " " << ndegree << endl;
  ...
  for (long p = 1; p <= 2 * ndegree; p++)
    {
      ...
      for (int j = ((0 > (p - ndegree)) ? 0 : (p - ndegree)); j <= ((ndegree < p) ? ndegree : p); j++)
        covar (j, p - j) = sum;
    }
  covar = mInvert (covar);
  for (int j = 0; j < m; j++)
...
template <typename T>
inline T &
Cube<T>::operator() (int n1_id, int n2_id, int n3_id)
{
  if (n1_id < 0 || n1_id >= _n1)
    {
...
template <typename T>
void
Cube<T>::CSize (int n1_id, int n2_id, int n3_id)
{
  _n1 = n1_id;
  _n2 = n2_id;
  _n3 = n3_id;
...
template <typename T>
inline void
Cube<T>::get_xyz (int cell_num, int &n1_id, int &n2_id, int &n3_id)
{
  div_t div_result;
...
FourVector<T>::operator() (int n1_id, int n2_id, int n3_id, int n4_id)
{
  if (n1_id < 0 || n1_id >= _n1)
    {
      cout << "out_of_range FourVector::operator(), element 1 " << n1_id << "," << n2_id << "," << n3_id << " " << n4_id
           << " " << _n1 << endl;
  ...
  if (n2_id < 0 || n2_id >= _n2)
    {
      cout << "out_of_range FourVector::operator(), element 2 " << n1_id << "," << n2_id << "," << n3_id << " " << n4_id
           << " " << _n2 << endl;
  ...
  if (n3_id < 0 || n3_id >= _n3)
    {
      cout << "out_of_range FourVector::operator(), element 3 " << n1_id << "," << n2_id << "," << n3_id << " " << n4_id
           << " " << _n3 << endl;
  ...
  if (n4_id < 0 || n4_id >= _n4)
    {
      cout << "out_of_range FourVector::operator(), element 4 " << n1_id << "," << n2_id << "," << n3_id << " " << n4_id
           << " " << _n4 << endl;
      std::string ExceptionObject = "out_of_range FourVector::operator(), element 3";
      throw std::out_of_range (ExceptionObject);
    }
...
template <typename T>
inline void
FourVector<T>::get_xyzz (int cell_num, int &n1_id, int &n2_id, int &n3_id, int &n4_id)
{
  div_t div_result;
  if (cell_num < 0 || cell_num > (int)this->size ())
    {
