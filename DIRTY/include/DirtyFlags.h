...
const std::string CDE_VECTOR_SIZE_MISMATCH
    = "ComputeDustEmission(): Vector size mismatch, J and wave.";
...
const std::string EQ_ZEROBOUND_LOLIM
    = "EqTemp(): Zero bound failure - upper and lower limits equal at lower bound.";
const std::string EQ_ZEROBOUND_HILIM
    = "EqTemp(): Zero bound failure - upper and lower limits equal at upper bound.";
...
const std::string ST_ZERO_PROBABILITY
    = "StochasticHeating(): Transition probability zero everywhere.";
const std::string ST_SMALL_PROBABILITY_HI = "StochasticHeating(): Transition probablity less than "
                                            "cutoff everywhere, setting upper bound.";
const std::string ST_SMALL_PROBABILITY_LO = "StochasticHeating(): Transition probablity less than "
                                            "cutoff everywhere, setting lower bound.";
...
const std::string ST_CTM_ZERO_ENTH_DEN
    = "StochasticHeating()->ComputeTransitionMatrix(): Enthalpy denominator is zero.";
const std::string ST_CTM_TRANSITIONID_EXCEEDS_WAVEID
    = "StochasticHeating()->ComputeTransitionMatrix(): Found index of transition wavelength that "
      "exceeds maximum wave size.";
const std::string ST_CTM_WAVE_ZERO
    = "StochasticHeating()->ComputeTransitionMatrix(): wave[id]-wave[id-1] == 0 WTF?";
// --> SolveTransitionMatrix
const int FST_STM_ZERO_COOLING = 14;
const std::string ST_STM_ZERO_COOLING = "StochasticHeating()->SolveTransitionMatrix(): Found a "
                                        "zero cooling element; T-Matrix unsolvable.";
...
const std::string GDTE_NONFINITE_EMITTED_ENERGY_PRE
    = "get_dust_thermal_emission(): Computed emitted energy is not finite, pre n_H.";
const std::string GDTE_NONFINITE_EMITTED_ENERGY_PST
    = "get_dust_thermal_emission(): Computed emitted energy is not finite, post n_H.";
