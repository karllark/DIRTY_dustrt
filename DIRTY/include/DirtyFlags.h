...
const std::string EQ_VECTOR_SIZE_MISMATCH =
...
const std::string ST_EXCEED_MAXBINS =
    "StochasticHeating(): Exceed maximum bin count.";
const std::string ST_ZERO_PROBABILITY =
    "StochasticHeating(): Transition probability zero everywhere.";
const std::string ST_SMALL_PROBABILITY_HI =
    "StochasticHeating(): Transition probablity less than cutoff everywhere, "
    "setting upper bound.";
const std::string ST_SMALL_PROBABILITY_LO =
    "StochasticHeating(): Transition probablity less than cutoff everywhere, "
    "setting lower bound.";
...
const std::string ST_CTM_ZERO_ENTH_DEN =
    "StochasticHeating()->ComputeTransitionMatrix(): Enthalpy denominator is "
    "zero.";
...
const std::string ST_CTM_WAVE_ZERO =
    "StochasticHeating()->ComputeTransitionMatrix(): wave[id]-wave[id-1] == 0 "
    "WTF?";
// --> SolveTransitionMatrix
const int FST_STM_ZERO_COOLING = 14;
const std::string ST_STM_ZERO_COOLING =
    "StochasticHeating()->SolveTransitionMatrix(): Found a zero cooling "
...
const std::string GDTE_ZERO_ABSORBED_ENERGY =
    "get_dust_thermal_emission(): No energy absorbed.";
