...
{ // Begin Grain Class definition
  ...
  public:
    ...
    void MakeGrain(string const &fOpticalConstants, string const &fCalorimetry, vector<float> &MasterWave,
                   vector<float> &MasterSize, string const &Path, float a_min = -1, float a_max = -1);

    // Return the bulk density of the grain material
    inline float getDensity(void)
    {
        return density;
    }

    // Return sublimation temperature of the grain
    inline float getSubTemp(void)
    {
        return SubT;
    }

    // Return number of grain sizes in object
    inline int getNSize(void)
    {
        return nsize;
    }

    // Return number of wavelengths in object
    inline int getNWave(void)
    {
        return nwave;
    }
    ...
    inline vector<float> getHeatCapacity(int _szid)
    {
        return HeatCapacity[_szid];
    }

    // Return the specific enthalpy of the grain material
    inline vector<float> getSpecificEnthalpy(void)
    {
        return SpecificEnthalpy;
    }
    inline vector<float> getEnthalpy(int _szid)
    {
