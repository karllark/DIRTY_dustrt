  ...
  if (!runinfo.do_dust_emission)
  ...
  else
    {
      if (runinfo.do_emission_grain && (geometry.num_observers > 1))
        {
          cout << "not possible to do dust emission output of emission by "
                  "grain "
                  "type and"
               << endl;
          cout << "multiple-lines-of-sight without new code." << endl;
          cout << "Set do_emission_type=0 to get multiple-lines-of-sight and "
                  "total "
                  "IR emission."
               << endl;
          exit (8);
  ...
  while (!iter_done)
    {
      ...
      radiative_transfer_many_waves (geometry, runinfo, de_output, photon, random_obj, DE_RT,
                                     iter_num);
