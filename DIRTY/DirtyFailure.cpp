...
DirtyFailure::DirtyFailure(string &_FailureLogName, int &_nWave) {
  ...
  FlagMap[Flags::FST_EXCEED_CONVERGENCE_LOOP_COUNT] =
      Flags::ST_EXCEED_CONVERGENCE_LOOP_COUNT;
  ...
  FlagMap[Flags::FST_CTM_TRANSITIONID_EXCEEDS_WAVEID] =
      Flags::ST_CTM_TRANSITIONID_EXCEEDS_WAVEID;
  ...
  FlagMap[Flags::FGDTE_POOR_ENERGY_CONSERVATION] =
      Flags::GDTE_POOR_ENERGY_CONSERVATION;
  FlagMap[Flags::FGDTE_NONFINITE_EMITTED_ENERGY_PRE] =
      Flags::GDTE_NONFINITE_EMITTED_ENERGY_PRE;
  FlagMap[Flags::FGDTE_NONFINITE_EMITTED_ENERGY_PST] =
      Flags::GDTE_NONFINITE_EMITTED_ENERGY_PST;
...
void DirtyFailure::WriteFailureLog(void) {
  ...
  if (failurelog) {
    ...
    for (long i = 0; i < nFailure; ++i) {
      ...
      failurelog << "Failed on cell " << iaxis[i] << "," << jaxis[i] << ","
                 << kaxis[i] << " of " << maxis[i] << endl;
      failurelog << "--> Failure info: " << endl;
      failurelog << "    Failure Flag - " << FailureFlag[i] << endl;
      failurelog << "    Failure Description - " << FailureDescription[i]
                 << endl;
      failurelog << "--> Grain info: " << endl;
      failurelog << "    Grain Model Name - " << GrainModelName[i] << endl;
      failurelog << "    Size - " << GrainSize[i] << " cm  of component "
                 << GrainComponent[i] << endl;
      failurelog << "--> Energy/Radiation Field info: " << endl;
      failurelog << "    Total energy absorbed in bin-  " << AbsorbedEnergy[i]
                 << endl;
