...
// Add an error
void
DirtyFailure::AddFailure (int _flag)
{
  nFailure++;
...
void
DirtyFailure::WriteFailureLog (void)
{
      ...
      for (long i = 0; i < nFailure; ++i)
        {
          ...
          failurelog << "Failed on cell " << iaxis[i] << "," << jaxis[i] << "," << kaxis[i]
                     << " of " << maxis[i] << endl;
          ...
          failurelog << "    Size - " << GrainSize[i] << " cm  of component " << GrainComponent[i]
                     << endl;
