...
void get_dust_parameters(ConfigFile &param_data, GrainModel &CurGrainModel,
...

{
  ...
  if (strcmp(dust_type.c_str(), "single_wavelength") == 0) {
    ...
    // check that if a single wavlength run is picked, then do_global_output is
    // not
    if (runinfo.do_global_output) {
      cout << "Cannot do_global_output when only for a single wavelength run"
           << endl;
      exit(8);
    }

    // check that if a single wavlength run is picked, then do_dust_emission is
    // not
    if (runinfo.do_dust_emission) {
      cout << "Cannot do_dust_emission when only for a single wavelength run"
           << endl;
      exit(8);
    }
    ...
    if (phi_file.fail()) {
      cout << "scattering phase function phi file (" << phi_filename
           << ") does not exist." << endl;
      exit(8);
    }
    phi_file.close();
    ...
    check_input_param("model scattering phase function (phi)", phi[0], 0.,
                      1e10);
    ...
    for (i = 1; i < angle.size(); i++) {
      ...
      check_input_param("model scattering phase function (phi)", phi[i], 0.,
                        1e10);
    ...
    if (empir_dust_file.fail()) {
      cout << "Empirical dust file (" << empir_dust_filename
           << ") does not exist." << endl;
      exit(8);
    }
    ...
    string grainpath =
        param_data.SValue("Model Book Keeping", "Path to Dust Properties");
    string grainsubdir =
        param_data.SValue("Model Book Keeping", "Model SubDir");
    runinfo.dust_grain_filename =
        grainpath + grainsubdir +
        param_data.SValue("Model Book Keeping", "Model Name");
    // get dust grain info
    CurGrainModel.MakeGrainModel(param_data, runinfo.wavelength);
    // now get the tau, albedo, and g values
    runinfo.effective_grain_heating =
        param_data.BValue("Model Book Keeping", "Effective Grain for Heating");
#ifdef DEBUG_GDP
    ...
    check_input_param(
        "wavelength to normalize tau",
        runinfo.norm_tau_wave * (Constant::UM_CM),
    ...
    float norm_tau =
        CurGrainModel.getTau(runinfo.norm_tau_wave * (Constant::UM_CM));
