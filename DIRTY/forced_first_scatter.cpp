...
int
forced_first_scatter (geometry_struct &geometry, photon_data &photon, random_dirty &random_obj)

{
  ...
  distance_traveled = calc_photon_trajectory (dummy_photon, geometry, target_tau, target_dist,
                                              ffs_escape, tau_to_surface, 0);
  ...
  if (tau_to_surface == 0.0)
  ...
  else
    {
      ...
      //    calculation good to very high taus (700+), zero afterwards (KDG 28
      //    Dec 2004)
      ...
      if (random_obj.random_num () >= geometry.scat_bias_fraction)
        { // classical forced scattering

          target_tau = -log (1.0 - random_obj.random_num () * (1.0 - new_stellar_weight));
        }
      else
        { // uniformaly sampled in optical depth
      ...
      biased_weight_factor = (1.0 - geometry.scat_bias_fraction)
                             + geometry.scat_bias_fraction * (1.0 - exp (-photon.first_tau))
                                   * exp (target_tau) / photon.first_tau;
      ...
      // *not done* this is done correctly in the classify_stellar_photon
      // routine KDG - 28 Feb 2007 photon.stellar_weight *= new_stellar_weight;
...
#endif
      distance_traveled = calc_photon_trajectory (photon, geometry, target_tau, target_dist, escape,
                                                  tau_traveled, 0);
...
#endif
      //     if (fabs((target_tau - tau_traveled)/target_tau) >
      //     ROUNDOFF_ERR_TRIG)
      //     {
      ...
      //       cout << "tau_traveled = " << tau_traveled << endl;
      //       cout << "diff = " << target_tau - tau_traveled << endl;
      //       cout << "diff/target_tau = " << (target_tau -
      //       tau_traveled)/target_tau << endl; cout << ROUNDOFF_ERR_TRIG <<
      //       endl; cout << "photon # = " << photon.number << endl; cout <<
      //       "tau_to_surface = " << photon.first_tau << endl; exit(8);
      //   } else {
      //     // This is a special case where the photon has traveled the
      //     correct distance
      //     // but has also just exited a subgrid.  This means that the photon
      //     index
      //     // for the subgrid is below or above the actual subgrid size for
      //     one dimension.
      //     // Easy to fix here by just redetermining the photon indexes [and
      //     does not
      ...
      // 	cout << "m = " << m << " ";
      // 	cout << "tmp_photon.position_index = " <<
      // photon.position_index[k][m] << endl;
      //       }
      //     }
...

      {
         ...
         *   total tau traveled when the photon packet enters/leaves the grid
         * cell prob_entering/prob_leaving: probability that the photon packet
         * enters/leaves the grid cell
         */
        const double abs_weight_init
            = (1. - geometry.albedo) * dummy_photon.stellar_weight; // not the reduced scat_weight
        ...
        for (int i = 0; i < dummy_photon.path_cur_cells; i++)
          {
            ...
            if (this_cell.last_photon_number != photon.number)
              ...
              {
                // compute the difference in x2 values previously added to what
                // should be added avoids having to save this information
                // separately and then have a special step at the end to add
                // the total photon's contribution
