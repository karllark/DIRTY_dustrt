...
// KDG 16 Jun 2008 - fixed max_grid_depth calculation & fixed assignment of
// grid number to parent KDG 14 Jul 2008 - fixed error of interating over
// recently created subgrids KDG 17 Jun 2015 - modified code to all for
// different subdivision for different axes
// ======================================================================
#include "setup_dust_grid_subdivide_overdense_cells.h"
// #define DEBUG_SDGSOC

void
setup_dust_grid_subdivide_overdense_cells (geometry_struct &geometry, int spherical_clumps)
                    ...
                    for (i = 0; i < geometry.grids[m].index_dim[0]; i++)
                        {
                            ...
                            if (z_tau > geometry.max_tau_per_cell_z)
                                subdivide = 1;
                            if ((spherical_clumps)
                                && (geometry.grids[0].grid (i, j, k).dust_tau_per_pc == geometry.clump_densities[0]))
                                subdivide = 1;
                            ...
                            if (subdivide)
                                {
                                    ...
                                    if (spherical_clumps)
                                    ...
                                    else
                                        {
                                            ...
                                            //  	      cout <<
                                            //  x_tau/float(geometry.max_tau_per_cell_x)
                                            //  << " ";
                                            // 	      cout <<
                                            // y_tau/float(geometry.max_tau_per_cell_y)
                                            // << " "; 	      cout <<
                                            ...
                                            // 	      cout <<
                                            // int(y_tau/float(geometry.max_tau_per_cell_y)) << " ";
                                            // cout << int(z_tau/float(geometry.max_tau_per_cell_z))
                                            // << endl;
                                            ...
                                            // "; 	      cout << geometry.max_tau_per_cell_z <<
                                            // endl; 	      exit(8);
                                        }
                                    ...
                                    subgrid.phys_grid_size[0]
                                        = geometry.grids[m].positions[0][i + 1] - geometry.grids[m].positions[0][i];
                                    subgrid.phys_grid_size[1]
                                        = geometry.grids[m].positions[1][j + 1] - geometry.grids[m].positions[1][j];
                                    subgrid.phys_grid_size[2]
                                        = geometry.grids[m].positions[2][k + 1] - geometry.grids[m].positions[2][k];
                                    ...
                                    int l;
                                    // ensure that the first position exactly equals the edge of
                                    // the parent cell
                                    ...
                                    // ensure that the last position exactly equals the edge of
                                    // the parent cell
                                    //   no roundoff error problems (hopefully)
                                    ...
                                    float index_radius; // radius of subgrid position in index
                                                        // values
                                    int n, o;
                                            ...
                                            for (l = 0; l < subgrid.index_dim[0]; l++)
                                                {
                                                    ...
                                                    if (spherical_clumps)
                                                        {
                                                            index_radius = sqrt (
                                                                pow (float (o) - subgrid.index_dim[2] / 2., 2.0)
                            ...
                            else
                                {
                                    ...
                                    // 	    if (geometry.grids[m].grid(i,j,k).dust_tau_per_pc >
                                    // 0.0) 	      geometry.grids[m].grid(i,j,k).num_H =
