...
void setup_dust_grid_shell(ConfigFile &param_data, geometry_struct &geometry,
...

{
  ...
  check_input_param("very_inner_radius", very_inner_radius, 0.0,
                    geometry.radius);
  ...
  if (outer_radius <= inner_radius) {
    cout << "outer shell radius is less than or equal to the inner shell radius"
         << endl;
  ...
  double radial_density_poly =
      param_data.FValue("Geometry", "shell_density_poly");
  check_input_param("shell density polynomial", radial_density_poly, -100.,
                    100);
#ifdef DEBUG_SDG
  cout << "poly = " << radial_density_poly << endl;
#endif
  if (radial_density_poly == -1.) {
    cout << "shell density polynomial is equal to -1 - new code needed!"
         << endl;
    exit(8);
  }
  ...
  if (very_inner_radius == inner_radius) {
    ...
    // 1st term
    tmp_den_constant2 =
        pow(inner_radius, radial_density_poly) * 0.5 *
    ...
    tmp_den_constant1 = tmp_den_constant2 *
                        pow(inner_radius, radial_density_poly) /
                        (inner_radius - very_inner_radius);
  }
  ...
  //   if ((fabs(tmp_den_constant2 - tmp_den_constant)/tmp_den_constant) > 0.01)
  //   {
  ...
  if (subdivide_radius > 0.0) {
    ...
    cout << "subdivide radius used (depends on main grid size) = "
      ...
      for (j = 0; j < geometry.grids[m].index_dim[1]; j++) {
        ...
        for (i = 0; i < geometry.grids[m].index_dim[0]; i++) {
          ...
          if (radius <= subdivide_radius) {
            ...
            cout << (inner_radius - very_inner_radius) * save_outer_radius
                 << endl;
            poss_index =
                int(5. * phys_cube_size /
                    ((inner_radius - very_inner_radius) * save_outer_radius)) +
                1;
            if (poss_index > 1) {
              subdivide = 1;
...
#ifdef DEBUG_SDG
#endif
            cout << "very inner radius = "
                 << very_inner_radius * save_outer_radius << " ";
