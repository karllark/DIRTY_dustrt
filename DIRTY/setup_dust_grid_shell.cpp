...
void setup_dust_grid_shell(ConfigFile &param_data, geometry_struct &geometry, random_dirty &random_obj)

{
    ...
    {
        cout << "outer shell radius is less than or equal to the inner shell radius" << endl;
    ...
    else
    {
        ...
        tmp_den_constant1 =
            tmp_den_constant2 * pow(inner_radius, radial_density_poly) / (inner_radius - very_inner_radius);
    }
            ...
            for (j = 0; j < geometry.grids[m].index_dim[1]; j++)
            {
                ...
                for (i = 0; i < geometry.grids[m].index_dim[0]; i++)
                {
                    ...
                    if (radius <= subdivide_radius)
                    {
                        ...
                        poss_index =
                            int(5. * phys_cube_size / ((inner_radius - very_inner_radius) * save_outer_radius)) + 1;
                        if (poss_index > 1)
                        {
                            subdivide = 1;
                            subdivide_any = 1;
