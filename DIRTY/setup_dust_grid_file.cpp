  ...
  for (i = 0; i < num_grids; i++)
    {
      ...
      fpixel[0] = 1;
      fpixel[1] = 1;
      fits_read_pix (pos_file_ptr, TDOUBLE, &fpixel[0], subgrid.index_dim[0] + 1, &nulval,
                     &x_pos[0], &anynul, &status);
      ...
      fpixel[0] = 1;
      fpixel[1] = 2;
      fits_read_pix (pos_file_ptr, TDOUBLE, &fpixel[0], subgrid.index_dim[1] + 1, &nulval,
                     &y_pos[0], &anynul, &status);
      ...
      fpixel[0] = 1;
      fpixel[1] = 3;
      fits_read_pix (pos_file_ptr, TDOUBLE, &fpixel[0], subgrid.index_dim[2] + 1, &nulval,
                     &z_pos[0], &anynul, &status);
      ...
      if (i == 0)
        {
          ...
          // max grid depth
          fits_read_key (tau_pc_file_ptr, TLONG, "GRDDEPTH", &geometry.max_grid_depth, comment,
                         &status);
          if (status != 0)
            {
              if (num_grids == 1)
      ...
      fits_read_pix (tau_pc_file_ptr, TFLOAT, &cfpixel[0],
                     subgrid.index_dim[0] * subgrid.index_dim[1] * subgrid.index_dim[2], &nulval,
                     &tmp_tau[0], &anynul, &status);
      ...
      if (i == 0)
        ...
        {
          fits_read_key (tau_pc_file_ptr, TINT, "PAR_GRID", &subgrid.parent_grid_num, comment,
                         &status);
          check_fits_io (status, "fits_read_key : tau_pc_file parent_grid number (PAR_GRID)");
          check_input_param ("arbitrary file input: parent_grid number", subgrid.parent_grid_num, 0,
                             (num_grids - 1));
        }
