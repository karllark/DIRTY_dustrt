...
double
calc_photon_trajectory (photon_data &photon, geometry_struct &geometry, double target_tau, double target_dist,
  ...
  while ((tau_left > ROUNDOFF_ERR_TRIG) && (dist_left > ROUNDOFF_ERR_TRIG) && (!escape))
    {
      ...
      // if the photon has escaped the main grid and if repeat boundary option
      // is set, then move the photon to the opposite side of the main grid
      if ((repeat_boundary) && (geometry.repeat_boundary_xy) && (escape) && (photon.current_grid_num == 0))
        {
          int i;
          // cout << photon.number << " " << tau_left << " " << dist_left <<
          // endl; cout << "escaped main grid." << endl; for (i = 0; i < 3;
          // i++) cout << photon.position[i] << " ";
          ...
          if (found_repeat)
            {
              ...
              // cout << photon.number << " " << tau_left << " " << dist_left
              // << endl; cout << "new positions" << endl; for (i = 0; i < 3;
              // i++) cout << photon.position[i] << " "; cout << endl; for (i =
              // 0; i < 3; i++) cout << photon.position_index[0][i] << " ";
              // cout << endl;
            }
        }
    }
