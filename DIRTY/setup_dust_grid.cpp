...
void setup_dust_grid(ConfigFile &param_data, geometry_struct &geometry,
...

{
  ...
  if (geometry.type == "sphere") {
    ...
    cout << "Setup for input geometry type (" << geometry.type
  ...
  if (geometry.source_type == "stars") {
    ...
    if (geometry.num_stars == 1) {
      ...
      geometry.star_positions[0][0] =
          param_data.FValue("Geometry", "star_pos_x");
      check_input_param("star_pos_x", geometry.star_positions[0][0],
                        -1 * geometry.radius, geometry.radius);
      geometry.star_positions[1][0] =
          param_data.FValue("Geometry", "star_pos_y");
      check_input_param("star_pos_y", geometry.star_positions[1][0],
                        -1 * geometry.radius, geometry.radius);
      geometry.star_positions[2][0] =
          param_data.FValue("Geometry", "star_pos_z");
      check_input_param("star_pos_z", geometry.star_positions[2][0],
                        -1 * geometry.radius, geometry.radius);
      // current set this to a default value (maybe use input for later)
      geometry.total_source_luminosity = 1.;
    } else {
      // get the filename
      string multiple_stars_filename =
          param_data.SValue("Geometry", "star_file");
      // check that the file exists
      ifstream multiple_stars_file(multiple_stars_filename.c_str());
      if (multiple_stars_file.fail()) {
        cout << "Multiple star file w/ positions (" << multiple_stars_filename
             << ") does not exist." << endl;
        exit(8);
      }
      ...
      if (int(pos_x.size()) != geometry.num_stars) {
        cout << "The number of positions in multiple star file ("
             << pos_x.size() << ")" << endl;
        cout << "does not match the number expected (" << geometry.num_stars
             << ")" << endl;
        exit(8);
      }
      ...
      for (i = 0; i < geometry.num_stars; i++) {
        check_input_param("multiple star pos_x", pos_x[i],
                          -1 * geometry.grids[0].phys_grid_size[0] / 2.,
                          geometry.grids[0].phys_grid_size[0] / 2.);
        check_input_param("multiple star pos_y", pos_y[i],
                          -1 * geometry.grids[0].phys_grid_size[1] / 2.,
                          geometry.grids[0].phys_grid_size[1] / 2.);
        check_input_param("multiple star pos_z", pos_z[i],
                          -1 * geometry.grids[0].phys_grid_size[2] / 2.,
      ...
      // now divide the 5th position by the total luminosity to get a number
      // between 0 and 1 for the easy random generation of photons
      for (i = 0; i < geometry.num_stars; i++) {
        geometry.star_positions[4][i] /= geometry.total_source_luminosity;
    ...
    // diffuse field this would be the case for a diffuse field illuminating the
    // source from just one direction
    ...
    if (source_filename == "isotropic") {
      ...
      if (source_file.fail()) {
        cout << "Diffuse source file (" << source_filename
             << ") does not exist." << endl;
        exit(8);
      }
      source_file.close();
      ...
      for (i = 0; i < int(theta.size()); i++) {
        check_input_param("diffuse source theta", theta[i], -1 * M_PI / 2.,
                          M_PI / 2.);
        ...
        geometry.diffuse_source_sum_intensity.push_back(
            geometry.total_source_luminosity);
      }
      // now divide the sum_intensity by the total luminosity to get a number
      // between 0 and 1 for the easy random generation of photons
      for (i = 0; i < int(theta.size()); i++) {
        geometry.diffuse_source_sum_intensity[i] /=
            geometry.total_source_luminosity;
      }
      ...
      // no longer needed as this the size of each cell is explicitly in
      // calculation
    ...
    if (geometry.type == "sphere") {
      ...
      geometry.total_source_luminosity *=
          M_PI * pow(0.95 * geometry.radius * Constant::PC_CM, 2.0);
      //      cout << geometry.total_source_luminosity << endl;
    } else {
      cout << "diffuse source from file not setup for anything but sphere"
           << endl;
    ...
    if (alpha == -3.) {
      cout << "pow_sphere: exponent == -3 case not implemented (need logarithm)"
           << endl;
      exit(8);
    } else if ((alpha < -3.) && (r1 == 0.)) {
      cout << "pow_sphere: exponent < -3 and outer_radius == 0 (infinity at "
              "origin)"
           << endl;
      exit(8);
    }
    ...
    cout << "Setup for input source type (" << geometry.source_type
         << ") not found [NEW CODE NEEDED]." << endl;
    exit(8);
  }
  ...
  if (geometry.num_observers == 1) {
    ...
    if (geometry.internal_observer == 0) {
      geometry.observer_angles[0][0] =
          param_data.FValue("Geometry", "obs_theta");
      ...
      check_input_param("obs_y", geometry.observer_position[0],
                        -1.0 * geometry.radius, geometry.radius);
      geometry.observer_position[2] = param_data.FValue("Geometry", "obs_z");
      check_input_param("obs_z", geometry.observer_position[0],
                        -1.0 * geometry.radius, geometry.radius);
    }
    ...
    if (multiple_obs_file.fail()) {
      cout << "Multiple obs file w/ positions (" << multiple_obs_filename
           << ") does not exist." << endl;
      exit(8);
    }
    ...
    if (int(obs_theta.size()) != geometry.num_observers) {
      cout << "The number of directions in multiple observers file ("
           << obs_theta.size() << ")" << endl;
      cout << "does not match the number expected (" << geometry.num_observers
           << ")" << endl;
      exit(8);
    }
