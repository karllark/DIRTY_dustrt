...
void
setup_dust_grid (ConfigFile &param_data, geometry_struct &geometry, photon_data &photon,
...

{
  ...
  if (geometry.type == "sphere")
    ...
    {
      cout << "Setup for input geometry type (" << geometry.type << ") not found [NEW CODE NEEDED]."
           << endl;
      exit (8);
    }
      ...
      if (geometry.num_stars == 1)
        {
          ...
          check_input_param ("star_pos_x", geometry.star_positions[0][0], -1 * geometry.radius,
                             geometry.radius);
          geometry.star_positions[1][0] = param_data.FValue ("Geometry", "star_pos_y");
          check_input_param ("star_pos_y", geometry.star_positions[1][0], -1 * geometry.radius,
                             geometry.radius);
          geometry.star_positions[2][0] = param_data.FValue ("Geometry", "star_pos_z");
          check_input_param ("star_pos_z", geometry.star_positions[2][0], -1 * geometry.radius,
                             geometry.radius);
      ...
      else
        {
          ...
          if (int (pos_x.size ()) != geometry.num_stars)
            {
              cout << "The number of positions in multiple star file (" << pos_x.size () << ")"
                   << endl;
  ...
  if (geometry.num_observers == 1)
    {
      ...
      if (geometry.internal_observer == 0)
        ...
        { // internal observer
          ...
          check_input_param ("obs_x", geometry.observer_position[0], -1.0 * geometry.radius,
                             geometry.radius);
          geometry.observer_position[1] = param_data.FValue ("Geometry", "obs_y");
          check_input_param ("obs_y", geometry.observer_position[0], -1.0 * geometry.radius,
                             geometry.radius);
          geometry.observer_position[2] = param_data.FValue ("Geometry", "obs_z");
          check_input_param ("obs_z", geometry.observer_position[0], -1.0 * geometry.radius,
                             geometry.radius);
        }
    }
  else
    {
      ...
      if (multiple_obs_file.fail ())
        {
          cout << "Multiple obs file w/ positions (" << multiple_obs_filename << ") does not exist."
               << endl;
          exit (8);
        }
      ...
      if (int (obs_theta.size ()) != geometry.num_observers)
        {
          cout << "The number of directions in multiple observers file (" << obs_theta.size ()
               << ")" << endl;
