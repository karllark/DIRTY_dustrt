...
void
classify_stellar_photon (output_struct &output, photon_data &photon, geometry_struct &geometry, runinfo_struct &runinfo,
            ...
            if ((i == 0) || (geometry.num_observers > 1))
                {
                    ...
                    if (photon.number == OUTNUM)
                        {
                            ...
                            cout << "classify_stellar_photon: in stellar_weight = " << tmp_photon.stellar_weight
                                 << endl;
                        }
#endif
                    ...
                    if (geometry.internal_observer == 0)
                        {
                            // if averging over the entire 4pi steradians is desired,
                            // randomize the observer position
                        ...
                        { // internal observer
                            for (int k = 0; k < 3; k++)
                                output.outputs[i].observer_position[k] = geometry.observer_position[k];
                        }
                    // determine stellar weight towards observer
                    tmp_photon.stellar_weight
                        = stellar_weight_towards_observer (tmp_photon, geometry, output.outputs[i].observer_position);
                    ...
                    if (geometry.internal_observer == 0)
                        {
                            ...
                            for (k = 0; k < 2; k++)
                                {
                                    angle
                                        = atan (tmp_photon.position[k] / (geometry.distance - tmp_photon.position[2]));
                                    image_indxs[k]
                                        = int ((1.0 + (angle / geometry.angular_radius)) * output.image_size[k] * 0.5);
                                    ...
                                    if ((image_indxs[k] < 0) || (image_indxs[k] > (output.image_size[k] - 1)))
                                        {
                                            cout << "classify stellar photon: image_indxs[" << k
                                                 << "] = " << image_indxs[k] << " (beyond image bounds)" << endl;
                                            cout << "real version of index = "
                                                 << (1.0 + (angle / geometry.angular_radius)) * output.image_size[k]
                                                        * 0.5
                                                 << endl;
                                            exit (8);
                                        }
                    ...
                    else
                        {
                            ...
                            for (k = 0; k < 3; k++)
                                {
                                    obs_to_birth[k] = output.outputs[i].observer_position[k] - tmp_photon.position[k];
                    ...
                    if (photon.number == OUTNUM)
                        {
                            ...
                            cout << "stellar photon image indexs = (" << image_indxs[0] << "," << image_indxs[1] << ")"
                                 << endl;
                        }
#endif
                }
