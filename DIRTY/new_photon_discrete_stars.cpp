...
void new_photon_discrete_stars(photon_data &photon, geometry_struct &geometry, random_dirty &random_obj)

{
    ...
    if (geometry.num_stars > 1)
    {
        ...
        while ((i < geometry.num_stars) && (ran_num > geometry.star_positions[4][i]))
            i++;
        pos_index = i;
