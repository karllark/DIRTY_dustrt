...
// 22 Oct 2013/KDG - added option to deal with forced_first_scatter getting
// zero optical depth
// ======================================================================
...
void
radiative_transfer (geometry_struct &geometry, runinfo_struct &runinfo, output_struct &output,
      ...
      if ((i > 0) && ((i % (geometry.n_photons / 20)) == 0) && (runinfo.rt_check_converged))
        {
          ...
          if (total_stellar_weight_err > 0.0)
            total_stellar_weight_err
                = sqrt (total_stellar_weight_err / output.outputs[0].total_num_photons);
          else
          ...
          total_stellar_weight_err
              /= output.outputs[0].total_stellar_weight / output.outputs[0].total_num_photons;

          // compute total scattered weight uncertainty
          // first compute the uncertainty on the average weight of a single
          // scattered photon
          if (output.outputs[0].total_num_photons > 0)
            total_scattered_weight_err = output.outputs[0].total_scattered_weight_x2
          ...
          total_scattered_weight_err /= output.outputs[0].total_scattered_weight
                                        / output.outputs[0].total_num_scattered_photons;
          ...
          if (((geometry.n_photons > 100) && ((i % (geometry.n_photons / 20)) == 0)) && (i > 0))
            {
              ...
              cout << " ave 1st tau = "
                   << output.outputs[0].ave_first_tau / output.outputs[0].total_num_photons;
              cout << endl;
            }
        }
