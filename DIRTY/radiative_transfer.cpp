...
void radiative_transfer(geometry_struct &geometry, runinfo_struct &runinfo,
...

{
  ...
  while ((i < geometry.n_photons) && (!done_rt)) {
    if ((i > 0) && ((i % (geometry.n_photons / 20)) == 0) &&
        (runinfo.rt_check_converged)) {
      ...
      if (output.outputs[0].total_num_photons > 0)
        total_stellar_weight_err = output.outputs[0].total_stellar_weight_x2 /
                                       output.outputs[0].total_num_photons -
                                   pow(output.outputs[0].total_stellar_weight /
                                           output.outputs[0].total_num_photons,
                                       2);
      if (total_stellar_weight_err > 0.0)
        total_stellar_weight_err = sqrt(total_stellar_weight_err /
                                        output.outputs[0].total_num_photons);
      else
      ...
      total_stellar_weight_err /= output.outputs[0].total_stellar_weight /
                                  output.outputs[0].total_num_photons;
      ...
      if (output.outputs[0].total_num_photons > 0)
        total_scattered_weight_err =
            output.outputs[0].total_scattered_weight_x2 /
                output.outputs[0].total_num_scattered_photons -
            pow(output.outputs[0].total_scattered_weight /
                    output.outputs[0].total_num_scattered_photons,
                2);
      ...
      total_scattered_weight_err /=
          output.outputs[0].total_scattered_weight /
          output.outputs[0].total_num_scattered_photons;
    ...
    if (runinfo.verbose >= 1) {
      if (((geometry.n_photons > 100) &&
           ((i % (geometry.n_photons / 20)) == 0)) &&
          (i > 0)) {
        //|| ((i >= 25650) && (i <= 34200))) {
        cout << "current # = " << i;
        cout << " stel sl = "
             << output.outputs[0].total_stellar_weight /
                    output.outputs[0].total_num_photons;
        cout << " scat sl = "
             << output.outputs[0].total_scattered_weight /
                    output.outputs[0].total_num_photons;
        ...
        cout << " ave 1st tau = "
             << output.outputs[0].ave_first_tau /
                    output.outputs[0].total_num_photons;
        cout << endl;
      }
    }
