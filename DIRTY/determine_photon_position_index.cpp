...

{
  ...
  while (not done) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      // take care of case when we are at the edge of the grid in the maximum
      // sense
      ...
      if (photon.number == OUTNUM) {
        ...
        cout << geometry.grids[cur_grid_num]
      ...
      if ((photon.position[i] < geometry.grids[cur_grid_num].positions[i][0]) ||
        ...
        int not_ok = 0;
        if (photon.position[i] < geometry.grids[cur_grid_num].positions[i][0]) {
          float frac_miss = (geometry.grids[cur_grid_num].positions[i][0] -
                             photon.position[i]) /
                            (geometry.grids[cur_grid_num].positions[i][1] -
                             geometry.grids[cur_grid_num].positions[i][0]);
          ...
          float frac_miss =
              ...
              (geometry.grids[cur_grid_num]
               ...
               geometry.grids[cur_grid_num]
                   .positions[i]
                             [geometry.grids[cur_grid_num].index_dim[i] - 1]);
          cout << "frac_miss++ = " << frac_miss << endl;
          if (frac_miss < ROUNDOFF_ERR_INDEX)
            photon.position[i] =
                geometry.grids[cur_grid_num]
        ...
        if (not_ok) {
          ...
          // geometry.grids[cur_grid_num].positions[i][0])/
          // 				     geometry.grids[cur_grid_num].phys_cube_size[i])
          // << endl; cout << "real value of pindex = " << (photon.position[i] -
          // geometry.grids[cur_grid_num].positions[i][0])/
          //   geometry.grids[cur_grid_num].phys_cube_size[i] << endl;
          cout << "out of a possible "
               << geometry.grids[cur_grid_num].index_dim[i] << endl;
          cout << "out of a possible2 "
               << geometry.grids[cur_grid_num].positions[i].size() << endl;
          int parent_grid_num = geometry.grids[cur_grid_num].parent_grid_num;
          cout << "parent grid cell min = "
               << geometry.grids[parent_grid_num]
                      .positions[i][photon.position_index[k - 1][i]]
               << endl;
          cout << "parent grid cell max = "
               << geometry.grids[parent_grid_num]
                      .positions[i][photon.position_index[k - 1][i] + 1]
               << endl;
          exit(8);
        }
      }
