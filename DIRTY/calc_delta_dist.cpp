...
// 2007 Feb/KDG - fixed case where dir cosines are zero -> nan distance
// traveled
...
double
calc_delta_dist (photon_data &photon, geometry_struct &geometry, double target_tau, double target_dist, int &escape,
                 double &tau_traveled)
    ...
    if (photon.number == OUTNUM)
        {
            ...
            cout << "photon.grid_number[photon.current_grid_num] = " << photon.grid_number[photon.current_grid_num]
                 << endl;
            cout << geometry.grids[photon.grid_number[photon.current_grid_num]].grid.nRow () << " ";
            cout << geometry.grids[photon.grid_number[photon.current_grid_num]].grid.nCol () << " ";
            cout << geometry.grids[photon.grid_number[photon.current_grid_num]].grid.n3rd () << endl;
    ...
    if ((k < (photon.num_current_grids - 1)) || (dust_tau_ref_per_pc <= -1.0))
        {
            ...
            int escape_grid = 0;
            if (escape)
                { // determine which of x,y,z exited first
                    photon.current_grid_num--;
        ...
        { // otherwise determine the distance in this cell

            double delta_position[3]; // x,y,z distance to the edge of the cell
                                      // (parallel to the x,y,z axes)
            double delta_distance[3]; // distance the photon would need to travelel
                                      // to exit the cell in the x,y,z directions
            ...
            for (i = 0; i < 3; i++)
                {
                    if (photon.dir_cosines[i] > 0.0)
                        {
                            delta_position[i] = geometry.grids[photon.grid_number[photon.current_grid_num]]
                                                    .positions[i][photon.position_index[k][i] + 1]
                                                - photon.position[i];
                        }
                    ...
                    // calculate the distance the photon would  have to travel in each
                    // x,y,z direction to exit the cell
                    if (photon.dir_cosines[i] != 0.0)
            ...
            // determine the optical depth transversed before the photon exits the
            // cell
            tau_traveled = dust_tau_per_pc * distance_traveled;

            // if this tau_traveled is larger than the amount of tau_left, then
            // decrease the distance/tau traveled 2nd if statement is the same for
            // distance
            ...
            if (photon.path_cur_cells >= 0)
                { // if -1, don't save
                    ...
                    if (photon.path_max_cells < photon.path_cur_cells)
                        {
                            cout << "photon path # cells is smaller than the current cell "
                                    "number"
                                 << endl;
                            exit (8);
                        }
            ...
            if (photon.dir_cosines[min_index] > 0.0)
                {
                    ...
                    photon.position[min_index] = geometry.grids[photon.grid_number[photon.current_grid_num]]
