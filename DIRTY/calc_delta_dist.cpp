...

{
        ...
        { // determine which of x,y,z exited first
            ...
            for (i = 0; i < 3; i++)
            {
                if (((photon.dir_cosines[i] > 0.0) &&
                     (photon.position[i] == geometry.grids[photon.grid_number[photon.current_grid_num]]
    ...
    { // otherwise determine the distance in this cell
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            {
                delta_position[i] = photon.position[i] - geometry.grids[photon.grid_number[photon.current_grid_num]]
                                                             .positions[i][photon.position_index[k][i]];
            }
            else
            { // photon.dir_cosines[i] == 0.0
