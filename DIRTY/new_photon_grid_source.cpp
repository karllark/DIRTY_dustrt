...
// function object for searching grid layer according to the emitted_energy
struct grid_comparator_uniform
    : public std::binary_function<one_grid, double, bool> {
  int x;
...
// function object for searching grid cell according to the emitted_energy
struct cell_comparator_uniform
    : public std::binary_function<grid_cell, double, bool> {
  int x;
...
// full procedure
void new_photon_grid_source(photon_data &photon, geometry_struct &geometry,
...

{
  ...
  if (random_obj.random_num() >=
    ...
    if (ran_val <= geometry.grids[0].grid.back().emitted_energy_weighted[x]) {
      ...
      selected_grid =
          lower_bound(geometry.grids.begin() + 1, geometry.grids.end(), ran_val,
                      grid_comparator(x));
    ...
    NumUtils::Cube<grid_cell>::iterator selected_cell =
        lower_bound(selected_grid->grid.begin(), selected_grid->grid.end(),
                    ran_val, cell_comparator(x));
    ...
    NumUtils::Cube<grid_cell>::iterator selected_cell =
        lower_bound(selected_grid->grid.begin(), selected_grid->grid.end(),
                    ran_val, cell_comparator_uniform(x));
    int cell_num = selected_cell - selected_grid->grid.begin();
    selected_grid->grid.get_xyz(cell_num, x_val, y_val, z_val);
  ...
  biased_weight_factor = (1.0 - geometry.emit_bias_fraction) +
                         geometry.emit_bias_fraction *
                             runinfo.emitted_lum[0][x] /
                             (geometry.grids[grid_num]
