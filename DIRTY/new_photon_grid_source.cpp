    ...
    if (random_obj.random_num () >= geometry.emit_bias_fraction)
        ...
        { // uniformly sample the grid
            ...
            NumUtils::Cube<grid_cell>::iterator selected_cell = lower_bound (
                selected_grid->grid.begin (), selected_grid->grid.end (), ran_val, cell_comparator_uniform (x));
            int cell_num = selected_cell - selected_grid->grid.begin ();
            selected_grid->grid.get_xyz (cell_num, x_val, y_val, z_val);
    ...
    photon.position[0]
        = geometry.grids[grid_num].positions[0][x_val]
          + random_obj.random_num ()
                * (geometry.grids[grid_num].positions[0][x_val + 1] - geometry.grids[grid_num].positions[0][x_val]);
    photon.position[1]
        = geometry.grids[grid_num].positions[1][y_val]
          + random_obj.random_num ()
                * (geometry.grids[grid_num].positions[1][y_val + 1] - geometry.grids[grid_num].positions[1][y_val]);
    photon.position[2]
        = geometry.grids[grid_num].positions[2][z_val]
          + random_obj.random_num ()
                * (geometry.grids[grid_num].positions[2][z_val + 1] - geometry.grids[grid_num].positions[2][z_val]);
