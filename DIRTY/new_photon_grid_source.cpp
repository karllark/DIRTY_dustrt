...
struct grid_comparator : public std::binary_function<one_grid, double, bool>
{
  ...
  inline bool
  operator() (one_grid &grid, double value) const
  {
...
struct cell_comparator : public std::binary_function<grid_cell, double, bool>
{
  ...
  inline bool
  operator() (grid_cell &cell, double value) const
  {
...
// full procedure
void
new_photon_grid_source (photon_data &photon, geometry_struct &geometry, runinfo_struct &runinfo,
...

{
    ...
    { // sample radiation field
      ...
      if (ran_val <= geometry.grids[0].grid.back ().emitted_energy_weighted[x])
      ...
      else
        {
          ...
          selected_grid = lower_bound (geometry.grids.begin () + 1, geometry.grids.end (), ran_val,
                                       grid_comparator (x));
          grid_num = selected_grid - geometry.grids.begin ();
        }

      // use binary search to check which cell are we in
      NumUtils::Cube<grid_cell>::iterator selected_cell = lower_bound (
          selected_grid->grid.begin (), selected_grid->grid.end (), ran_val, cell_comparator (x));
    ...
    { // uniformly sample the grid
      ...
      if (ran_val <= geometry.grids[0].grid.back ().emitted_energy_uniform[x])
      ...
      else
        {
          // binary search, O(N) => O(lnN)
          selected_grid = lower_bound (geometry.grids.begin () + 1, geometry.grids.end (), ran_val,
                                       grid_comparator_uniform (x));
          grid_num = selected_grid - geometry.grids.begin ();
        }

      // use binary search to check which cell are we in
      NumUtils::Cube<grid_cell>::iterator selected_cell
          = lower_bound (selected_grid->grid.begin (), selected_grid->grid.end (), ran_val,
                         cell_comparator_uniform (x));
      int cell_num = selected_cell - selected_grid->grid.begin ();
      selected_grid->grid.get_xyz (cell_num, x_val, y_val, z_val);

      //     // uniform sampling gets a weight based on the fraction of the
      //     total emitted energy (correcting for uniform sampling)
      //     // this gives the number of photons that will be emitted *total*
      //     from this cell photon.stellar_weight =
      //     geometry.n_photons*geometry.grids[grid_num].grid(x_val, y_val,
      //     z_val).emitted_energy[0][x]/runinfo.emitted_lum[0][x];
      //     // divide by the number of photons that will be emitted per cell
      //     to get the weight for a single photon photon.stellar_weight /=
  ...
  biased_weight_factor
      = (1.0 - geometry.emit_bias_fraction)
        + geometry.emit_bias_fraction * runinfo.emitted_lum[0][x]
              / (geometry.grids[grid_num].grid (x_val, y_val, z_val).emitted_energy[0][x]
                 * geometry.emitted_lum_uniform[x]);
  ...
  photon.position[0] = geometry.grids[grid_num].positions[0][x_val]
                       + random_obj.random_num ()
                             * (geometry.grids[grid_num].positions[0][x_val + 1]
                                - geometry.grids[grid_num].positions[0][x_val]);
  photon.position[1] = geometry.grids[grid_num].positions[1][y_val]
                       + random_obj.random_num ()
                             * (geometry.grids[grid_num].positions[1][y_val + 1]
  ...
  // save the birth grain/emission type probabilities if needed for dust
  // thermal emission part of dirty
  if (runinfo.dust_thermal_emission && runinfo.do_emission_grain)
    for (i = 0; i < runinfo.n_emission_grain_types; i++)
      {
        photon.birth_photon_type_prob[i] = geometry.grids[grid_num]
                                               .grid (x_val, y_val, z_val)
                                               .emitted_energy[i][geometry.wave_index];
        //       cout << "i = " << i << "; " <<
