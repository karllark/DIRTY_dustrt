...
void
radiative_transfer_many_waves (geometry_struct &geometry, runinfo_struct &runinfo, output_struct &output,
            ...
            if (rt_type > REG_RT)
                {
                    switch (rt_type)
                        ...
                        case ERE_RT:
                            ...
                            break;
                        case DE_RT:
                            if (runinfo.emitted_lum[0][i] < 1e-20 * runinfo.out_sed_lum[0][i] * runinfo.sed_lum[i])
                                do_rt = 0;
                            break;
                        }
                }
            ...
            if (do_rt)
                {
                    if (runinfo.verbose >= 1)
                        {
                            cout << "working on wavelength [micron] = " << runinfo.wavelength[i] * Constant::CM_UM
                                 << endl;
                    ...
                    //     // check the absorbed energy grid (temp needed as energy not
                    //     conserved)
                    //     // KDG - 23 Mar 2008
