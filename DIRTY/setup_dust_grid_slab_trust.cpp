...
// KDG  8 May 2008 - fixed error in setting up the dust grid physical
// dimensions
...
void
setup_dust_grid_slab_trust (ConfigFile &param_data, geometry_struct &geometry, random_dirty &random_obj)
    ...
    for (k = 0; k < main_grid.index_dim[2]; k++)
        {
            ...
            for (j = 0; j < main_grid.index_dim[1]; j++)
                {
                    // y_val = (main_grid.positions[1][j] +
                    // main_grid.positions[1][j+1])/2.0;
                    for (i = 0; i < main_grid.index_dim[0]; i++)
                        {
                            // x_val = (main_grid.positions[0][i] +
                            // main_grid.positions[0][i+1])/2.0;
                            //  slab position should be a negative z if behind stars
                            //  (observer at positive z)
