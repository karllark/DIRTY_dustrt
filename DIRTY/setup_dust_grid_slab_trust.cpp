...
void setup_dust_grid_slab_trust(ConfigFile &param_data,
...

{
  ...
  for (i = 0; i <= main_grid.index_dim[1]; i++)
    y_pos[i] =
        double(i) * (size_xy) / double(main_grid.index_dim[1]) - size_xy / 2.;
  ...
  float log_deltaz =
      (log_slab_z2 - log_slab_z1) / double(main_grid.index_dim[2] - 1);
  for (i = 0; i <= (main_grid.index_dim[2] - 1); i++)
