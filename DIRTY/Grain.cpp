...
void Grain::MakeGrain(string const &fOpticalConstants,
  ...
  if (!_file.is_open()) {
    cout << "Unable to open optical constants file " << fOpticalConstants
         << endl;
    exit(8);
  }
  // ***************************************************************************
  ...
  if (interpSize) {
    ...
    if (a_min != -1) {  // been set...
      ...
      if (a_min > size[0]) size.insert(size.begin(), a_min);
      // for (int _sz=0;_sz<size.size();++_sz) cout << _sz << " " << size[_sz]
      // << endl;
      //  Remove everything ouside of a_min
      size.erase(
          remove_if(size.begin(), size.end(), bind2nd(less<float>(), a_min)),
          size.end());
      // cout << "post: " << endl;
      // for (int _sz=0;_sz<size.size();++_sz) cout << _sz << " " << size[_sz]
      // << endl;
    ...
    if (a_max != -1) {  // been set...
      ...
      if (a_max < size[size.size() - 1]) size.push_back(a_max);
      size.erase(
          remove_if(size.begin(), size.end(), bind2nd(greater<float>(), a_max)),
          size.end());
      // Since we may have modified the size distribution, do interpolate.
      interpSize = true;
    }
...

{
  if (a_size < size[0] || a_size > size[nsize - 1]) {
    cout << "You are attempting to retrieve optical constants outside of the"
         << endl;
    cout << "size range defined by your input grains.  This is dangerous and"
         << endl;
    cout << "I VANT TO MAKE AB-SA-LUTE-LY CLEE-AR, STREECTLY VORBIDEN!" << endl;
    cout << "Error in Grain::getCAbs(float)." << endl;
    cout << "Defined size range is " << size[0] << "--" << size[nsize - 1]
         << endl;
...

{
  ...
  if (it_loc != size.end()) {  // We have found an exact match
    ...
    iTmp1 = Tmp1.begin();
    for (iphFunc1 = phFunc[idx].begin(); iphFunc1 != phFunc[idx].end();
         iphFunc1++) {
