...
//                   The first line after the comments and before the data
//                   sould have the form: i   TMIN,TMAX NT:  1.0,2500.0 50
// ******************************************************************************
#include "Grain.h"
...
void
Grain::MakeGrain (string const &fOpticalConstants, string const &fCalorimetry, vector<float> &MasterWave,
                  vector<float> &MasterSize, string const &Path, float a_min, float a_max)
{
  ...
  if (interpSize)
    {
      MasterSize.erase (remove_if (MasterSize.begin (), MasterSize.end (), bind2nd (less<float> (), _size[0])),
                        MasterSize.end ());
      MasterSize.erase (
          remove_if (MasterSize.begin (), MasterSize.end (), bind2nd (greater<float> (), _size[_nsize - 1])),
          MasterSize.end ());
  ...
  else
    {
      ...
      if (a_min != -1)
        { // been set...
          ...
          // for (int _sz=0;_sz<size.size();++_sz) cout << _sz << " " <<
          // size[_sz]
          // << endl;
          //  Remove everything ouside of a_min
          size.erase (remove_if (size.begin (), size.end (), bind2nd (less<float> (), a_min)), size.end ());
          // cout << "post: " << endl;
          // for (int _sz=0;_sz<size.size();++_sz) cout << _sz << " " <<
          // size[_sz]
          // << endl;
      ...
      if (a_max != -1)
        { // been set...
          ...
          size.erase (remove_if (size.begin (), size.end (), bind2nd (greater<float> (), a_max)), size.end ());
          // Since we may have modified the size distribution, do interpolate.
          interpSize = true;
        }
