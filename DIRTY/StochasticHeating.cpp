...
//               Modified return values of all sub calls to be int - will
//               return status to be passed back up the line -KAM
#include <iostream>
#include <vector>
...
int
StochasticHeating (vector<float> &wave, vector<float> &cJprod, vector<float> &cabs,
                   vector<float> &Temperature, vector<float> &Enthalpy, float EAbs, float &TMin,
                   float &TMax, float &TEq, uint _sz, vector<vector<double> > &StochasticLum)

{
  ...
  while (!converged)
    { // convergence bracket
      ...
      for (int i = 0; i < nWave; i++, idb++, it1++, it++)
        {
          ...
          _pFac = accumulate (_pofTint.begin (), _pofTint.end (),
                              0.0); // Sum_T (P(T)*B(T))
          *idb = (*it) * _pFac;
          StochasticLum[_sz][i] = *idb; // this is C(lam)*Sum_T (P(T)*B(T,lam))
                                        // ~ stochastic L(lam)
        }
      ...
      if (loop_count >= 100)
        {
          cout << "** Breaking the convergence loop in StochasticHeating(): "
                  "loop "
               << loop_count << endl;
