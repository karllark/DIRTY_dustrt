...
void
setup_dust_grid_check_grid (geometry_struct &geometry, int cur_grid, int par_grid,
                            vector<int> &par_idim)
      ...
      for (i = 0; i < 3; i++)
        {
          ...
          if (geometry.grids[cur_grid].positions[i][0]
            ...
            {
              cout << "min subgrid i = " << i << " position does not match parent cell position"
                   << endl;
              cout << "subgrid min = " << geometry.grids[cur_grid].positions[i][0];
              cout << "; parent cell min = " << geometry.grids[par_grid].positions[i][par_idim[i]]
                   << endl;
          ...
          if (geometry.grids[cur_grid].positions[i][geometry.grids[cur_grid].index_dim[i]]
            ...
            {
              cout << "max subgrid i = " << i << " position does not match parent cell position"
                   << endl;
              cout << "subgrid max = "
                   << geometry.grids[cur_grid].positions[i][geometry.grids[cur_grid].index_dim[i]];
