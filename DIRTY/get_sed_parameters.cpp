...
void get_sed_parameters(ConfigFile &param_data, runinfo_struct &runinfo,
...

{
  ...
  if (runinfo.sed_type == "NULL") {
    ...
    for (i = 0; i < runinfo.wavelength.size(); i++)
      // reasonable for a star, needed to get radiation field correct for single
      // wavelength case
    ...
    if (runinfo.sed_type == "ssp_file") {
      ...
      // convert input SED from log(a) to a and scale by sfr/mass and change
      // from Hz^-1 to cm^-1 convert the input wavelength to microns
      ...
      for (i = 0; i < wavelength.size(); i++) {
        ...
        luminosity[i] = sfr_or_mass * pow(10., luminosity[i]) *
                        (Constant::LIGHT) / pow(wavelength[i], 2.0);
      }
      ...
      cout << "Type of SED [" << runinfo.sed_type << "] not recognized."
           << endl;
      exit(8);
    }

    // determine if the mapping the input SED to the requested wavelength grid
    ...
    if (mapping_bin == 1) {
      ...
      for (i = 0; i < luminosity.size(); i++) {
        ...
        if ((wavelength[i] >= runinfo.wavelength[0]) &&
            (wavelength[i] <=
             runinfo.wavelength[runinfo.wavelength.size() - 1])) {
          closeIter =
              find_if(runinfo.wavelength.begin(), runinfo.wavelength.end(),
                      bind2nd(greater_equal<float>(),
                              static_cast<float>(wavelength[i])));
#ifdef DEBUG_GSP
      ...
      // put the luminosity in log space to make the interpolation better for
      // rapidly changing seds
      vector<double> temp_sed_nozero;
      vector<double> temp_sed_nozero_wave;
      for (i = 0; i < runinfo.wavelength.size(); i++) {
        if (temp_sed_npts[i] > 0) {
          temp_sed_nozero.push_back(
              log10l(double(temp_sed[i] / temp_sed_npts[i])));
      ...
      if (runinfo.wavelength.back() > temp_sed_nozero_wave.back()) {
        ...
        // log10l(*(x_iter - 1)))*(log10l(x) - log10l(*x_iter)) + *y_iter;
        double x = log10l(runinfo.wavelength.back());
        double y = (*y_iter - *(y_iter - 1)) / (*x_iter - *(x_iter - 1)) *
                       (x - *x_iter) +
                   *y_iter;
        // cout << x << "\t" << y << endl;
      ...
      // numbers are power law coefficents for extrapolation on the left & right
      // sides
      runinfo.sed_lum =
          interpol(temp_sed_nozero, temp_sed_nozero_wave, run_wave, 0, 0);
