...
void
get_run_parameters (ConfigFile &param_data, output_struct &output, geometry_struct &geometry, runinfo_struct &runinfo)
{
  ...
  // removed possibility to do disk storage when polyc added (KDG - 30 Mar
  // 2009) leave in the code (where possible) in case we want to use the disk
  // at a later date
  ...
  // repeat xy boundary - only works for models that fully fill the main grid
  // with dust
  geometry.repeat_boundary_xy = param_data.IValue ("Run", "repeat_boundary_xy");
  if (geometry.repeat_boundary_xy == -99)
    geometry.repeat_boundary_xy = 0; // set to no if not initially set
  check_input_param ("repeat_boundary_xy", geometry.repeat_boundary_xy, 0, 1);
  ...
  if ((image_size < 0) && (image_x_size < 0) && (image_y_size < 0))
    {
      cout << "either image_size or (image_x_size, image_y_size) need to be "
              "set."
           << endl;
      exit (8);
    }
  ...
  runinfo.total_emitted_energy = 0.0; // need to zero out so the first iteration
                                      // always happens (measurement is a delta)
  ...
  if (runinfo.ran_seed == -99)
    runinfo.ran_seed = long (987654321); // default
