...
void new_photon_diffuse_source(photon_data &photon, geometry_struct &geometry,
...

{
  ...
  if (geometry.new_photon_source_type == NEW_PHOTON_DIFFUSE_ISOTROPIC) {
    ...
    // testing of single illumination directions (should probably make this an
    // option)
    //     phi = 0.*(M_PI/180.);
    ...
    while ((i < int(geometry.diffuse_source_sum_intensity.size())) &&
           (ran_num > geometry.diffuse_source_sum_intensity[i]))
      i++;
    int pos_index = i;
...
//     cout << geometry.diffuse_source_phi[pos_index] << endl;
    ...
    photon.dir_cosines[2] =
        cos(geometry.diffuse_source_theta[pos_index] + (M_PI / 2.));
    double temp = sqrt(1.0 - pow(photon.dir_cosines[2], 2));
    photon.dir_cosines[0] = cos(phi) * temp;
    photon.dir_cosines[1] = sin(phi) * temp;
  }
  ...
  double pos_radius =
      sqrt(pow(0.95 * geometry.radius, 2.) * random_obj.random_num());
  ...
  // derived as the matrix multiplication between rotation in the xy(phi) and
  // xz(theta) planes
  ...
  for (i = 0; i < 3; i++)
    cout << i << " " << photon.position[i] << " " << photon.dir_cosines[i]
         << endl;
  cout.flush();
#endif
  ...
  distance_traveled = calc_photon_trajectory(
      photon, geometry, target_tau, target_dist, escape, tau_to_surface, 0);
#ifdef DEBUG_NPDS
  ...
  // now determine the position indexes of the photon (again to handle the
  // edges)
  determine_photon_position_index_initial(geometry, photon);
