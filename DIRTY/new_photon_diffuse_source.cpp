...
void
new_photon_diffuse_source (photon_data &photon, geometry_struct &geometry, random_dirty &random_obj)
  ...
  if (geometry.new_photon_source_type == NEW_PHOTON_DIFFUSE_ISOTROPIC)
    {
      ...
      // testing of single illumination directions (should probably make this
      // an option)
      //     phi = 0.*(M_PI/180.);
...
//     cout << geometry.diffuse_source_phi[pos_index] << endl;
  ...
  // now rotate the photon position so that it is perpendicular to the
  // direction
