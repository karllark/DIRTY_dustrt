...
// main routine
//
void output_results(output_struct &output, geometry_struct &geometry,
...

{
  ...
  for (i = 0; i < output.num_outputs; i++) {
    ...
    if (output.outputs[i].total_num_photons > 0)
      total_stellar_weight_err = output.outputs[i].total_stellar_weight_x2 /
                                     output.outputs[i].total_num_photons -
                                 pow(output.outputs[i].total_stellar_weight /
                                         output.outputs[i].total_num_photons,
                                     2);
    if (total_stellar_weight_err > 0.0)
      total_stellar_weight_err =
          sqrt(total_stellar_weight_err / output.outputs[i].total_num_photons);
    else
    ...
    total_stellar_weight_err /= output.outputs[i].total_stellar_weight /
                                output.outputs[i].total_num_photons;

    if (runinfo.verbose >= 2) {
      cout << "Stellar weight = " << output.outputs[i].total_stellar_weight
           << endl;
    }

    double stellar_sl = output.outputs[i].total_stellar_weight /
                        output.outputs[i].total_num_photons;
    if (runinfo.verbose >= 2) {
    ...
    lum_to_flux =
        1.0 / (4. * M_PI * pow(geometry.distance * (Constant::PC_CM), 2.0));
    //     cout << "lum_to_flux = " << lum_to_flux << endl;

    // constant to convert from ratio to surface brightness for images
    double ratio_to_sb_image =
        geometry.total_source_luminosity * lum_to_flux *
    ...
    if (runinfo.verbose >= 2) {
      ...
      cout << "# scattered photons = "
           << output.outputs[i].total_num_scattered_photons << endl;
    }
    // save the result
    if (runinfo.do_global_output) {
      ...
      runinfo.out_sed_lum[0 + runinfo.out_sed_lum_offset + (2 * i)]
                         [geometry.wave_index] = stellar_sl;
      runinfo.out_sed_lum_unc[0 + runinfo.out_sed_lum_offset + (2 * i)]
                             [geometry.wave_index] =
          stellar_sl * total_stellar_weight_err;
    }

    // compute total scattered weight uncertainty
    double total_scattered_weight_err = 0.0;
    // first compute the uncertainty on the average weight of a single scattered
    // photon
    if (output.outputs[i].total_num_photons > 0)
      total_scattered_weight_err =
          output.outputs[i].total_scattered_weight_x2 /
              output.outputs[i].total_num_scattered_photons -
          pow(output.outputs[i].total_scattered_weight /
                  output.outputs[i].total_num_scattered_photons,
              2);
    if (total_scattered_weight_err > 0.0)
      total_scattered_weight_err =
          sqrt(total_scattered_weight_err /
               output.outputs[i].total_num_scattered_photons);
    else
      total_scattered_weight_err = 0.0;
    // this converts to a fractional uncertainty which is the same as the
    // fractional uncertainty on the total scattered weight
    total_scattered_weight_err /= output.outputs[i].total_scattered_weight /
                                  output.outputs[i].total_num_scattered_photons;

    double scattered_sl = output.outputs[i].total_scattered_weight /
                          output.outputs[i].total_num_photons;
    if (runinfo.verbose >= 2) {
      cout << "Scattered weight = " << output.outputs[i].total_scattered_weight
           << endl;
    ...
    if (runinfo.do_global_output) {
      ...
      runinfo.out_sed_lum[1 + runinfo.out_sed_lum_offset + (2 * i)]
                         [geometry.wave_index] = scattered_sl;
      runinfo.out_sed_lum_unc[1 + runinfo.out_sed_lum_offset + (2 * i)]
                             [geometry.wave_index] =
          scattered_sl * total_scattered_weight_err;
      //       cout << "done." << endl;
    }
    ...
    scattered_flux =
        scattered_sl * geometry.total_source_luminosity * lum_to_flux;
    if (runinfo.verbose >= 2) {
      cout << "Scattered flux [ergs cm^-2 s^-1 A^-1] = " << scattered_flux
           << endl;
    }
    ...
    if (runinfo.do_image_output) {
      ...
      for (j = 0; j < output.image_size[0]; j++)
        for (k = 0; k < output.image_size[1]; k++) {
          // calculate the stellar uncertainty image (reuse x2 location for
          // uncertainty) first compute the uncertainty on the average weight of
          // a single stellar photon
          output.outputs[i].stellar_weight_xy_x2(j, k) =
              output.outputs[i].stellar_weight_xy_x2(j, k) /
                  output.outputs[i].num_stellar_photons_xy(j, k) -
              pow(output.outputs[i].stellar_weight_xy(j, k) /
                      output.outputs[i].num_stellar_photons_xy(j, k),
                  2);
          if (output.outputs[i].stellar_weight_xy_x2(j, k) > 0.0)
            output.outputs[i].stellar_weight_xy_x2(j, k) =
                sqrt(output.outputs[i].stellar_weight_xy_x2(j, k) /
                     output.outputs[i].num_stellar_photons_xy(j, k));
          else
          ...
          // fractional uncertainty on the total scattered weight
          output.outputs[i].stellar_weight_xy_x2(j, k) /=
              output.outputs[i].stellar_weight_xy(j, k) /
              output.outputs[i].num_stellar_photons_xy(j, k);

          // calculate the scattered uncertainty image (reuse x2 location for
          // uncertainty) first compute the uncertainty on the average weight of
          // a single scattered photon
          output.outputs[i].scattered_weight_xy_x2(j, k) =
              output.outputs[i].scattered_weight_xy_x2(j, k) /
                  output.outputs[i].num_photons_xy(j, k) -
              pow(output.outputs[i].scattered_weight_xy(j, k) /
                      output.outputs[i].num_photons_xy(j, k),
                  2);
          if (output.outputs[i].scattered_weight_xy_x2(j, k) > 0.0)
            output.outputs[i].scattered_weight_xy_x2(j, k) =
                sqrt(output.outputs[i].scattered_weight_xy_x2(j, k) /
                     output.outputs[i].num_photons_xy(j, k));
          else
          ...
          // fractional uncertainty on the total scattered weight
          output.outputs[i].scattered_weight_xy_x2(j, k) /=
              output.outputs[i].scattered_weight_xy(j, k) /
              output.outputs[i].num_photons_xy(j, k);
          ...
          total_weight_xy(j, k) = output.outputs[i].stellar_weight_xy(j, k) +
                                  output.outputs[i].scattered_weight_xy(j, k);
          total_weight_xy_x2(j, k) =
              sqrt(output.outputs[i].stellar_weight_xy_x2(j, k) *
                       output.outputs[i].stellar_weight_xy_x2(j, k) +
                   output.outputs[i].scattered_weight_xy_x2(j, k) *
                       output.outputs[i].scattered_weight_xy_x2(j, k));
      ...
      if (runinfo.dust_thermal_emission &&
          (runinfo.n_emission_grain_types > 1)) {
      ...
      if (runinfo.n_waves > 1) {
        ...
        filename +=
            "_" +
            StringManip::vtos(runinfo.wavelength[index] * Constant::CM_UM) +
            "um";
      }
      filename += ".fits";
      ...
      fits_write_comment(out_ptr, "**---------------------------------**",
                         &status);
      fits_write_comment(out_ptr, "**-Parameter File------------------**",
                         &status);
      ...
      if (runinfo.model_dust) {
        fits_write_comment(out_ptr, "**---------------------------------**",
                           &status);
        fits_write_comment(out_ptr, "**-Dust grain file-----------------**",
                           &status);
        fits_params_to_header(runinfo.dust_grain_filename, out_ptr);
        fits_write_comment(out_ptr, "**---------------------------------**",
                           &status);
        check_fits_io(status,
                      "fits_write_comment : output results (dust_grain_file)");
      }

      fits_write_key(out_ptr, TFLOAT, "RADIUS", &geometry.radius,
                     "model radius [pc]", &status);
      fits_write_key(out_ptr, TFLOAT, "DIST", &geometry.distance,
                     "distance to model [pc]", &status);

      float tdusta = float(geometry.albedo);
      fits_write_key(out_ptr, TFLOAT, "DUST_A", &tdusta, "dust grain albedo",
                     &status);
      float tdustg = float(geometry.g);
      fits_write_key(out_ptr, TFLOAT, "DUST_G", &tdustg, "dust grain g",
                     &status);
      float cur_rad_tau = geometry.tau * geometry.tau_to_tau_ref;
      fits_write_key(out_ptr, TFLOAT, "DUST_TAU", &cur_rad_tau,
                     "radial dust optical depth at current wavelength",
                     &status);
      fits_write_key(out_ptr, TFLOAT, "DUST_FF", &geometry.filling_factor,
                     "dust filling factor", &status);
      fits_write_key(out_ptr, TFLOAT, "DUST_DR", &geometry.density_ratio,
                     "dust density ratio (k2/k1)", &status);
      fits_write_key(out_ptr, TFLOAT, "DUST_K1", &geometry.clump_densities[0],
                     "k1 optical depth per pc", &status);
      fits_write_key(out_ptr, TFLOAT, "DUST_K2", &geometry.clump_densities[1],
                     "k2 optical depth per pc", &status);

      check_fits_io(status,
                    "fits_write_key : output results (geometry details)");

      fits_write_key(out_ptr, TINT, "N_OBS", &geometry.num_observers,
                     "number of observer line-of-sights", &status);
      int cur_obs_num = i + 1;
      fits_write_key(out_ptr, TINT, "CUR_OBS", &cur_obs_num,
                     "current observer line-of-sight", &status);
      float theta_deg = geometry.observer_angles[0][i] * 180.0 / M_PI;
      fits_write_key(out_ptr, TFLOAT, "LOSTHETA", &theta_deg,
                     "theta of line-of-sight [degrees]", &status);
      float phi_deg = geometry.observer_angles[1][i] * 180.0 / M_PI;
      fits_write_key(out_ptr, TFLOAT, "LOSPHI", &phi_deg,
                     "phi of line-of-sight [degrees]", &status);
      ...
      check_fits_io(
          status,
          "fits_create_image : output_results (stellar intensity/luminosity)");
      ...
      check_fits_io(
          status,
          "fits_create_image : output_results (number of stellar photons)");
      ...
      check_fits_io(
          status,
          "fits_create_image : output_results (number of scattered photons)");
