      ...
      if (runinfo.do_image_output)
        {
            ...
            for (k = 0; k < output.image_size[1]; k++)
              {
                ...
                if (output.outputs[i].stellar_weight_xy_x2 (j, k) > 0.0)
                  output.outputs[i].stellar_weight_xy_x2 (j, k) = sqrt (
                      output.outputs[i].stellar_weight_xy_x2 (j, k) / output.outputs[i].num_stellar_photons_xy (j, k));
                else
                ...
                if (output.outputs[i].scattered_weight_xy_x2 (j, k) > 0.0)
                  output.outputs[i].scattered_weight_xy_x2 (j, k) = sqrt (
                      output.outputs[i].scattered_weight_xy_x2 (j, k) / output.outputs[i].num_photons_xy (j, k));
                else
                ...
                total_weight_xy (j, k)
                    = output.outputs[i].stellar_weight_xy (j, k) + output.outputs[i].scattered_weight_xy (j, k);
                total_weight_xy_x2 (j, k) = sqrt (output.outputs[i].stellar_weight_xy_x2 (j, k)
                                                      * output.outputs[i].stellar_weight_xy_x2 (j, k)
                                                  + output.outputs[i].scattered_weight_xy_x2 (j, k)
                                                        * output.outputs[i].scattered_weight_xy_x2 (j, k));
              }
          ...
          fits_write_key (out_ptr, TFLOAT, "DUST_TAU", &cur_rad_tau, "radial dust optical depth at current wavelength",
                          &status);
          ...
          fits_write_key (out_ptr, TINT, "N_OBS", &geometry.num_observers, "number of observer line-of-sights",
                          &status);
          int cur_obs_num = i + 1;
          ...
          fits_write_img (out_ptr, TDOUBLE, 1, output.image_size[0] * output.image_size[1], &total_weight_xy[0],
                          &status);
          ...
          fits_write_img (out_ptr, TDOUBLE, 1, output.image_size[0] * output.image_size[1], &total_weight_xy_x2[0],
                          &status);
