...
//
void
output_results (output_struct &output, geometry_struct &geometry, runinfo_struct &runinfo, int index)
    ...
    for (i = 0; i < output.num_outputs; i++)
        {
            ...
            if (output.outputs[i].total_num_photons > 0)
                total_scattered_weight_err
                    = output.outputs[i].total_scattered_weight_x2 / output.outputs[i].total_num_scattered_photons
                      - pow (output.outputs[i].total_scattered_weight / output.outputs[i].total_num_scattered_photons,
                             2);
            ...
            total_scattered_weight_err
                /= output.outputs[i].total_scattered_weight / output.outputs[i].total_num_scattered_photons;
            ...
            if (runinfo.do_image_output)
                {
                    ...
                    fits_write_key (out_ptr, TFLOAT, "DUST_FF", &geometry.filling_factor, "dust filling factor",
                                    &status);
                    fits_write_key (out_ptr, TFLOAT, "DUST_DR", &geometry.density_ratio, "dust density ratio (k2/k1)",
                                    &status);
                    fits_write_key (out_ptr, TFLOAT, "DUST_K1", &geometry.clump_densities[0], "k1 optical depth per pc",
                                    &status);
                    fits_write_key (out_ptr, TFLOAT, "DUST_K2", &geometry.clump_densities[1], "k2 optical depth per pc",
                                    &status);
                    ...
                    fits_write_key (out_ptr, TFLOAT, "LOSTHETA", &theta_deg, "theta of line-of-sight [degrees]",
                                    &status);
                    ...
                    if (!finite (scattered_sl))
                        scattered_sl = 0.0;
                    fits_write_key (out_ptr, TDOUBLE, "SCAT_SL", &scattered_sl, "scattered/emitted luminosity",
                                    &status);
