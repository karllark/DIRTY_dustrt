...
// main routine
//
void output_results(output_struct &output, geometry_struct &geometry, runinfo_struct &runinfo, int index)
            ...
            for (j = 0; j < output.image_size[0]; j++)
                for (k = 0; k < output.image_size[1]; k++)
                {
                    ...
                    output.outputs[i].stellar_weight_xy_x2(j, k) =
                        ...
                        pow(output.outputs[i].stellar_weight_xy(j, k) / output.outputs[i].num_stellar_photons_xy(j, k),
                            2);
