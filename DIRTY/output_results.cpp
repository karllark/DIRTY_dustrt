...
// helper routine
void
output_2d_info (fitsfile *out_ptr, char extname[100], char itype[100])
...
//
void
output_results (output_struct &output, geometry_struct &geometry, runinfo_struct &runinfo,
                int index)
  ...
  for (i = 0; i < output.num_outputs; i++)
    {
      ...
      if (output.outputs[i].total_num_photons > 0)
        total_stellar_weight_err
            = output.outputs[i].total_stellar_weight_x2 / output.outputs[i].total_num_photons
              - pow (output.outputs[i].total_stellar_weight / output.outputs[i].total_num_photons,
                     2);
      ...
      total_stellar_weight_err
          /= output.outputs[i].total_stellar_weight / output.outputs[i].total_num_photons;
      ...
      if (runinfo.do_global_output)
        {
          ...
          runinfo.out_sed_lum[0 + runinfo.out_sed_lum_offset + (2 * i)][geometry.wave_index]
              = stellar_sl;
          runinfo.out_sed_lum_unc[0 + runinfo.out_sed_lum_offset + (2 * i)][geometry.wave_index]
      ...
      // scattered photon
      if (output.outputs[i].total_num_photons > 0)
        total_scattered_weight_err = output.outputs[i].total_scattered_weight_x2
      ...
      if (total_scattered_weight_err > 0.0)
        total_scattered_weight_err
            = sqrt (total_scattered_weight_err / output.outputs[i].total_num_scattered_photons);
      else
      ...
      double scattered_sl
          = output.outputs[i].total_scattered_weight / output.outputs[i].total_num_photons;
      if (runinfo.verbose >= 2)
        {
      ...
      if (runinfo.do_global_output)
        {
          ...
          runinfo.out_sed_lum[1 + runinfo.out_sed_lum_offset + (2 * i)][geometry.wave_index]
              = scattered_sl;
          runinfo.out_sed_lum_unc[1 + runinfo.out_sed_lum_offset + (2 * i)][geometry.wave_index]
      ...
      if (runinfo.do_image_output)
        {
          ...
          for (j = 0; j < output.image_size[0]; j++)
            for (k = 0; k < output.image_size[1]; k++)
              {
                // calculate the stellar uncertainty image (reuse x2 location
                // for uncertainty) first compute the uncertainty on the
                // average weight of a single stellar photon
                ...
                // this converts to a fractional uncertainty which is the same
                // as the fractional uncertainty on the total scattered weight
                output.outputs[i].stellar_weight_xy_x2 (j, k)
                    /= output.outputs[i].stellar_weight_xy (j, k)
                       / output.outputs[i].num_stellar_photons_xy (j, k);

                // calculate the scattered uncertainty image (reuse x2 location
                // for uncertainty) first compute the uncertainty on the
                // average weight of a single scattered photon
                ...
                // now convert the fractional uncertainty to a absolute
                // uncertainty
                output.outputs[i].stellar_weight_xy_x2 (j, k)
                    *= output.outputs[i].stellar_weight_xy (j, k);
                output.outputs[i].scattered_weight_xy_x2 (j, k)
          ...
          if (runinfo.n_waves > 1)
            {
              ...
              filename
                  += "_" + StringManip::vtos (runinfo.wavelength[index] * Constant::CM_UM) + "um";
            }
          filename += ".fits";
          ...
          // create a FITS file with extensions to fill with the output of the
          // model
          ...
          // put the dust grain file in the header (if using a dust grain
          // model)
          ...
          fits_write_key (out_ptr, TFLOAT, "RADIUS", &geometry.radius, "model radius [pc]",
                          &status);
          fits_write_key (out_ptr, TFLOAT, "DIST", &geometry.distance, "distance to model [pc]",
                          &status);
          ...
          fits_write_key (out_ptr, TFLOAT, "DUST_TAU", &cur_rad_tau,
                          "radial dust optical depth at current wavelength", &status);
          ...
          int cur_obs_num = i + 1;
          fits_write_key (out_ptr, TINT, "CUR_OBS", &cur_obs_num, "current observer line-of-sight",
                          &status);
          ...
          fits_write_key (out_ptr, TFLOAT, "LOSPHI", &phi_deg, "phi of line-of-sight [degrees]",
                          &status);
          ...
          if (!finite (stellar_sl))
            stellar_sl = 0.0;
          fits_write_key (out_ptr, TDOUBLE, "STEL_SL", &stellar_sl, "stellar/emitted luminosity",
                          &status);
          ...
          fits_write_img (out_ptr, TDOUBLE, 1, output.image_size[0] * output.image_size[1],
          ...
          fits_create_img (out_ptr, DOUBLE_IMG, 2, output.image_size, &status);
          check_fits_io (status, "fits_create_image : output_results (number "
                                 "of stellar photons)");
