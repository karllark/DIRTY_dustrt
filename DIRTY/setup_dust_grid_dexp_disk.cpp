...
void
setup_dust_grid_dexp_disk (ConfigFile &param_data, geometry_struct &geometry, random_dirty &random_obj)

{
  ...
  //     cout << "non cubical cells required -> not allowed (maybe check if
  //     this could be allowed?)" << endl; cout << "start_grid_size [pc] = " <<
  ...
  double tmp_den_constant
      = geometry.tau / (dust_scaleheight * (1.0 - exp (-1. * dust_vertical_trunc / dust_scaleheight)));
  float x_val = 0.0;
  float y_val = 0.0;
  float z_val = 0.0;
                  ...
                  if (subdivide)
                    {
...
#ifdef DEBUG_SDGDD
                      cout << "subgrid x cube size = " << geometry.grids[m].phys_grid_size[0] / float (poss_index)
                           << endl;
                      ...
                      // ensure that the first position exactly equals the edge
                      // of the parent cell
                      ...
                      // ensure that the last position exactly equals the edge
                      // of the parent cell
                      //   no roundoff error problems (hopefully)
                              ...
                              for (l = 0; l < subgrid.index_dim[0]; l++)
                                {
                                  ...
                                  tmp_density = tmp_den_constant * exp (-1.0 * abs (tz_val) / dust_scaleheight)
