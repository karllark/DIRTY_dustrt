...

{
  ...
  double dust_vertical_trunc =
      param_data.FValue("Geometry", "dust_vertical_trunc");
  check_input_param("dust_vertical_trunc", dust_vertical_trunc, 0.0,
                    geometry.radius);

  // stellar scalelength
  geometry.stellar_scalelength =
      param_data.FValue("Geometry", "stellar_scalelength");
  check_input_param("stellar_scalelength", geometry.stellar_scalelength, 0.0,
                    geometry.radius);

  // stellar scaleheight
  geometry.stellar_scaleheight =
      param_data.FValue("Geometry", "stellar_scaleheight");
  check_input_param("stellar_scaleheight", geometry.stellar_scaleheight, 0.0,
                    geometry.radius);

  // constant needed for z calculation
  geometry.stellar_emit_constant_z =
      1.0 /
      (1.0 - exp(-1.0 * dust_vertical_trunc / geometry.stellar_scaleheight));
  ...
  for (ii = 1; ii < geometry.stellar_emit_n_xy; ii++) {
    tmp_min_rad =
        ((double(ii) - 1.0) / double(geometry.stellar_emit_n_xy - 1)) *
        geometry.radius;
    tmp_rad = ((double(ii)) / double(geometry.stellar_emit_n_xy - 1)) *
              geometry.radius;
    geometry.stellar_emit_xy_vals[ii] =
        exp(-1.0 * tmp_rad / geometry.stellar_scalelength);
    geometry.stellar_emit_xy_vals[ii] *=
        (tmp_rad * tmp_rad - tmp_min_rad * tmp_min_rad);
  ...
  for (i = 0; i <= main_grid.index_dim[2]; i++) {
    tmp_val = double(i) * (2.0 * dust_vertical_trunc) /
                  double(main_grid.index_dim[2]) -
              dust_vertical_trunc;
    z_pos[i] = tmp_val;
  }
  // do xy
  for (i = 0; i <= main_grid.index_dim[0]; i++) {
    tmp_val =
        double(i) * (2.0 * geometry.radius) / double(main_grid.index_dim[0]) -
        geometry.radius;
    x_pos[i] = tmp_val;
    y_pos[i] = tmp_val;
#ifdef DEBUG_SDGDD
  ...
  // allocate main grid
  main_grid.grid.CSize(main_grid.index_dim[0], main_grid.index_dim[1],
                       main_grid.index_dim[2]);
  ...
  geometry.clump_densities[0] =
      (geometry.filling_factor +
       geometry.density_ratio * (1.0 - geometry.filling_factor));
  geometry.clump_densities[1] =
      geometry.density_ratio * geometry.clump_densities[0];

  int j, k;

  double tmp_density = 0.0;
  double tmp_den_constant =
      geometry.tau / (dust_scaleheight * (1.0 - exp(-1. * dust_vertical_trunc /
                                                    dust_scaleheight)));
  float x_val = 0.0;
  float y_val = 0.0;
  float z_val = 0.0;
  for (k = 0; k < main_grid.index_dim[2]; k++) {
    ...
    for (j = 0; j < main_grid.index_dim[1]; j++) {
      ...
      for (i = 0; i < main_grid.index_dim[0]; i++) {
        x_val =
            (main_grid.positions[0][i] + main_grid.positions[0][i + 1]) / 2.0;
        float xy_val = sqrt(x_val * x_val + y_val * y_val);
        tmp_density = tmp_den_constant *
                      exp(-1.0 * abs(z_val) / dust_scaleheight) *
        ...
        for (i = 0; i < geometry.grids[m].index_dim[0]; i++) {
          ...
          if (subdivide) {
            ...
            cout << "subgrid x cube size = "
                 << geometry.grids[m].phys_grid_size[0] / float(poss_index)
                 << endl;
            cout << "subgird x cubes (1 dim) = " << subgrid.index_dim[0]
                 << endl;
#endif
            ...
            for (o = 0; o < subgrid.index_dim[2]; o++) {
              tz_val =
                  (subgrid.positions[2][o] + subgrid.positions[2][o + 1]) / 2.0;
              for (n = 0; n < subgrid.index_dim[1]; n++) {
                ty_val =
                    (subgrid.positions[1][n] + subgrid.positions[1][n + 1]) /
                    2.0;
                for (l = 0; l < subgrid.index_dim[0]; l++) {
                  tx_val =
                      (subgrid.positions[0][l] + subgrid.positions[0][l + 1]) /
                      2.0;
