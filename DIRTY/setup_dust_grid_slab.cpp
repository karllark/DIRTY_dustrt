...
void setup_dust_grid_slab(ConfigFile &param_data, geometry_struct &geometry,
...

{
  ...
  float nonslab_density_ratio =
      param_data.FValue("Geometry", "nonslab_density_ratio");
  ...
  if (finite(geometry.max_tau_per_cell)) {
    ...
    geometry.max_tau_per_cell_x =
        param_data.FValue("Geometry", "max_tau_per_cell_x");
    check_input_param("max_tau_per_cell_x", geometry.max_tau_per_cell_x, 0.0,
                      1e10);

    geometry.max_tau_per_cell_y =
        param_data.FValue("Geometry", "max_tau_per_cell_y");
    check_input_param("max_tau_per_cell_y", geometry.max_tau_per_cell_y, 0.0,
                      1e10);

    geometry.max_tau_per_cell_z =
        param_data.FValue("Geometry", "max_tau_per_cell_z");
    check_input_param("max_tau_per_cell_z", geometry.max_tau_per_cell_z, 0.0,
                      1e10);
  }
  ...
  for (i = 0; i <= main_grid.index_dim[0]; i++) {
    tmp_val =
        double(i) * (size_xy) / double(main_grid.index_dim[0]) - size_xy / 2.;
    x_pos[i] = tmp_val;
    y_pos[i] = tmp_val;
  }
  for (i = 0; i <= main_grid.index_dim[2]; i++) {
    tmp_val =
        double(i) * (size_z) / double(main_grid.index_dim[2]) - size_z / 2.;
    z_pos[i] = tmp_val;
#ifdef DEBUG_SDG
  ...
  double nonslab_density =
      nonslab_density_ratio * geometry.tau / (slab_z2 - slab_z1);
