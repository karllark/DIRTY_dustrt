...
void output_global_results(runinfo_struct &runinfo, output_struct &output,
...

{
  ...
  if (runinfo.do_dust_emission && runinfo.do_emission_grain) {
    ...
    for (i = 0; i < runinfo.n_emission_grain_types; i++) {
      ...
      sttype[eg_offset + 1 + (4 * (i))] =
          "Flux_de_d_unc_" + grain_emission_type[i];
      sttype[eg_offset + 2 + (4 * (i))] = "Flux_de_s_" + grain_emission_type[i];
      sttype[eg_offset + 3 + (4 * (i))] =
          "Flux_de_s_unc_" + grain_emission_type[i];
    }
  }

  // now setup the needed char** variables for passing to the cfitsio routine
  //   figured this out from the CCFIT code
  //   for some reason, the strings have to be in a vector (not a single string
  //   variable)
  ...
  for (i = 0; i < int(runinfo.wavelength.size()); i++) {
    ...
    flux_total_unc.push_back(
        runinfo.out_sed_lum_unc[0][i] * runinfo.out_sed_lum_unc[0][i] +
        runinfo.out_sed_lum_unc[1][i] * runinfo.out_sed_lum_unc[1][i]);
    out_sed_lum_offset = 2;
    if (runinfo.do_ere_emission) {
      // ere direct
      runinfo.out_sed_lum[out_sed_lum_offset][i] *=
          runinfo.emitted_ere_lum[0][i] / Constant::CM_UM;
      runinfo.out_sed_lum_unc[out_sed_lum_offset][i] *=
          runinfo.emitted_ere_lum[0][i] / Constant::CM_UM;
      // ere scattered
      runinfo.out_sed_lum[out_sed_lum_offset + 1][i] *=
          runinfo.emitted_ere_lum[0][i] / Constant::CM_UM;
      runinfo.out_sed_lum_unc[out_sed_lum_offset + 1][i] *=
          runinfo.emitted_ere_lum[0][i] / Constant::CM_UM;

      // add the total ere emitted component to the total
      flux_total[i] += runinfo.out_sed_lum[out_sed_lum_offset][i] +
                       runinfo.out_sed_lum[1 + out_sed_lum_offset][i];
      flux_total_unc[i] +=
          runinfo.out_sed_lum_unc[out_sed_lum_offset][i] *
              runinfo.out_sed_lum_unc[out_sed_lum_offset][i] +
          runinfo.out_sed_lum_unc[1 + out_sed_lum_offset][i] *
              runinfo.out_sed_lum_unc[1 + out_sed_lum_offset][i];
    ...
    if (runinfo.do_dust_emission) {
      ...
      for (j = 0; j < runinfo.n_emission_grain_types; j++) {
        ...
        runinfo.out_sed_lum[out_sed_lum_offset + (2 * j)][i] *=
            runinfo.emitted_lum[0][i] / Constant::CM_UM;
        runinfo.out_sed_lum_unc[out_sed_lum_offset + (2 * j)][i] *=
            runinfo.emitted_lum[0][i] / Constant::CM_UM;
        // dust thermal emitted scattered
        runinfo.out_sed_lum[1 + out_sed_lum_offset + (2 * j)][i] *=
            runinfo.emitted_lum[0][i] / Constant::CM_UM;
        runinfo.out_sed_lum_unc[1 + out_sed_lum_offset + (2 * j)][i] *=
            runinfo.emitted_lum[0][i] / Constant::CM_UM;
      }
  ...
  double total_stellar_energy =
      NumUtils::integrate<double>(out_wavelength, out_sed_lum);
  double total_rt_direct_energy =
      NumUtils::integrate<double>(out_wavelength, runinfo.out_sed_lum[0]);
  double total_rt_scat_energy =
      NumUtils::integrate<double>(out_wavelength, runinfo.out_sed_lum[1]);
  double total_de_direct_energy = 0.0;
  double total_de_scat_energy = 0.0;
  if (runinfo.do_emission_grain) {
    total_de_direct_energy = NumUtils::integrate<double>(
        out_wavelength, runinfo.out_sed_lum[out_sed_lum_offset]);
    total_de_scat_energy = NumUtils::integrate<double>(
        out_wavelength, runinfo.out_sed_lum[out_sed_lum_offset + 1]);
  }
  if (runinfo.verbose >= 1) {
    ...
    if (runinfo.do_emission_grain) {
      ...
      cout << "Absorbed(rt) = "
           << (total_stellar_energy - total_rt_direct_energy -
               total_rt_scat_energy)
           << endl;
      cout << "Emitted(de) = "
           << (total_de_direct_energy + total_de_scat_energy) << endl;
    }
  }
  ...
  fits_create_tbl(out_ptr, ASCII_TBL, 0, tfields, &ttype[0], &tform[0],
                  &tunit[0], "Global_Outputs", &status);
  ...
  fits_write_key(out_ptr, TDOUBLE, "TOT_HMASS", &thmass, "Total H atom mass",
                 &status);
  check_fits_io(status, "fits_write_key : output_global_results");

  if (runinfo.do_dust_emission) {
    fits_write_key(out_ptr, TDOUBLE, "TOT_ABS", &runinfo.total_absorbed_energy,
                   "total absorbed energy", &status);
    check_fits_io(status, "fits_write_key : output_global_results, abs energy");
    fits_write_key(out_ptr, TDOUBLE, "TOT_EMIT", &runinfo.total_emitted_energy,
                   "total emitted energy", &status);
    check_fits_io(status,
                  "fits_write_key : output_global_results, emit energy");

    fits_write_key(out_ptr, TLONG, "N_CELLS", &geometry.num_cells,
                   "# cells in model", &status);
    check_fits_io(status, "fits_write_key : output_global_results, cells");
    fits_write_key(out_ptr, TLONG, "N_ENOUGH", &runinfo.num_cells_enough,
                   "# cells with enough for dust emission", &status);
    check_fits_io(status, "fits_write_key : output_global_results, cells");
    fits_write_key(out_ptr, TLONG, "N_NOT_EN", &runinfo.num_cells_not_enough,
                   "# cells with not enough for dust emission", &status);
    check_fits_io(status, "fits_write_key : output_global_results, cells");
    fits_write_key(out_ptr, TLONG, "N_ZERO", &runinfo.num_cells_zero,
                   "# cells with zero absorbed energy", &status);
    check_fits_io(status, "fits_write_key : output_global_results, cells");
    fits_write_key(out_ptr, TLONG, "N_FEWWAV", &runinfo.num_cells_too_few_waves,
                   "# cells with too few wavelengths for dust emission",
                   &status);
    check_fits_io(status, "fits_write_key : output_global_results, cells");
  }

  // write the columns
  fits_write_col(out_ptr, TDOUBLE, 1, 1, 0, runinfo.n_waves, &out_wavelength[0],
                 &status);
  check_fits_io(status,
                "fits_write_col : output global results, wavelength column");

  fits_write_col(out_ptr, TFLOAT, 2, 1, 0, runinfo.n_waves,
                 &runinfo.tau_to_tau_ref[0], &status);
  check_fits_io(status,
                "fits_write_col : output global results, tau_to_tau_ref");

  fits_write_col(out_ptr, TDOUBLE, 3, 1, 0, runinfo.n_waves, &out_sed_lum[0],
                 &status);
  check_fits_io(status, "fits_write_col : output global results, out_sed_lum");

  fits_write_col(out_ptr, TDOUBLE, 4, 1, 0, runinfo.n_waves, &flux_total[0],
                 &status);
  check_fits_io(status, "fits_write_col : output global results, flux_total");

  fits_write_col(out_ptr, TDOUBLE, 5, 1, 0, runinfo.n_waves, &flux_total_unc[0],
                 &status);
  check_fits_io(status,
  ...
  for (i = 0; i < n_col; i++) {
    ...
    fits_write_col(out_ptr, TDOUBLE, (7 + (4 * i)), 1, 0, runinfo.n_waves,
                   &runinfo.out_sed_lum_unc[2 * i][0], &status);
    fits_write_col(out_ptr, TDOUBLE, (8 + (4 * i)), 1, 0, runinfo.n_waves,
                   &runinfo.out_sed_lum[(2 * i) + 1][0], &status);
    fits_write_col(out_ptr, TDOUBLE, (9 + (4 * i)), 1, 0, runinfo.n_waves,
                   &runinfo.out_sed_lum_unc[(2 * i) + 1][0], &status);
    check_fits_io(
        status,
        "fits_write_col : output global results, grain emission components");
  }
