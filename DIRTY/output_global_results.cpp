...

{
            ...
            if (runinfo.do_ere_emission)
                {
                    ...
                    flux_total[i]
                        += runinfo.out_sed_lum[out_sed_lum_offset][i] + runinfo.out_sed_lum[1 + out_sed_lum_offset][i];
    ...
    if (runinfo.do_emission_grain)
        {
            ...
            total_de_scat_energy
                = NumUtils::integrate<double> (out_wavelength, runinfo.out_sed_lum[out_sed_lum_offset + 1]);
        }
            ...
            if (runinfo.do_emission_grain)
                {
                    ...
                    cout << "Absorbed(rt) = " << (total_stellar_energy - total_rt_direct_energy - total_rt_scat_energy)
                         << endl;
    ...
    if (runinfo.do_dust_emission)
        {
            fits_write_key (out_ptr, TDOUBLE, "TOT_ABS", &runinfo.total_absorbed_energy, "total absorbed energy",
                            &status);
            check_fits_io (status, "fits_write_key : output_global_results, abs energy");
            fits_write_key (out_ptr, TDOUBLE, "TOT_EMIT", &runinfo.total_emitted_energy, "total emitted energy",
                            &status);
            ...
            check_fits_io (status, "fits_write_key : output_global_results, cells");
            fits_write_key (out_ptr, TLONG, "N_ZERO", &runinfo.num_cells_zero, "# cells with zero absorbed energy",
                            &status);
            check_fits_io (status, "fits_write_key : output_global_results, cells");
    ...
    for (i = 0; i < n_col; i++)
        {
            fits_write_col (out_ptr, TDOUBLE, (6 + (4 * i)), 1, 0, runinfo.n_waves, &runinfo.out_sed_lum[2 * i][0],
                            &status);
            fits_write_col (out_ptr, TDOUBLE, (7 + (4 * i)), 1, 0, runinfo.n_waves, &runinfo.out_sed_lum_unc[2 * i][0],
                            &status);
