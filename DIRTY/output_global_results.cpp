...
void output_global_results(runinfo_struct &runinfo, output_struct &output, geometry_struct &geometry)

{
    ...
    for (i = 0; i < int(runinfo.wavelength.size()); i++)
    {
        ...
        if (runinfo.do_ere_emission)
        {
            ...
            flux_total_unc[i] +=
                runinfo.out_sed_lum_unc[out_sed_lum_offset][i] * runinfo.out_sed_lum_unc[out_sed_lum_offset][i] +
    ...
    if (runinfo.do_dust_emission)
    {
        ...
        fits_write_key(out_ptr, TLONG, "N_ENOUGH", &runinfo.num_cells_enough, "# cells with enough for dust emission",
                       &status);
        check_fits_io(status, "fits_write_key : output_global_results, cells");
    ...
    for (i = 0; i < n_col; i++)
    {
        ...
        fits_write_col(out_ptr, TDOUBLE, (7 + (4 * i)), 1, 0, runinfo.n_waves, &runinfo.out_sed_lum_unc[2 * i][0],
                       &status);
        fits_write_col(out_ptr, TDOUBLE, (8 + (4 * i)), 1, 0, runinfo.n_waves, &runinfo.out_sed_lum[(2 * i) + 1][0],
                       &status);
        fits_write_col(out_ptr, TDOUBLE, (9 + (4 * i)), 1, 0, runinfo.n_waves, &runinfo.out_sed_lum_unc[(2 * i) + 1][0],
                       &status);
