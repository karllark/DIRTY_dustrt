...
void
output_global_results (runinfo_struct &runinfo, output_struct &output, geometry_struct &geometry)

{
      ...
      if (runinfo.do_ere_emission)
        {
          ...
          flux_total_unc[i] += runinfo.out_sed_lum_unc[out_sed_lum_offset][i]
                                   * runinfo.out_sed_lum_unc[out_sed_lum_offset][i]
  ...
  if (runinfo.do_emission_grain)
    {
      total_de_direct_energy
          = NumUtils::integrate<double> (out_wavelength, runinfo.out_sed_lum[out_sed_lum_offset]);
      ...
      if (runinfo.do_emission_grain)
        {
          ...
          cout << "Absorbed(rt) = "
               << (total_stellar_energy - total_rt_direct_energy - total_rt_scat_energy) << endl;
  ...
  fits_create_tbl (out_ptr, ASCII_TBL, 0, tfields, &ttype[0], &tform[0], &tunit[0],
                   "Global_Outputs", &status);
  ...
  if (runinfo.do_dust_emission)
    {
      ...
      fits_write_key (out_ptr, TLONG, "N_FEWWAV", &runinfo.num_cells_too_few_waves,
                      "# cells with too few wavelengths for dust emission", &status);
      check_fits_io (status, "fits_write_key : output_global_results, cells");
    }
