 ...
 public: 
  ...
  inline int getFailureFlag ( string _description ) {
    ...
    while (mIter != FlagMap.end() && !_found) { 
      if (mIter->second != _description) mIter++; else _found=true; 
    }
    if (mIter != FlagMap.end()) return mIter->first; else return FailureModeNotFound; 
  }

  void AddFailure ( int _flag );
  inline void AddCellBook ( int & _maxis, int & _iaxis, int & _jaxis, int & _kaxis ) { 
    maxis[nFailure-1] = _maxis; 
    iaxis[nFailure-1] = _iaxis; jaxis[nFailure-1] = _jaxis; kaxis[nFailure-1] = _kaxis; 
  }
  inline void AddGrainInfo ( string _GrainModelName, float & _GrainSize, int & _GrainComponent ) { 
    GrainModelName[nFailure-1] = _GrainModelName; 
    GrainSize[nFailure-1]      = _GrainSize; 
    GrainComponent[nFailure-1] = _GrainComponent; 
  }
  inline void AddEnergyInfo ( double & _AbsorbedEnergy, vector <float> & _RadiationField ) { 
    AbsorbedEnergy[nFailure-1] = _AbsorbedEnergy; 
    copy(_RadiationField.begin(),_RadiationField.end(),RadiationField[nFailure-1].begin()); 
  }
