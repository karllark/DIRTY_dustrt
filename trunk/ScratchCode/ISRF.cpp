...
ISRF::ISRF(vector <float> in_wave, float in_XMMP) { 
  ...
  if (loIter != wave.end() ) {
    ...
    for (int i=loIdx;i<hiIdx;i++) theISRF[i] += _UVP[0]*exp(-pow( (wave[i]-_UVP[1])/_UVP[2],2)/2.0) 
      + _UVP[3] + _UVP[4]*wave[i] + _UVP[5]*wave[i]*wave[i];

    // Make everything below low cutoff equal to 0.0
    if (loIdx >=0) for (int i=0;i<loIdx;i++) theISRF[i] = 0.0;
  ...
  // One division as opposed to ISRF.size() divisions.  Scale the radiation field
  // by X_MMP
