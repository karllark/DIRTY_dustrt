...
namespace HeatUtils { // Define a namespace to avoid confusion with other 
  ...
  template <typename T> T getTmean (vector <T>& Temp, vector <T>& Enth, 
				    T & mpe) { 
    int nEnth=Enth.size();
    if (mpe < Enth[0]) return Temp[0]*sqrt(sqrt(mpe/Enth[0]));
    else if (mpe > Enth[nEnth-1]) return Temp[nEnth-1]*(mpe/Enth[nEnth-1]);
    else { 
  ...
  // Template prototypes. 
  // ****************************************************************************
  template <typename T> T getTauAbs (vector <T>& J, vector <T>& C, 
				     vector <T>& wave, T & mpe);
  ...
  template <typename T> T getTauAbs (vector <T>& J, vector <T>& C, 
	...
	*ii2 = (*ii1)*(*iw);
	...
	iw++;
      ...
      mpe = NumUtils::integrate(w,integrand1); 
      T TauAbs = NumUtils::integrate(w,integrand2); 
  ...
  template <typename T> T getTauRad (vector <T>& C, vector <T>& w, T mpe, T Tu)
	...
	*(itw) = *(iw); 
	iw++; 
      ...
      integrand = NumUtils::bbodyCGS(tw,Tu); 
	...
	*(ii) *= *(iC); 
	iC++; 
      ...
      ret = NumUtils::integrate(tw,integrand); 
